USE [_Desktop]
GO
/****** Object:  StoredProcedure [dbo].[SGSP_SofiaVideoData]    Script Date: 2025-10-13 07:40:52 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER PROCEDURE [dbo].[SGSP_SofiaVideoData]
    @iRecID INT
AS
BEGIN
    SET NOCOUNT ON;
    ...
    DECLARE @idCta INT, @cAlarma CHAR(3), @cZona CHAR(3), @cContenido VARCHAR(MAX);
    SELECT 
        @idCta = rec_iidcuenta, 
        @cAlarma = rec_calarma, 
        @cZona = rec_czona, 
    ...
    /*
        ...
        - sra_iid: Identificador del punto de control (solo eventos ruta; proviene de `SV_Route_AnalysisPoints.sra_iid`).
    ...
    */
    ...
    /*
        ...
        - Este stored obtiene la cuenta/alarma, identifica si el evento es `_SR` y, en ese caso,
          relaciona cada cámara con su punto de control en `SV_Route_AnalysisPoints`, devolviendo
          el JSON de configuración que se almacenó como `sra_cConfig`.
