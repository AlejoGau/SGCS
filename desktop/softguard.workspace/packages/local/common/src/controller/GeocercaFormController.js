//MIGRADO2024
Ext.define( 'Common.controller.GeocercaFormController', {
    extend: 'Ext.app.Controller',
    stores: [ 'Common.store.TablaLineasStore' ],
models: [ 'GeoGroupModel', 'TGFlotasByUserModel', 'GeocercaModel', 'GeocercaSearchModel', 'GeocercaMapModel', 'TablasLineasSearchModel', 'GeocercaCuentaModel', 'LineaByUserSearchModel' ],
views: [ 'GeocercaFormView' ],
init: function(config ) {
    this.control( {
        'geocercaformview': {
            afterrender: this.initview
        },
        'geocercaformview button[action=save]': {
            click: this.onSaveClick
        },
        'geocercaformview gmappanel6': {
...
initview: function(view ) {
    ...
    comboGrupo.bindStore( geoGroupStore );
    geoGroupStore.load();
    if( record.get( 'GeoGroup' ) != '' ) {
        view.down( '#comboGrupo' ).setValue( record.get( 'GeoGroup' ) )
    }
    ...
    if( view.readOnly ) {
        ...
        view.down( '#GeoType' ).setDisabled( true )
        view.down( '#comboFlota' ).setDisabled( true )
        view.down( '#comboGrupo' ).setDisabled( true )
        view.down( '#poiAddress' ).setDisabled( true )
        view.down( '#googlemap' ).setDisabled( true )
        view.down( '#savebtn' ).setDisabled( true )
        view.down( '#cancelbtn' ).setDisabled( true )
        view.down( '#poiButton' ).setDisabled( true )
        view.down( '#googlemap' ).setDisabled( true )
    }
},
...
onMapReady: function(gmappanel6, gmap ) {
    var me = this;
    var view = gmappanel6.up( 'geocercaformview' );
    ...
    if( metadata.get( 'Type' ) == 'circle' ) {
        ...
        var radius = metadata.get( 'Radius' );
    ...
    google.maps.event.addListener( gmappanel6.drawingManager, 'overlaycomplete', function( e ) {
        if( e.type != google.maps.drawing.OverlayType.MARKER ) {
            ...
            if( metadata.data && metadata.data.data ) {
                metadata.data.data.CenterLat = metadata.data.CenterLat;
                metadata.data.data.CenterLng = metadata.data.CenterLng;
                metadata.data.data.Radius = metadata.data.Radius;
                metadata.data.data.Type = metadata.data.Type;
                metadata.data.data.Path = metadata.data.Path;
                record.set( 'MetaData', Ext.encode( metadata.data.data ) );
            }
            else {
...
onSaveClick: function (button, event, options ) {
    ...
    var geogroup = view.down( '#comboGrupo' );
    if( !dealer.getValue() ) {
        notify( 'Debe selecionar un dealer.' );
        return
    }
    var geoGroupValue = geogroup.getValue();
    geoGroupValue = parseInt( geoGroupValue, 10 ) || 0;
    var geoGroupName = geogroup.getRawValue();
    const modelGeoCerca = controller.getGeocercaModelModel();

    record.setConfig({
        proxy: modelGeoCerca.getProxy()
    });
    
    record.set( 'GeoType', geotype.getValue() );
    record.set( 'Dealer', dealer.getValue() );
    record.set( 'Name', name.getValue() );
    record.set( 'GeoGroup', geoGroupValue );
    record.set( 'GeoGroupName', geoGroupName );
    if( myform.isValid() ) {
        ...
        view.record.save( {
            success: function( record, operation ) {
                ...
                if( vehicle && isnew == 0 ) {
                    var cuentaid = vehicle.get( 'OwnerId' ) | vehicle.get( 'cue_iid' );
