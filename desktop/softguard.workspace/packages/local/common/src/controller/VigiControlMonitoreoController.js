//MIGRADO2024
Ext.define('Common.controller.VigiControlMonitoreoController', {
    extend : 'Ext.app.Controller',
    stores : [  ],
    models : [ 'EventoPosicionSearchModel', 'SmartPanicGpsModel', 'SmartTrackSearchModel', 'GpsHistoricoSearchModel', 'RoutesPointsSearchModel' ],
    views : [ 'VigiControlGpsView' ],
    ...
    prepareMap: function(gmappanel6){
        view = gmappanel6.up('vigicontrolgpsview');
        if(myQueryString.eventId){
            view.eventId = myQueryString.eventId
        }
        if(myQueryString.objectId){
	...
	openById : function(objectId, view) {
        ...
        var smartpanicgpsview = view?view:viewport.down('vigicontrolgpsview');
        ...
        store.load({
            callback: function(records, operation, success){
                ...
                //smartpanicgpsview.record = records[0]; (dedalo esta pisando el recor original, se rompe en timeline)
                gmap.ultimasPosiciones = store;
                me.showVehicle(records[0],0,[records[0]], gmap, me);   
            }
        });
    ...
    onMapReady: function(gmappanel6,googlemap){
        var view= gmappanel6.up('vigicontrolgpsview');
            ...
            store.load({
                callback: function(records, operation, success){
                    ...
                    if (!record){
                        ...
                        var routeId = record.get('rxt_iRouteID');
                        if(routeId!=0){//me fijo si tiene ruta y la muestro
                            controller.showRoute(routeId, view);
                        }
                        
                        if (!view.preventPlayer){
                            ...
                            gmappanel6.ultimasPosiciones = store;
                            controller.showVehicle(records[0],0,[records[0]], gmappanel6, controller);  
                        }
                    }
                }
            });
    ...
    showVehicle: function(vehicle,index,array,gmappanel6,controller){
        if (!vehicle)
            return false;
        
        if (!gmappanel6) {gmappanel6 = this.gmappanel6}
    ...
    showHistory: function(map, vehicle, event){
        var controller = this;
        var view = map.up('vigicontrolgpsview');
        var store = Ext.create('Ext.data.Store',{
        ...
        var routeId = view.event.get('rxt_iRouteID');
        if(routeId!=0){//me fijo si tiene ruta y la muestro
            controller.showRoute(routeId, view);
        }
        ...
        store.load({callback: function(records){
            ...
            */
            ...
            if (center){
                var bounds = controller.getBounds(map.cache.marker);
    ...
    getHistoryMarkerIcon: function(i,total,old,record, map){
        ...
        switch (i)
        {
            case 0:
                ...
                var tipoIcon = 'icon_dot_verde';
                
                var iconUrl = '/resources/softguard/images/'+tipoIcon+'.gif';
                var image = new google.maps.MarkerImage(
                    iconUrl,
                    new google.maps.Size(48,48),
                    new google.maps.Point(0,0),
