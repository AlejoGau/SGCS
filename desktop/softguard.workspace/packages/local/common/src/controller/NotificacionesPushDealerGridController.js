Ext.define('Common.controller.NotificacionesPushDealerGridController', {
    ...
    onDeleteClick : function(button, event, options) { 
        ...
        if (selection) {
            ...
            */
           var model = controller.getNotificacionesPushDealerModelModel();
              Ext.Array.each(selection, function (rec) {
                model.load(rec.getId(), {
                    callback: function(recordToErase, operation) {
                        recordToErase.erase({
    ...
    onSelectionChange:function (model, selected, eOpts) {
           model.view.up('notificacionespushdealergridview').down('[action="delete"]').setDisabled(selected.length == 0);
    }    
});