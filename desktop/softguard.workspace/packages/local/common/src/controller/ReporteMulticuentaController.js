//MIGRADO2024
Ext.define('Common.controller.ReporteMulticuentaController', {
    extend: 'Ext.app.Controller',
    stores: ['Common.store.TablasGruposStore', 'Common.store.EventoEstadoStore', 'Common.store.EventoOrigenStore'],
    models: ['EventoSonidoSearchModel', 'TablasGruposSearchModel', 'TablasCodigosAlarmaSearchModel', 'ComboEventosModel', 'TablaHistoricoSearchModel', 'CuentaRecepcionModel'],
    views: ['EventoSonidoButtonView', 'MulticuentaGridView'],
    interval: 10000,
    init: function (config) {
        this.control({
            ...
            'multicuentagridview button[action=search]': {
                click: this.onBuscarClick
            },
            'multicuentagridview #comboEstados': {
                //	select : this.onComboSelect
            },
            'multicuentagridview #comboOrigenes': {
                //	select : thizs.onComboSelect
            },
            'multicuentagridview #comboTipos': {
                //	select : this.onComboSelect
            },
            'multicuentagridview #dealer': {
                //	change : this.onComboSelect
            },
            'multicuentagridview button[action=play]': {
                click: this.onPlayClick
            },
            'multicuentagridview button[action=stop]': {
                click: this.onStopClick
            },
            'multicuentagridview button[action=groupAlarmas]': {
                click: this.onGroupAlarmasClick
            },
            'multicuentagridview button[action=groupPrioridad]': {
                click: this.onGroupPrioridadClick
            },
            'multicuentagridview button[action=groupCuenta]': {
                click: this.onGroupCuentaClick
            },
            'multicuentagridview #grupos': {
            ...
            'multicuentagridview #grupos-excluir': {
                //	select : this.onGrupoExcluirChange,
                //  change : this.onGrupoExcluirChangeClear
            },
            'multicuentagridview #procesartodosfull': {
                click: this.onProcesarTodosFullClick
            },
            'multicuentagridview button[action=soloAlarmas]': {
                click: this.onSoloAlarmasClick
            },
            'multicuentagridview #clearfilters': {
    ...
    initView: function (view) {
        ...
        var storeComboConexiones = Ext.create('Ext.data.Store', {
            model: 'Common.model.ConexionesModel',
            pageSize: 500,
            remoteSort: false,
            remoteFilter: false,
            filters: [
        ...
        var mystore = Ext.create('Ext.data.Store', {
            model: 'Common.model.EventosTiempoRealModel',
            remoteGroup: false,
            remoteSort: true,
            remoteFilter: true,
            autoDestroy: true,
            ...
            listeners: {
                ...
                load: function (store) {
                    // Cada vez que se carga el store, emite el evento
        ...
        mygrid.bindStore(mystore, false);
        mystore.grid = mygrid;
        mystore.view = view;
        view.allowAutoLoad = false;

        view.down('#play').show();
        view.down('#stop').hide();
        Ext.Msg.alert('Información', 'Debe aplicar al menos un filtro para ver los eventos.');
    ...
    onClearFiltersClick: function (button, event, options) {
        var view = button.up('multicuentagridview');
    ...
    onDealerChange: function (field, newValue, oldValue) {
        var view = field.up('multicuentagridview');
    ...
    onActivate: function (view) {
        if (view.allowAutoLoad) {
            this.loadData(view);
        } else {
            console.log('Vista activada, pero sin carga automática');
        }
    },
    ...
    onGrupoChange: function (combo, records, options) {
        var view = combo.up('multicuentagridview');
    ...
    onBuscarClick: function (button, event, options) {
        ...
        var playButton = Ext.ComponentQuery.query('#playSonidoEventos')[0];
        if (playButton) {
            playButton.toggle(true);       // esto lo deja presionado y dispara evento toggle
            playButton.pressed = true;     // ?? fuerza el flag interno
            playButton.fireEvent('toggle', playButton, true); // ?? dispara el handler
        }
    },
    ...
    onBeforeload: function (store, operation, options) {
        ...
        if (origenes) {
            var val = origenes.getValue();
            if (val && Ext.isArray(val) && val.length > 0) {
                params.Origenes = val.join();
            } else {
                params.Origenes = ''; // o simplemente no lo agregues
            }
        }
        ...
        if (tipos) {
            var val = tipos.getValue();
            if (val && Ext.isArray(val) && val.length > 0) {
                params.Tipos = val.join();
            } else {
                params.Tipos = ''; // o simplemente no lo agregues
            }
        }
        ...
        if (prioridad) {
            var val = prioridad.getValue();
            if (val && Ext.isArray(val) && val.length > 0) {
                params.Prioridad = val.join();
            } else {
                params.Prioridad = ''; // o simplemente no lo agregues
            }
        }
    ...
    onItemClick: function (grid, record, item, index, e, options) {
        ...
        var view = grid.up('multicuentagridview') ? grid.up('multicuentagridview') : grid;
