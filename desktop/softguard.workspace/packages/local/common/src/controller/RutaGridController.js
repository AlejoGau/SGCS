//MIGRADO2024
Ext.define('Common.controller.RutaGridController', {
    extend : 'Ext.app.Controller',
    stores : [  ],
    models : [ 'DesktopModuleDetailByUserModel', 'GeocercaCuentaModel', 'GeocercaMapModel', 'GeocercaModel', 'GeocercaSearchModel', 'TGFlotasByUserModel' ],
    views : [ 'RutaGridView' ],
    init : function(config) {
		...
		this.control({
            ...
            'rutagridview button[action=delete]': {
                click: this.onDeleteClick
            },
            'rutagridview button[action=add]': {
                click: this.onAddClick
            },
            'rutagridview button[action=save]': {
                click: this.onSaveClick
            },
            'flotagpsview' : {
                vehicleSelected: this.onVehicleSelected
            },
            'rutagridview button[action=geoAssign]': {
                click: this.onGeoAssignClick
            },
            'rutagridview button[action=geoDesAssign]':{
    ...
    onVehicleSelected: function(record, view){
        ...
        if(dataPanel)
            var grid = dataPanel.down('rutagridview');
    ...
    loadData: function (view) {
        ...
        */
         ...
         *      10/12/2020
         *      https://basecamp.com/2249105/projects/12939010/todos/429452509
         */
        filters.push(
            {
        ...
        if (record) {
            ...
            if (profile >= 2){
                ...
                var disponible =Ext.create('Ext.data.Store',{
                    model: this.getGeocercaModelModel(),
                    remoteFilter: true,
                    filters: [
                        {
                            property: 'Cuenta:NOT',
    ...
    onItemDblClick: function(view,record,item,index,e,options){
        ...
        var mylong = 0;
        var me = this;
        var pais = view.pais?view.pais:view.up('rutagridview').pais;
...
/*
    ...
    onDeleteClick: function(button, object, options){
        var view= button.up('rutagridview');
    ...
    onAddClick: function(button, object, options){
        ...
        var rec = this.getGeocercaModelModel().create({
            Name: getLocale('Nueva Ruta')+' ('+(view.store.count()+1)+')',
    ...
    onGeoDesAssignClick: function (button, event, options ) {
        ...
        geocercaAsignadaStore.load( {
            callback: function( records ) {
                if( records.length > 0 ) {
                    records[0].setConfig({
                        proxy: controller.getGeocercaCuentaModelModel().getProxy()
                    });
                    records[ 0 ].destroy( {
