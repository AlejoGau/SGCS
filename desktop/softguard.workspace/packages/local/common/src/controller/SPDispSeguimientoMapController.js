//MIGRADO2024
Ext.define('Common.controller.SPDispSeguimientoMapController', {
  extend: 'Ext.app.Controller',
  stores: [],
  models: [
    'GeocercaMapModel',
    'SmartPanicsGeocercasSearchModel',
    'SmartPanicSearchModel',
    'SmartPanicModel',
    'p_posicionesSPModel'
  ],
  views: ['SPSeguimientoMapView', 'DateTimeField'],
  init: function (config) {
    // genero los eventos
    this.control({
      spseguimientomapview: {
        beforerender: this.initview
      },
      'spseguimientomapview #gridpuntos': {
        itemclick: this.onPuntosItemClick
      },

      'spseguimientomapview gmappanel6': {
        mapready: this.onMapReady
      },
      'spseguimientomapview button[action=filterprecision]': {
        click: this.onFilterPrecisionClick
      },
      'spseguimientomapview button[action=Buscar]': {
        click: this.onBuscarClick
      },
      'spseguimientomapview button[action=tiempo-1]': {
        click: this.onTiempo1Click
      },
      'spseguimientomapview button[action=tiempo-2]': {
        click: this.onTiempo2Click
      },
      'spseguimientomapview button[action=tiempo-3]': {
        click: this.onTiempo3Click
      },
      'spseguimientomapview button[action=tiempo-4]': {
        click: this.onTiempo4Click
      },
      'spseguimientomapview button[action=posicion-1]': {
        click: this.onPosicion1Click
      },
      'spseguimientomapview button[action=posicion-2]': {
        click: this.onPosicion2Click
      },
       'spseguimientomapview button[action=center]': {
                toggle: this.onCenterClick   // usar toggle porque el botón es enableToggle
        },
    })
  }, // cierro init
  ...
  onCenterClick: function(btn, pressed) {
    var view = btn.up('spseguimientomapview');
    ...
    if (pressed) {
        ...
        var active = gmappanel.cache && gmappanel.cache.marker
            ? Ext.Array.filter(gmappanel.cache.marker, function(item) {
                return item.getMap();
            })
            : [];
        ...
        if (active.length > 0) {
            ...
            // 🔎 Ajuste de zoom máximo
            var listener = google.maps.event.addListenerOnce(map, 'bounds_changed', function() {
                if (map.getZoom() > 15) {  // podés cambiar 15 por el nivel deseado
                    map.setZoom(15);
                }
            });
        }

        // Si hay vehículo seleccionado, zoom directo a él
        if (view.vehicleSelected && view.vehicleSelected.position) {
            map.setCenter(view.vehicleSelected.position);
            map.setZoom(16); // zoom más cercano al vehículo seleccionado
        }
  ...
  onMapReady: function (gmappanel6, googlemap) {
    var view = gmappanel6.up('spseguimientomapview')
  ...
  onBuscarClick: function (button) {
    var view = button.up('spseguimientomapview')
    ...
    geocoder.geocode({ location: latlng }, function (results, status) {
      if (status === 'OK') {
        if (results[0]) {
          ...
          divDireccion[0].style.display = 'none'
        }
      } else {
  ...
  onPuntosItemClick: function (gridview, record, item, index, e, options) {
    ...
    var view = puntos.up('spseguimientomapview')
    ...
    if (record.marker) {
      ...
      record.get('sp_rAccuracy') > view.rangoPrecision &&
      filterprecision
    ) {
      notify(
        'Punto fuera de rango de precisión de: ' + view.rangoPrecision + ' mts'
      )
    } else {
      gmappanel6.clearMarkers()
  ...
  showMarker: function (view, record) {
    ...
    var point = {
      ...
      fecha: record.get('sp_tfechahora')
    }
    var marker = {
      marker: null,
      lat: record.get('sp_rLatitud'),
  ...
  showHistory: function (map, vehicle, event) {
    var controller = this
    var view = map.up('spseguimientomapview')
    ...
    store.each(function (record, index, total) {
      ...
      if (
        ...
        markers.push({
          ...
          record: record,
          title: Ext.Date.format(record.get('sp_tfechahora'), 'Y-m-d H:i:s'),
          icon: controller.getHistoryMarkerIcon(
            index,
            total,
            null,
            record,
  ...
  getHistoryMarkerIcon: function (i, total, old, record, map) {
    ...
    if (record) {
      ...
      if (record.get('rec_calarma')) {
        iconUrl =
          '/resources/softguard/images/spEventIcons/' +
        ...
        iconUrl = old
          ? '/resources/softguard/images/icon_dot_verde.gif'
          : '/resources/softguard/images/icon_dot-nonew.gif'
        var image = new google.maps.MarkerImage(
          iconUrl,
  ...
  getHistoryInfoWindowHtml: function (vehicle, gps) {
    ...
    html = html.replace(
      /\{fecha\}/,
      Ext.Date.format(gps.get('sp_tfechahora'), 'd-m-Y H:i:s')
    )
    html = html.replace(
      /\{precision\}/,
      Ext.util.Format.number(gps.get('sp_rAccuracy'), '0')
    )
    html = html.replace(
      /\{batt\}/,
      gps.get('sp_iBatt') ? gps.get('sp_iBatt') : getLocale('Sin valor')
    )
    html = html.replace(/\{rumbo\}/, gps.get('sp_iRumbo'))
    html = html.replace(
      /\{evento\}/,
      gps.get('rec_calarma')
        ? gps.get('cod_cdescripcion')
        : getLocale('Seguimiento')
    )
