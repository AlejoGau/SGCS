Ext.define('Common.controller.m_usuariosFormController', {
    extend : 'Ext.app.Controller',
    stores : [ 'SoftguardUsuarioTipoStore' ],
    models : [ 'Common.model.CuentaSearchModel', 'Common.model.m_usuariosSearchModel', 'Common.model.m_usuariosModel' ],
	views : [ 'Common.view.m_usuariosFormView' ],
	...
	saveObject : function(button, event, options) {
        ...
        if (myform.isValid()){
            ...
            record.set('usu_cmetadata', Ext.encode({
               ...
               seguroVto: view.down('#seguroVto') ? Ext.Date.format(view.down('#seguroVto').getValue(),'c') : '',
               seguroCia: view.down('#seguroCia') ?  view.down('#seguroCia').getValue(): '',
               vtv: view.down('#vtv') ? Ext.Date.format( view.down('#vtv').getValue(), 'c' ) : '',
