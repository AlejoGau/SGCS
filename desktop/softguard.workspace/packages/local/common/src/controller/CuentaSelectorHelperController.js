Ext.define('Common.controller.CuentaSelectorHelperController', {
    extend : 'Ext.app.Controller',
    stores : [ /*'SoftguardAlarmasSmsStore'*/ ],
    models : [ 'CuentaSearchModel' ],
views : [ 'CuentaSelectorHelperView'/*, 'EventSelectField'*/ ],

    init : function(config) {
        // genero los eventos
    	this.control({
			'cuentaselectorhelperview' : {
            ...
            'cuentaselectorhelperview #agregar' : {
    			click: this.onAgregarClick
			},
            'cuentaselectorhelperview #quitar' : {
        		click: this.onQuitarClick
			},
            'cuentaselectorhelperview #listo' : {
        		click: this.onListoClick
			},
            'cuentaselectorhelperview #buscar' : {
            	click: this.onBuscarClick
			},
            'cuentaselectorhelperview #todos' : {
                click: this.onTodosClick
			},
            'cuentaselectorhelperview #gridtodos' : {
                itemclick: this.onItemDblClick,
                itemdblclick: this.onItemDblTodosClick
        	},
            'cuentaselectorhelperview #gridselecionados' : {
    ...
    onEventoClick: function (btn) {
        var view = btn.up('cuentaselectorhelperview');
        ...
        view.eventosSeleccionados = view.down('#codevento').getValue()
         ...
         var myWindow = Ext.widget('window',{
            ...
            closeAction:'destroy',
            items: [{
                xtype: 'cuentaselectorhelperview',
    ...
    onItemDblSeleccionadosClick: function(view,record,item,index,e,options){        
        ...
        var view = view.up('cuentaselectorhelperview')
        var  controller = this;
        if(!view.simpleSelect) {
            ...
            todos.getStore().addSorted(controller.getCuentaSearchModelModel().create(Ext.clone (record.data))) 
    ...
    onItemDblTodosClick: function(view,record,item,index,e,options){        
        ...
        if(!view.simpleSelect) {
            ...
            grillaselecionados.getStore().addSorted(
                controller.getCuentaSearchModelModel().create(Ext.clone (record.data))
            )                
    ...
    onBuscarClick: function (btn) {
        ...
        Ext.Array.push(filter,{
            property:'cue_ncuenta:ORcue_cnombre',
            //id: 'vb.[Name]',
            value:view.down('#query').getValue()
        })
        view.combostore.filter(filter);

        //view.combostore.proxy.extraParams={'cuenta':view.down('#query').getValue()};
    ...
    onListoClick:  function (btn) {
        ...
        /*if(view.limitEventSelect) {
         ...
         view.caller.fireEvent(view.toEvent?view.toEvent:'selectedLotes',view.down('#gridselecionados').getStore().data, view.caller)      
         //view.caller.fireEvent('selectedLotes',view.down('#gridselecionados').getStore().data, view.caller)      
    ...
    onAgregarClick :  function (btn) {
        ...
        if (selection) {
            ...
            Ext.Array.each(selection, function (rec) {
                ...
                grillaselecionados.getStore().addSorted(controller.getCuentaSearchModelModel().create(Ext.clone (rec.data)))                
    ...
    onQuitarClick :  function (btn) {
        ...
        if (grillaselecionados) {
            ...
            Ext.Array.each(grillaselecionadosseleccion, function (rec) {
                ...
                todos.getStore().addSorted(controller.getCuentaSearchModelModel().create(Ext.clone (rec.data))) 
            });
