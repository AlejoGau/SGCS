Ext.define( 'Common.controller.EventObservacionesFormGuiadoController', {
    ...
    views: [ 'EventObservacionesFormGuiadoView' ],
    init: function(config ) {
        // genero los eventos
        this.control( {
            'eventobservacionesformguiadoview': {
            ...
            'eventobservacionesformguiadoview button[action="agregar-observacion"]': {
                click: this.onAgregarObservacionClick
            },
            'eventobservacionesformguiadoview button[action="save"]': {
                click: this.onAgregarObservacionClick
            },
            'eventobservacionesformguiadoview #observaciones': {
                select: this.onSelectPredefinidas
            },
            'eventobservacionesformguiadoview #categorizacion': {
                change: this.onChangeCategorizacion
            },
            'eventobservacionesformguiadoview #resolucion': {
                change: this.onChangeResolucion
            },
            'eventobservacionesformguiadoview button[action="openView"]': {
                click: this.onOpenViewClick
            },
            'eventobservacionesformguiadoview button[action="confirm"]': {
                click: this.onConfirmClick
            },
            'eventobservacionesformguiadoview button[action="cancel"]': {
                click: this.onCancelClick
            }
        });
    }, // cierro init
    onConfirmClick: function(btn) {
        var view = btn.up( 'eventobservacionesformguiadoview' );
        var obsfield = view.down( '#obsfield' );    
        var obs = obsfield.getValue();
        var categorizacion = view.down( '#categorizacion' ).getValue();
        var resolucion = view.down( '#resolucion' ).getValue();
        if(view.caller){
            view.caller.record.set( 'rec_cObservaciones', obs );
            view.caller.record.set( 'rec_idresolucion', categorizacion );
            view.caller.record.set( 'rec_ccategorizacion', resolucion );
        }
    ...
    onSelectPredefinidas: function(combo, records ) {
        var view = combo.up( 'eventobservacionesformguiadoview' );
    ...
    initview: function(view ) {
        ...
        //-------agrego lógica de monitoreo guiado. Como resolución y categorización son obligatorias, están
        // en pasos separados voy haciendo asignaciones parciales por cada ventana de cada paso. Entonces, asigno el valor
        // al iniciarse la vista.
        if(view.caller){
            view.down('#categorizacion').setValue(view.caller.record.get('rec_idresolucion'));
            view.down('#resolucion').setValue(view.caller.record.get('rec_ccategorizacion'));
            view.down('#obsfield').setValue(view.caller.record.get('rec_cObservaciones'));
        }
    },    
   decimalColorToHTMLcolor : function(number) {
    ...
    onAgregarObservacionClick: function(button, object, options ) {
        var view = button.up( 'eventobservacionesformguiadoview' );
