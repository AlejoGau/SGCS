...
Ext.define('Common.view.SMPAttachGridView', {
    extend : 'Ext.grid.GridPanel',
    alias : ['widget.smpattachsearchview','widget.smpattachgridview'],
    title : 'Adjuntos',
    autoHeight : true,
    hideHeaders: true,
    searchName: 'SmartMailAttachment',
    columns : [{
            xtype:'actioncolumn',
            header: '',
            width: 40,
            items: [
                {
                    ...
                    handler: function(grid, rowIndex, colIndex,item, event) {
                        var view = grid.up('smpattachsearchview');
            			...
            			fileuploaded: function(uploader, file){
                            ...
                            if (record){
                                ...
                                if (record.get('ObjectTypeName') == 'SmartMailProgram'){
                                    var attach = Ext.create('Common.model.SmartMailProgramAttachModel',{
                                        Id:0,
                                        Name: filename,
                                    ...
                                    attach.save({callback: function(attach, operation){
                                        ...
                                        var relation = Ext.create(relationModel,{
                                            ...
                                            ObjectId: record.get('Id'),//AQUI TIENE QUE IR LE id del ActionMail
                                            RelationObjectTypeId: attach.get('ObjectTypeId'),
                                            RelationObjectId: attach.get('Id')
                                        });
