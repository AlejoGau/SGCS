-- DK-1520: SP MG_ContratoAFactura actualizado
-- Cambio: cuando el contrato tiene cnt_cantidad_auto = 1,
-- la cantidad de sus items se calcula como el número de cuentas activas del cliente.
-- La configuración a nivel producto (pro_cantidad_auto = 1) sigue teniendo prioridad.
...
BEGIN
        ...
        declare @cnt_cantidad_auto int = 0
        ...
        SELECT @idcliente = cnt_idcliente
                ...
                ,@cnt_cantidad_auto = ISNULL(cnt_cantidad_auto, 0)
        ...
        print '[MG_ContratoAFactura] Modo cantidad automática del contrato: ' + CONVERT(varchar, @cnt_cantidad_auto)
        print '[MG_ContratoAFactura] Cuentas activas del cliente: ' + CONVERT(varchar, @cuentasActivas)

        SELECT
                @totalItemsSinImpuestos = ISNULL(sum(
                        (CASE con.cnt_dinamico WHEN 1 THEN mglpd.mglpd_valor ELSE it.Price END) * qty.cantidad_efectiva
                ), 0)
                ,@Total = ISNULL(sum(
                        (CASE con.cnt_dinamico WHEN 1 THEN mglpd.mglpd_valor ELSE it.Price END) * qty.cantidad_efectiva * (1 + ISNULL(imp_nporcentaje, 0) / 100)
                ), 0)
                ...
                outer apply (
                    select case
                        when ISNULL(p.pro_cantidad_auto, 0) = 1 then CAST(@cuentasActivas AS float)
                        when ISNULL(con.cnt_cantidad_auto, 0) = 1 then CAST(@cuentasActivas AS float)
                        else ISNULL(it.Quantity, 0)
                    end as cantidad_efectiva
                ) qty
                WHERE cnt_iid = @IdContrato
                  AND qty.cantidad_efectiva > 0
                group by cnt_idcliente, cbt_ccodigo, cli_iorganizacion

        if ISNULL(@totalItemsSinImpuestos, 0) <= 0
        BEGIN
                print '[MG_ContratoAFactura] el contrato no tiene items facturables para este período'
                ROLLBACK TRAN
                set noexec on
        END
        ...
        DECLARE items_contrato_cursor CURSOR LOCAL FOR
                SELECT
                        ...
                        qty.cantidad_efectiva as cbi_icantidad,
                        (CASE con.cnt_dinamico WHEN 1 THEN mglpd.mglpd_valor ELSE it.Price END) * qty.cantidad_efectiva as cbi_yimporte,
        ...
        if @items = 0
        BEGIN
                print '[MG_ContratoAFactura] no se encontraron items para el contrato'
                rollback tran
        END
