Ext.define('SmartPanics.controller.SmartPanicsPendingGridController', {
    ...
    onDeleteClick: function (button, event, options) {
        ...
        if (selection.length) {
            for (var sel in selection) {
                model.load(selection[sel].get('Id'), {
        ...
        Ext.Array.each(selection, function (record, key) {
            ...
            store.load({
                callback: function (records) {
                    ...
                    /*Federico V. comente este condicional porque es raro que pida que el length este igualod a 0. Esto ocasionaba que no puedas asignar nuevamente una cuenta que ya tuvo asignada.
                    if(records.length == 0){
        ...
        if (view.QtyUsers != 0) {
            ...
            store.load(function () {
                ...
                if (asignados == view.QtyUsers) {
                    ...
                    view.down('[action="delete"]').setDisabled(true);
                    ...
                    view.down('[action="delete"]').setDisabled(false);
                    if (callback) {
                        callback();
                    }
                }
