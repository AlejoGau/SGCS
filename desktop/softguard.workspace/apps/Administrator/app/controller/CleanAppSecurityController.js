Ext.define('Administrator.controller.CleanAppSecurityController', {
    extend : 'Ext.app.Controller',
    stores : [ 'EventSecurityModuleStore', 'CleanAppSecurityModulesStore' ],
    models : [ 'ModuleModel' ],
    views : [ 'EventSecurityView', 'CleanAppSecurityView' ],
    ...
    init : function(config) {
    	...
    	this.control({
            'CleanAppSecurity button[action=saveSecurity]': {
            ...
            'CleanAppSecurity[moduleFlag=vigicontrol] button[action=refreshModules]': {
                click: this.onRefreshModulesClick
            },
            'CleanAppSecurity button[action=applyPerfil]': {
                click: this.onApplyPerfilClick
            },
            'EventSecurity[moduleFlag=cleanapp]' : {
                beforerender : this.initEventView
        	},
            'EventSecurity[moduleFlag=cleanapp] button[action=saveEvent]': {
                click: this.onSaveEventClick
            },
            'EventSecurity[moduleFlag=cleanapp] button[action=applyPerfilEventos]': {
                click: this.onApplyPerfilEventClick
            },
            'EventSecurity[moduleFlag=cleanapp] button[action=refreshModulesEvent]': {
    ...
    onApplyPerfilEventClick : function(button, event, options) {
        var view = button.up('EventSecurity[moduleFlag=cleanapp]');
    ...
    onRefreshModulesClick : function(button, event, options) {
        var view = button.up('CleanAppSecurity')?button.up('CleanAppSecurity'):button;
        var gridstore = view.getStore();
        var store = this.getCleanAppSecurityModulesStoreStore();
        var cantidad = store.data.length-1;
    ...
    onApplyPerfilClick : function(button, event, options) {
        var view = button.up('CleanAppSecurity');
        var selmodel = view.getSelectionModel();
        var selection = selmodel.getSelection();
    ...
    onSaveEventClick : function(button, event, options) {
        this.onSaveClick(button.up('tabpanel').down('CleanAppSecurity'))
    },
    ...
    setSecurity: function(view){
        var security = view.security;
        var securityStore =  this.getCleanAppSecurityModulesStoreStore();
	...
	onSaveClick : function(button, event, options) {
        ...
        var viewEvent = view.up('tabpanel').down('EventSecurity[moduleFlag=cleanapp]');
