Ext.define( 'SgAppSerTec.controller.TecguardAllSeguimientoMapController', {
    extend: 'Ext.app.Controller',
    stores: [  ],
models: [ 'TecguardCuentaSeguimientoModel' ],
views: [ 'TecguardAllSeguimientoMapView' ],

init: function(config ) {
    this.control( {
        'tecguardallseguimientomapview #googlemap': {
            mapready: this.onMapReady,
            //beforerender : this.prepareMap,
            markersChange: this.onMarkersChange
        },
        'tecguardallseguimientomapview cuentagridview': {
            selectionchange: this.onSelectionChange,
        },
        'tecguardallseguimientomapview': {
            //afterrender : this.initView,
            vehicleSelected: this.onVehicleSelected
        },
        'tecguardallseguimientomapview #searchall': {
            click: this.onClickSearchAll
        },
        'tecguardallseguimientomapview #btnAddress': {
            click: this.onAddressClick
        },
        'tecguardallseguimientomapview button[action=center]': {
...
onSelectionChange: function(selectionModel, records, options ) {
    ...
    var viewport = view.up( 'tecguardallseguimientomapview' );
    var gmap = view.map;
...
onClickSearchAll: function (view ) {
    var view = view.up( 'tecguardallseguimientomapview' ) ? view.up( 'tecguardallseguimientomapview' ) : view;
    var filters;
    ...
    var store = Ext.create( 'Ext.data.Store', {
        model: this.getTecguardCuentaSeguimientoModelModel(),
        remoteFilter: true,
        pageSize: 500,
        filters: filters,
        sorters: [
...
onMapReady: function(gmappanel6 ) {
    var view = gmappanel6.up( 'tecguardallseguimientomapview' );
...
onAddressClick: function(button ) {
    /* var form = button.up( 'form' );
     var gmappanel6 = Ext.getCmp( 'gmappanel6' ) ? Ext.getCmp( 'gmappanel6' ) : button.up( 'tecguardallseguimientomapview' ).down( 'gmappanel6' );
     var address = form.down( '#address' ).getValue();
     const view = button.up( 'tecguardallseguimientomapview' );
     ...
     store.load(records => gmappanel6.vehicleList = records);
 
     const newVehicleList = gmappanel6.vehicleList.filter( vehicle => vehicle.address?.toLowerCase().includes( address.toLowerCase() ) );
 
     gmappanel6.vehicleList = newVehicleList;
     gmappanel6.clearMarkers();
     controller.showMarkerArray( gmappanel6, controller );*/

    var form = button.up( 'form' );
    var view = button.up( 'tecguardallseguimientomapview' );
...
showMarkerArray: function(gmappanel6, controller ) {
    ...
    var center = view.down( '#centerBtn' ).pressed;
...
onVehicleSelected: function(record, view ) {
    ...
    var mon = Ext.widget( 'tecguardseguimientomapview', {
        title: title,
        record: record,
        translate: false,
        closable: true,
