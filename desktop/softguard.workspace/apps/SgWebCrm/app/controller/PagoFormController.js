Ext.define("SGWebCrm.controller.PagoFormController", {
  ...
  models: [
    ...
    "t_bancos_fcSearchModel",
    "t_firmante_fcSearchModel",
  ...
  onRealizarPagoClick: function (btn) {
    ...
    if (
      view.down("#recibos").getValue() == "" ||
      view.down("#recibos").getValue() == null
    ) {
      ...
      success: function (response, action) {
        ...
        if (json.rows[0].Error == "1") {
          ...
          Ext.MessageBox.alert(
            getLocale(json.rows[0].msg),
            getLocale(json.rows[0].msg) +
              "<br>" +
              getLocale("No fue realizado el pago."),
            function () {}
          );
        } else {
          notify(json.rows[0].msg);
  ...
  onCalcularContadores: function (view, rec, pagarTodo) {
    ...
    if (view.down("#cardspanel").getLayout().activeItem.itemId == "card-0") {
      ...
      view.down("#cardspanel").getLayout().activeItem.itemId == "card-1"
    ) {
    ...
    if (
      ...
      if (view.down("#forma").getValue() == "003") {
        view.pagosStore.add({
          ...
          vencimiento: Ext.Date.format(
            view.down("#vencimiento").getValue(),
            "d/m/Y"
          ),
        });
      } else {
  ...
  initview: function (view) {
    ...
    view.TipoComprobanteStore.load({
      callback: function () {
        ...
        storeUltimoMovimiento.load({
          callback: function (recordsUltimoMovimiento) {
            if (recordsUltimoMovimiento.length > 0) {
              ...
              view
                .down("#fecharecibo")
                .setMinValue(recordsUltimoMovimiento[0].get("cta_dCobro"));
    ...
    comprobanteRecibo.load({
      callback: function (records) {
        if (records.length <= 0) {
          Ext.MessageBox.alert(
            getLocale("Faltan recibos"),
            getLocale(
              "No se encontraron recibos para esta organizacion. Puede crear uno en administratoSearch en tipo de comprobantes."
            ),
            function () {}
          );
        } else {
          view
            .down("#recibos")
            .select(view.down("#recibos").getStore().getAt(0));
        }
      },
    });
