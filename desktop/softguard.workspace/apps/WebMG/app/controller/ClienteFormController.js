Ext.define("WebMG.controller.ClienteFormController", {
  ...
  initview: function (view) {
    var controller = this;
    var condicionPago = controller.getT_condiciones_pago_fcSearchModelModel();

    // Flag para evitar que onOrganizacionFacturadoraChange interfiera durante la carga
    view.initializing = true;

    // Ocultar botón de cierre (X) del window — solo se puede cerrar guardando
    var win = view.up("window");
    if (win) {
      win.closable = false;
      var closeTool = win.down("tool[type=close]");
      if (closeTool) {
        closeTool.hide();
      }
    }
  ...
  onOrganizacionFacturadoraChange: function (combo, newvalue, oldvalue) {
    ...
    if (newvalue && newvalue !== 0) {
      ...
      var triggerWrap = document.getElementById(
        categoriasCombo.id + "-triggerWrap",
      );
      if (inputEl) {
          ...
          listeners: {
            load: function (store, records, successfull) {
              ...
              if (records && records.length > 0) {
                infopago = records[0];
                infopago.setProxy(
                  controller.getMG_informacion_pagoModelModel().getProxy(),
                );
  ...
  onSaveClick: function (button, event, options) {
    ...
    mymodel = view.recordCliente;

    // Validar categoría impositiva obligatoria
    var categoriasCombo = view.down("#categoriasimpositivas");
