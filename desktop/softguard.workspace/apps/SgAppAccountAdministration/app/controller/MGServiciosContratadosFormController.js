Ext.define( 'SgAppAccountAdministration.controller.MGServiciosContratadosFormController', {
    extend : 'Ext.app.Controller',
    stores : [  ],
    models : [ 'crm_contratoModel', 'RelationModel' ],
    views : [ 'MGServiciosContratadosFormView' ],

    init : function(config) {
		// genero los eventos

		this.control({
					'mgservicioscontratadosformview' : {
						beforerender : this.initview,
                        productselected: this.onProductChanged,
					},

					'mgservicioscontratadosformview #save' : {
						click : this.onSaveClick
					},

        			'mgservicioscontratadosformview button[action="changeProduct"]' : {
						click : this.onChangeProductClick
					},
            		'mgservicioscontratadosformview #quantityCombo' : {
    ...
    openWindow: function(record, caller,recordOrganizacion){
        ...
        var view = Ext.widget('mgservicioscontratadosformview',{
            record: record,
	...
	onSaveClick : function(button, event, options) {
        ...
        var view = button.up('mgservicioscontratadosformview');
                ...
                mymodel.save({
                    ...
                    callback : function(record, operation, success) {
                        ...
                        if (operation.success){
                            ...
                            /*guardo la relacion con la cuenta */
                            var relationModel = controller.getRelationModelModel();
                            var relationRecord = Ext.create(relationModel,{
                                
                                ObjectTypeId: recordContrato.get('ObjectTypeId'),
                                ObjectTypeName:recordContrato.get('ObjectTypeName'),
                                RelationObjectTypeId:'3001', // cuenta
                                ObjectId: recordContrato.get('Id'),
                                RelationObjectId: view.recordCuenta.get('Id')
                            })
                            
                            relationRecord.save({callback:function(){
    ...
    onQuantityComboChange: function(combo, newValue){
        var view = combo.up('mgservicioscontratadosformview');
        this.calculateTotal(view);
    },
