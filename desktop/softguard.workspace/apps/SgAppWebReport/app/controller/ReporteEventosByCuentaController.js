Ext.define('SgAppWebReport.controller.ReporteEventosByCuentaController', {
    extend: 'Ext.app.Controller',
    stores: ['Common.store.SoftguardEstadoEstadoStore', 'Common.store.TablaTiposStore', 'Common.store.TablasResolucionesStore', 'Common.store.TablasCategorizacionStore', 'Common.store.EventoOrigenStore', 'Common.store.ProvinciasStore'],
    models: ['t_autoridadesSearchModel', 'TablasTiposSearchModel', 'TablasCodigosAlarmaSearchModel', 'ZonaSearchModel', 'UsuarioSearchModel', 'SoftguardCodigoAlarmaModel', 'soperadoresSearchModel', 'TablaHistoricoSearchModel', 'ZonaByCuentaSearchModel', 'TablasCategorizacionSearchModel', 'TablasResolucionesSearchModel', 'TablasGruposSearchModel'],
    views: ['ReporteEventosByCuentaView'],

    init: function (config) {
        // genero los eventos
        this.control({
            'reporteeventosbycuentaview': {
            ...
            'reporteeventosbycuentaview button[action=todos]': {
                click: this.onTodosClick
            },
            'reporteeventosbycuentaview button[action=btnprint]': {
                click: this.onBtnprintClick
            },
            'reporteeventosbycuentaview button[action=mail]': {
                click: this.onMailClick
            },
            'reporteeventosbycuentaview #combohistorico': {
                select: this.onComboHistoricoSelect
            },
            'reporteeventosbycuentaview #fechadesde': {
                select: this.onComboHistoricoSelect
            },
            'reporteeventosbycuentaview #fechahasta': {
                select: this.onComboHistoricoSelect
            },
            'reporteeventosbycuentaview button[action=openmenu]': {
                click: this.onOpenMenuClick
            },
            'reporteeventosbycuentaview #seleccionarcuenta': {
                click: this.onsSeleccionarCuenta
            },
            'reporteeventosbycuentaview button[action=export]': {
                click: this.onExportClick
            },
            'reporteeventosbycuentaview button[action=exportCsv]': {
                click: this.onExportClick
            },
            'reporteeventosbycuentaview button[action=exportSplit]': {
                click: this.onExportClick
            }
    ...
    initView: function (view) {
        ...
        var folderModule = view.module.get('folder');
        console.log('folderModule-----', folderModule);
        if (getParametro('TIPOREPORTE') == 1) {
            console.log("EventosByCuentaNuevoHTML")
            view.baseurl = '/handler/EventosByCuentaNuevoHTML';
        } else {
            console.log("EventosByCuentaHTML")
            view.baseurl = '/handler/EventosByCuentaHTML?search=ReporteHistorico&export=1';
        }
    ...
    onBtnprintClick: function (button) {
        var view = button.up('reporteeventosbycuentaview');
    ...
    onGrupoChange: function (combo, records, options) {
        ...
        var value = records.data.gru_ccodigo
        var t = this;
        ...
        codigosAlarmaStore.load({
            callback: function (records, opciones, success) {
                if (opciones.success) {
                    ...
                    codigosalarma.setValue(records.map(function (record) {
                        return record.get('cod_ccodigo'); // Asegúrate de usar el campo correcto
                    }));
                }
            }
        });
    ...
    onSearchClick: function (button, viewparent) {
        ...
        if (grupo) {
            ...
            url = Ext.String.urlAppend(url, "cod_cgrupo=" + grupo);
        }
        ...
        /* Agregado para filtrar por palabra el rec_iid */
        var leg = view.down('#leg').getValue();
        if (leg) {
            url = Ext.String.urlAppend(url, "leg=" + leg);
        }
        ...
        var legajo = view.down('#legCheck').getValue();
        if (legajo) {
            url = Ext.String.urlAppend(url, "Legajo=" + legajo)
        }
        ...
        if (reportecompleto) {
            ...
            url = url.replace('/handler/EventosByCuentaNuevoHTML', '/handler/EventosByCuentaHTML')
        }
        ...
        if (comboestado && comboestado !== 'null') {
    ...
    onExportClick: function (button) {
        ...
        var EXPORTREPORTEHISTORICOCOLUMNAS = getParametro( 'EXPORTREPORTEHISTORICOCOLUMNAS' );
        console.log('EXPORTREPORTEHISTORICOCOLUMNAS------', EXPORTREPORTEHISTORICOCOLUMNAS)
    ...
    onOpenMenuClick: function (button) {
        ...
        if (!view.win) {
                        ...
                        items: [
                            {
                                ...
                                items: [
                                    ...
                                    },
                                    {
                                        ...
                                        itemId: 'codigoalarma',
                                        displayField: 'Descripcion', // O el campo que quieras mostrar
                                        valueField: 'cod_ccodigo',   // Campo que sirve como valor
                                        queryMode: 'local',
                                        width: '100%',          // Asegúrate de usar 'local' si usas loadData
                                        multiSelect: true,           // Permitir selección múltiple
                                        editable: false,             // Evita errores de escritura
                                        store: {
                                            fields: ['cod_ccodigo', 'Descripcion'], // Campos esperados
                                            data: [] // Inicia vacío
                                        }
                                    },
                                    {
                                ...
                                items: [{
                                    ...
                                    listeners: {
                                        change: function (chk, value) {
                                            if (value) {
                                                /**
                                                 * BC 407862025 : Dejo siempre visible la posibilidad de agrupar por cuenta
                                                 * win.down('#agrupar').hide()
                                                 * */
                                ...
                                items: [
                                    {
                                        ...
                                        /**
                                        * BC 423858954 : Habilito la posibilidad de agrupar por cuenta+usuario juntos.
                                        * */
                                        xtype: 'combo',
                                    ...
                                    /* VIEJO, se migro por BC https://basecamp.com/2249105/projects/12939010/todos/416879616
                                    {
                                    xtype : 'checkbox',
                                    itemId: 'agruparcuentacheck',
                                    fieldLabel : 'por Cuenta',
            ...
            if (getParametro('TIPOREPORTE') == 1) {
                ...
                win.down('#legCheck').setValue(true);
            } else {
