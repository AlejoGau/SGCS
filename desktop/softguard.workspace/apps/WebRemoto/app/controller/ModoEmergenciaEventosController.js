Ext.define( 'WebRemoto.controller.ModoEmergenciaEventosController', {
    extend: 'Ext.app.Controller',

    models: ['TablasGruposSearchModel','SoftguardCodigoAlarmaModel',
                'ConexionesModel','ComboEventosModel','EventosTiempoRealModel'
                ,'TablasCodigosAlarmaSearchModel'
    ],
    stores: ['EventoEstadoStore'],
    views: ['ModoEmergenciaEventosView'],

init: function(config ) {
    this.control( {
        'modoemergenciaeventosview': {
        ...
        'modoemergenciaeventosview button[action=search]': {
            click: this.onBuscarClick
        },
        'modoemergenciaeventosview #comboEstados': {
            //	select : this.onComboSelect
        },
        'modoemergenciaeventosview #comboOrigenes': {
            //	select : thizs.onComboSelect
        },
        'modoemergenciaeventosview #comboTipos': {
            //	select : this.onComboSelect
        },
        'modoemergenciaeventosview #dealer': {
            //	change : this.onComboSelect
        },
        'modoemergenciaeventosview button[action=play]': {
            click: this.onPlayClick
        },
        'modoemergenciaeventosview button[action=stop]': {
            click: this.onStopClick
        },
        'modoemergenciaeventosview button[action=groupAlarmas]': {
            click: this.onGroupAlarmasClick
        },
        'modoemergenciaeventosview button[action=groupPrioridad]': {
            click: this.onGroupPrioridadClick
        },
        'modoemergenciaeventosview button[action=groupCuenta]': {
            click: this.onGroupCuentaClick
        },
        'modoemergenciaeventosview #grupos': {
        ...
        'modoemergenciaeventosview #grupos-excluir': {
            //	select : this.onGrupoExcluirChange,
            //  change : this.onGrupoExcluirChangeClear
        },
        'modoemergenciaeventosview #procesartodosfull': {
            click: this.onProcesarTodosFullClick
        },
        'modoemergenciaeventosview button[action=soloAlarmas]': {
            click: this.onSoloAlarmasClick
        },
        'modoemergenciaeventosview #clearfilters': {
...
initView: function(view ) {
    ...
    var CATMODOEMERGENCIA = getParametro( 'CATMODOEMERGENCIA' );

    // Rellenar con ceros a la izquierda hasta una longitud de 2 caracteres, por ejemplo
    var CATMODOEMERGENCIA_formatted = CATMODOEMERGENCIA.toString().padStart( 3, '0' );

    console.log( CATMODOEMERGENCIA_formatted ); // Ejemplo: 01, 02, etc.

    console.log( 'CATMODOEMERGENCIA-------------', CATMODOEMERGENCIA )
    var mystore = Ext.create( 'Ext.data.Store', {
        ...
        filters: [
            {
                property: 'rec_idResolucion ',
                value: CATMODOEMERGENCIA_formatted
            },
            {
    ...
    mystore.getProxy().setExtraParam( 'mostrarEventoAlarma', 1 );
    ...
    Ext.TaskManager.start( {
        run: function() {
            console.log('entre al taskmanger-------------')
            mystore.load();
        },
        interval: view.interval, // 10 segundos (10000 ms)
        scope: this
    });
...
objectChanged: function (view ) {
    console.log( 'Disparé el objectchanged' );
    var mygrid = Ext.ComponentQuery.query( '#modoemergenciaeventosview' )[ 0 ];
    if( mygrid ) {
...
onClearFiltersClick: function(button, event, options ) {
    var view = button.up( 'modoemergenciaeventosview' );
...
onDealerChange: function(field, newValue, oldValue ) {
    var view = field.up( 'modoemergenciaeventosview' );
...
onGrupoChange: function(combo, records, options ) {
    var view = combo.up( 'modoemergenciaeventosview' );
...
onItemClick: function(grid, record, item, index, e, options ) {
    ...
    var view = grid.up( 'modoemergenciaeventosview' );
