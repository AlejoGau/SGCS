Ext.define('iOT.controller.CuentaHelperController', {
    extend: 'Ext.app.Controller',
    stores: ['Common.store.ProvinciasStore'],
    models: ['CuentaSearchModel', 'Common.model.ProvinciasModel'],
    views: ['iOT.view.CuentaHelperView'],
    ...
    init: function (config) {
        ...
        this.control({
            ...
            'cuentahelperview button[action=filterNohabilitadas]': {
                click: this.onNohabilitadasClick
            },
            'cuentahelperview button[action=filterHabilitadas]': {
                click: this.onHabilitadasClick
            },
            'cuentahelperview button[action=filterEnprueba]': {
                click: this.onEnpruebaClick
            },
            'cuentahelperview button[action=removefilter]': {
                click: this.onRemovefilterClick
            },
            'cuentahelperview button[action=filterText]': {
                click: this.onFiltertextClick
            },
            'cuentahelperview button[action=selected]': {
                click: this.onSelectedClick
            },
            'cuentahelperview button[action=selectedall]': {
                click: this.onSelectedAllClick
            },
            'cuentahelperview button[action=particiones]': {
                click: this.onParticionesClick
            },
            'cuentahelperview #filiacion': {
                click: this.onFiliacionClick
            },
            'cuentahelperview button[action=filterFalloTest]': {
    ...
    initView: function (view) {
        ...
        if (view.filterTipo == 'nofilter') {
            ...
            Ext.Array.push(filter, { id: 'tip_nTipo', property: '_tip_nTipo', value: view.filterTipo })
    ...
    onParticionesClick: function (button, event, options) {
        var view = button.up('cuentahelperview');
    ...
    onFiliacionClick: function (btn) {
        ...
        var view = btn.up('cuentahelperview');
    ...
    onItemClick: function (view, record, item, index, e, options) {
        var helper = view.up('cuentahelperview');
        if (!view.up('panel').multiSelect) {
            ...
            if (helper.metodo == 'new') {
                ...
                caller = caller.down('iotenergymonitorsinasignargridview')
                caller.fireEvent(evento, record, caller, view.medidor);
            }
