CREATE OR ALTER TRIGGER [dbo].[TG_UPD_CecalRMQ] ON [dbo].[RemoteCallQueue] AFTER UPDATE AS
BEGIN
	SET NOCOUNT ON;
	...
	Select @cUrl=[rcq_url],@cRespuesta=[rcq_result],@iStatus=[rcq_estado] From inserted
		WHERE Upper([rcq_result]) Like  Upper('%RedirectorCecal%')
	...
	Begin
		...
		Begin
			...
			Set @idRec = CAST(
				SUBSTRING(
					@cUrl,
					CHARINDEX('rec_iid=', @cUrl) + 8,
					CASE 
						WHEN CHARINDEX('&', @cUrl, CHARINDEX('rec_iid=', @cUrl)) > 0 
						THEN CHARINDEX('&', @cUrl, CHARINDEX('rec_iid=', @cUrl)) - (CHARINDEX('rec_iid=', @cUrl) + 8)
						ELSE LEN(@cUrl)
					END
				) AS Int
			)
