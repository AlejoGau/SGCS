CREATE OR ALTER PROCEDURE crm_contratoByNameWithParent
              		@Name Varchar(128) = '',
              	...
              	AS
							...
							select @ObjectTypeId = dbo.GetObjectId('crm_contrato')
							...
							set @RowTotal = (SELECT COUNT(t.[cnt_iid]) FROM [_datos.dbo.crm_contrato] t 
															inner join RelationObject r 
              			           on r.ObjectTypeId = @RelationObjectTypeId 
              				       and r.ObjectId = @ObjectId
              				       and r.RelationObjectTypeId = @ObjectTypeId 
              				       and r.RelationObjectId = t.[cnt_iid] 
								   LEFT JOIN #Taxo taxo ON taxo.Id = t.[cnt_iid]
							...
							SELECT Id, Name , [cnt_org_fc], [cnt_idcliente], [cnt_fechaalta], [cnt_fechavto], [cnt_formapago], [cnt_metadata], [cnt_estado], [cnt_tmp_id], [cnt_dinamico]
							FROM (
							SELECT t.[cnt_iid] Id, '' Name , t.[cnt_org_fc], t.[cnt_idcliente], t.[cnt_fechaalta], t.[cnt_fechavto], t.[cnt_formapago], t.[cnt_metadata], t.[cnt_estado], t.[cnt_tmp_id], t.[cnt_dinamico] , taxo.Id as Taxo,
							ROW_NUMBER() OVER(
							ORDER BY t.[cnt_iid] ) AS RowNumber
							FROM [_datos.dbo.crm_contrato] t 
								inner join RelationObject r 
              			           on r.ObjectTypeId = @RelationObjectTypeId 
              				       and r.ObjectId = @ObjectId
              				       and r.RelationObjectTypeId = @ObjectTypeId 
