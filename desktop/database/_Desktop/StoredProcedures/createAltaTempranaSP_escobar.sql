...
-- =============================================================================================================
CREATE OR ALTER PROCEDURE [dbo].[createAltaTempranaSP_escobar] 
	 @dealer char(3) = 'ESC'
AS
BEGIN
	SET NOCOUNT ON;
	...
	IF ( @dealer != '')
		BEGIN
			...
			DECLARE @mailTo varchar(max) = ''; -- Reemplazar por el mail de la central
			DECLARE @automonitoreo varchar(2) = 'no';
			DECLARE @diasprueba int = 0;
			DECLARE @cuentatipo varchar(128) = '009';
			DECLARE @enviarEmail varchar(2) = 'no'; -- Reemplazar para enviar o no el mail de alta (no/si)
			...
			DECLARE @telefono float;
			DECLARE @comentario varchar(300);

			DECLARE @usuarioTipo VARCHAR(50);
			...
			DECLARE altaTemprana_cursor CURSOR FOR 
				SELECT top 100
				  ...
				  ,[Telefono]
				  ,[Tipo]
				from _Escobar..importacion with (NOLOCK)
				where [Telefono] not in (select Telefono FROM _Datos..SmartPanic WITH (NOLOCK))
			...
			BEGIN
				 ...
				 */
				SET @parcelaNumero = ''
				DECLARE @telefonoVARCHAR VARCHAR(MAX) = CAST(CAST(@telefono AS bigint) AS varchar)
                SET @email = @telefonoVARCHAR+'@escobar.com.ar'
                SET @nombreDispositivo = @nombre+@apellido

				/* ASIGNO DEALER Y TIPO CUENTA SEGUN CORRESPONDA 
					Los usuarios del tipo   VIOLENCIA GENERO se deben crear con el dealer ESV y tipo de cuenta 010

					El resto de los usuarios GENERAL - TERCERA EDAD -  ESCOLAR -  COMERCIO con el dealer ESC.
					Tercera edad  con tipo de cuenta 011
					General con tipo de cuenta 012
					Escolar con tipo de cuenta 013
					Comercial con tipo de cuenta 009
				
				*/
				IF ( @usuarioTipo = 'VIOLENCIA GENERO' )
					BEGIN
						SET @dealer = 'ESV'
						SET @cuentatipo = '010'
					END
				ELSE IF ( @usuarioTipo = 'GENERAL' )
					BEGIN
						SET @dealer = 'ESC'
						SET @cuentatipo = '011'
					END
				ELSE IF ( @usuarioTipo = 'TERCERA EDAD' )
					BEGIN
						SET @dealer = 'ESC'
						SET @cuentatipo = '012'
					END
				ELSE IF ( @usuarioTipo = 'ESCOLAR' )
					BEGIN
						SET @dealer = 'ESC'
						SET @cuentatipo = '013'
					END
				ELSE IF ( @usuarioTipo = 'COMERCIO' )
					BEGIN
						SET @dealer = 'ESC'
						SET @cuentatipo = '009'
					END
				...
				SELECT @telefonoSPExiste = Telefono FROM _Datos..SmartPanic with (NOLOCK) WHERE Telefono = @telefonoVARCHAR
					...
					BEGIN
						...
						IF ( @cue_iidExiste IS NULL OR @cue_iidExiste = '')
							BEGIN
								...
								SET @nombre = UPPER(@nombre+@apellido)
