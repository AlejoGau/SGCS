CREATE OR ALTER PROCEDURE crm_contrato_itemByText
								@Text Varchar(128) = '',
...
------------
							...
							select @ObjectTypeId = dbo.GetObjectId('crm_contrato_item')

							CREATE TABLE #Taxo (Id int)


							set @RowTotal = (SELECT COUNT([Id]) FROM [_datos..crm_contrato_item])
							if(@PageCount = 0)
							begin
							...
							set @RowTotal = (SELECT COUNT(t.[Id]) FROM [_datos..crm_contrato_item] t 
							...
							SELECT Id, Name , [idcontrato], [Price], [Currency], [Status], [Description], [Quantity], [QuantityDelivered], [Code], [VAT], [ProductId], [idlista]
							FROM (
							SELECT t.[Id] Id, '' Name , t.[idcontrato], t.[Price], t.[Currency], t.[Status], t.[Description], t.[Quantity], t.[QuantityDelivered], t.[Code], t.[VAT], t.[ProductId], t.[idlista] , taxo.Id as Taxo,
							ROW_NUMBER() OVER(
							ORDER BY t.[Id]  
								) AS RowNumber
							FROM [_datos..crm_contrato_item] t 
							LEFT JOIN #Taxo taxo ON taxo.Id = t.[Id] 
							WHERE  (len(@Text) = 2 OR Contains(t.*,  @Text ))
