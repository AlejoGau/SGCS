CREATE OR ALTER PROCEDURE [dbo].[ReporteHistoricoOld]            
...
IF @Cuentas != '' AND @Cuentas != '0' And @filter Not LIke '%rec_iid%'	--2022-12-12 Pablo.El llamado desde SP envia rec_iid en filter en lugar de enviarlo como parametro
Begin
...
set @rec_fields ='  
	r.rec_iid Id,
        ...
        r.rec_idMap,
			...
			*/
		usuario_cnombre As usr_cnombre,
        r.rec_idReceptor,
			...
			*/
			...
			ELSE
				BEGIN
					...
					--Pablo 2022/10/18 se agrego TOP a cada Select por problemas de performance NO SACAR
					select @sqlparcial2 = 'select '+@InTop +' '+@rec_fields+' 1 as Dummi from [_Datos].[dbo].['+@item+'] r' +@joinunion+ @whereunion
					--print '------------ SUMO SELECT AL UNION ------------'
