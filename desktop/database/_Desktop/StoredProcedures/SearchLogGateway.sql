CREATE OR ALTER PROCEDURE [dbo].[SearchLogGateway]
 @page INT = 1,               
 @start INT = 0,               
 @limit INT = 50,               
 @sort VARCHAR(256) = '',   
...
AS  
 ...
 SELECT @SqlSort = dbo.GetSqlSortForJson(@sort, 'n.[Logged] DESC')
 
 --Filters
 DECLARE @SqlFilter AS VARCHAR(4096)
 SELECT @SqlFilter = dbo.GetSqlFilterForJson(@filter, 'NLog')
 ...
 SET @Sql = 'FROM [_LogDB].[dbo].[NLog] n
			WHERE 1 = 1 ' + @SqlFilter
 ...
 SET @DynamicSqlReturnRows = 'SELECT * 
							   ...
							   , n.*
							   , REPLACE(n.Message,''%'','''') as _Message ' + @Sql + ' ) AS T
