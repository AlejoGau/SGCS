CREATE OR ALTER PROC [dbo].[SGSP_RestoreGenerator]
(
...
BEGIN
    ...
    (
        SELECT
            ...
            ,CASE ISNULL(@TargetDatabase_,'') WHEN '' THEN mf.Physical_Name ELSE Replace(SUBSTRING(mf.Physical_Name,1,CASE CHARINDEX('.',mf.Physical_Name) WHEN 0 THEN LEN(mf.Physical_Name) ELSE CHARINDEX('.',mf.Physical_Name) - 1 END) + '.mdf',@Database_,@TargetDatabase_) END  AS PhysicalName
    ...
    (
        SELECT
			...
			,CASE ISNULL(@TargetDatabase_,'') WHEN '' THEN mf.Physical_Name ELSE Replace(SUBSTRING(mf.Physical_Name,1,CASE CHARINDEX('.',mf.Physical_Name) WHEN 0 THEN LEN(mf.Physical_Name) ELSE CHARINDEX('.',mf.Physical_Name) - 1 END) + '.ldf',@Database_,@TargetDatabase_) END  AS PhysicalName
            ,mf.Name AS LogicalName
        FROM sys.master_files mf
    ...
    SELECT
        ...
        --CASE @StandbyMode_ WHEN 0 THEN 'NORECOVERY,' ELSE 'STANDBY =N' + '''' + ISNULL(@FromFileFullUNC_,SUBSTRING(#CTE.physical_device_name,1,LEN(#CTE.physical_device_name) - CHARINDEX('\',REVERSE(#CTE.physical_device_name)))) + '\' + d.name + '_' + REPLACE(REPLACE(SUBSTRING(CONVERT(VARCHAR(24),GETDATE(),127),12,12),':',''),'.','') + '_ROLLBACK_UNDO.bak ' + ''''  + ',' END + SPACE(1) + 
		CASE @WithRecovery_ WHEN 0 THEN 'NORECOVERY,' ELSE 'RECOVERY,' END + SPACE(1) + 
    ...
    */
    ...
    SELECT
        ...
        CASE b.type_desc 
            ...
            WHEN 'ROWS' THEN
                ...
                ELSE 
				CASE ISNULL(@TargetDatabase_,'') WHEN '' THEN  @WithMoveDataFiles_ + SUBSTRING(b.Physical_Name,LEN(b.Physical_Name) - CHARINDEX('\',REVERSE(b.Physical_Name),1) + 2,CHARINDEX('\',REVERSE(b.Physical_Name),1) + 1)
    ...
    BEGIN
        ...
        BEGIN
			...
			SELECT x4.TSQL, x4.BackupDate, x4.BackupDevice, x4.database_name, x4.SortSequence
