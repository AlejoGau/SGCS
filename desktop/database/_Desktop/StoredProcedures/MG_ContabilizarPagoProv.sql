...
-- Create date: 21/6/2019
-- Description:	Agrega los montos del pago a la tabla mg_cuentacontable PROVEEDORES
-- =============================================
CREATE OR ALTER PROCEDURE [dbo].[MG_ContabilizarPagoProv]
	@pag_iCodigo_ID int
AS
BEGIN

	SET NOCOUNT ON;
	print '[MG_ContabilizarPagoProv]'
	...
	print '[MG_ContabilizarPagoProv] busco los datos del pago'
	...
	IF @mgmc_idkey > 0
	BEGIN
		print '[MG_ContabilizarPagoProv] Pago ya procesado'
		set NOEXEC ON
	END
		print '[MG_ContabilizarPagoProv] recibo, nota credito, ajuste credito, resto en la CC'
		...
		print '[MG_ContabilizarPagoProv] Busco o creo en el plan de cuentas'
		print '[MG_ContabilizarPagoProv] @fpg_idkey'
		print @fpg_idkey
		
		EXECUTE [dbo].[MG_GetCreateCuentaIdFormaPago] @fpg_idkey ,@fpg_mgmcidkey OUTPUT

		print '[MG_ContabilizarPagoProv] @fpg_mgmcidkey'
		print @fpg_mgmcidkey

		if @fpg_mgmcidkey > 0
		BEGIN
			-- es un pago actualizo la cuenta CASH
		...
		print '[MG_ContabilizarPagoProv] Actualizo el pago'
