CREATE OR ALTER PROCEDURE [dbo].[MG_ContratoAFactura]
	@IdContrato int,
	@template int = 0,
	@userid int
AS
BEGIN
	...
	SELECT @idcliente = cnt_idcliente 
		...
		,@totalItemsSinImpuestos = sum(it.Price*it.Quantity)
		,@Total = sum(
			
			CASE con.cnt_dinamico WHEN 1 THEN mglpd.mglpd_valor ELSE it.Price END *it.Quantity*(1+isnull(imp_nporcentaje,0)/100)
		)
	...
	-- verificaciones a realizar antes de crear factura
	-- contrato habilitado?
	-- contrato tiene productos
	-- tipo de comprobante seleccionado


	-- calculo que impuesto usar y lo completo en el comprobante
	...
	-- calculo totales del comprobante
	...
	DECLARE items_contrato_cursor CURSOR LOCAL FOR
		SELECT 
			ProductId as cbi_iproducto,
			it.Description as [cbi_cdescripcion],
			it.Quantity as cbi_icantidad,
			(CASE con.cnt_dinamico WHEN 1 THEN mglpd.mglpd_valor ELSE it.Price END *it.Quantity) as cbi_yimporte,
	...
	-- veo que la factura tenga items
	if @items = 0
	BEGIN
