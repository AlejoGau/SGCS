CREATE OR ALTER PROCEDURE [dbo].[PoiByText]
								@Text Varchar(128) = '',
							...
							set @RowTotal = (SELECT COUNT([Id]) FROM _Datos.dbo.[p_Poi])
							if(@PageCount = 0)
							begin
							...
							set @RowTotal = (SELECT COUNT(t.[Id]) FROM _Datos.dbo.[p_Poi] t 
							...
							SELECT Id, Name , [FullAddress], [Icon], [Country], [State], [City], [Address], [Number], [Latitude], [Longitude], [CDealer]
							FROM (
							SELECT t.[Id] Id,  Name , t.[FullAddress], t.[Icon], t.[Country], t.[State], t.[City], t.[Address], t.[Number], t.[Latitude], t.[Longitude], [CDealer] , taxo.Id as Taxo,
							ROW_NUMBER() OVER(
							ORDER BY t.[Id]  
								) AS RowNumber
							FROM _Datos.dbo.[p_Poi] t 
							LEFT JOIN #Taxo taxo ON taxo.Id = t.[Id] 
