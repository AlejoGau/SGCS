...
--#############################################################################
CREATE OR ALTER PROCEDURE [dbo].[GeoFenseByDealerNocuenta]                
 @page INT = 1,               
 @start INT = 0,               
 @limit INT = 50,               
 @sort NVARCHAR(64) = '',            
...
set @SqlSort = @SortField +' '+@SortDirection
 ...
 SET @Sql = 'select g.[Id]
      ...
      ,g.[MetaData]
      ,g.[Style]
      ,g.[MaxSpeed], l.lin_crazonsocial, isnull(gr.grg_idKey,0) as GeoGroup, isnull(gr.grg_cdescripcion,'''') as GeoGroupName, ROW_NUMBER() OVER (ORDER BY ' + @SqlSort + ') AS RowNumber        
...
IF @filter != ''          
 BEGIN        
	...
	BEGIN		
		...
		--Set Filters		
		IF @FilterProperty = 'MetaData:LIKE'
