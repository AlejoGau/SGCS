CREATE OR ALTER PROCEDURE T_SimCard_MarcaByText
								@Text Varchar(128) = '',
...
------------
							...
							select @ObjectTypeId = dbo.GetObjectId('T_SimCard_Marca')

							CREATE TABLE #Taxo (Id int)


							set @RowTotal = (SELECT COUNT([tsm_idKey]) FROM [_Datos.dbo.T_SimCard_Marca])
							if(@PageCount = 0)
							begin
							...
							set @RowTotal = (SELECT COUNT(t.[tsm_idKey]) FROM [_Datos.dbo.T_SimCard_Marca] t 
							LEFT JOIN #Taxo taxo ON taxo.Id = t.[tsm_idKey] 
							...
							SELECT Id, Name , [tsm_cDescripcion]
							FROM (
							SELECT t.[tsm_idKey] Id, '' Name , t.[tsm_cDescripcion] , taxo.Id as Taxo,
							ROW_NUMBER() OVER(
							ORDER BY t.[tsm_idKey]  
								) AS RowNumber
							FROM [_Datos.dbo.T_SimCard_Marca] t 
							LEFT JOIN #Taxo taxo ON taxo.Id = t.[tsm_idKey] 
							WHERE  (len(@Text) = 2 OR Contains(t.*,  @Text ))
