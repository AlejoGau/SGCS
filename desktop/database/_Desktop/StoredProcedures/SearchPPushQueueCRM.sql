...
--#############################################################################
CREATE OR ALTER PROCEDURE [dbo].[SearchPPushQueueCRM]
	 @page INT = 1,               
	 @start INT = 0,               
	 @limit INT = 50,               
	 @sort NVARCHAR(256) = '',   
...
AS  
 ...
 SELECT @SqlSort = dbo.GetSqlSortForJson(@sort, 'o.[ppq_fechacreacion] DESC')
 ...
 SET @Sql = ' 	FROM _Datos.dbo.p_push_queue o
                ...
                WHERE (ms.EventoID IS NULL OR ms.EventoID=0)
                    AND ppq_idmessage IS NOT NULL and ms.Name !=''''
                ' + @SqlFilter
 ...
 SET @DynamicSqlReturnRows = ' SELECT *
							   ...
							   FROM ( SELECT ROW_NUMBER() OVER (ORDER BY ' + @SqlSort + ') AS RowNumber
									,o.*
                                    ,ms.ToId as idOrigen
                                    ,sp.Linea as lineaOrigen
                                    ,sp.Nombre as nombreOrigen
                                    ,ms.Name as msgTitle
                                    ,ms.Body as msgBody
                                    ,mc.cue_clinea as clineaDestino
                                    ,mc.cue_ncuenta as ncuentaDestino
                                    ,mc.cue_cnombre as cnombreDestino
                                    ' + @Sql + ' ) AS T
