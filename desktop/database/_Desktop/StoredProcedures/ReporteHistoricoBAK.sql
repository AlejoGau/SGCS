--#############################################################################
-- SOFTGUARD DESKTOP
-- updated : 2017-06-15 08:51:38.120 
--#############################################################################

--#############################################################################
-- SOFTGUARD DESKTOP
-- updated : 2017-06-15 08:45:34.183 
--#############################################################################

CREATE OR ALTER PROCEDURE [dbo].[ReporteHistoricoBAK]            
	 ...
	 BEGIN
			...
			IF @export = 0
				BEGIN
					...
					select @sqlparcial = 'select '+@Top+@unionfields+', '''+@item+''' as tablaDatos '+' from [_Datos].[dbo].['+@item+'] r'+ @join + @where + @specialWhere + ' ORDER BY ' + @SqlSort
...
--		,ROW_NUMBER() OVER (ORDER BY ' + @sortreversed + ') AS RowNumberReversed
...
-- paginacion
set @sql = 'with CTE as (SELECT * ' +@sql+')
	select  ' + @Top + ' * from CTE
