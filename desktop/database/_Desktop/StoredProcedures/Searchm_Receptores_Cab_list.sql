...
--#############################################################################
CREATE OR ALTER PROCEDURE [dbo].[Searchm_Receptores_Cab_list]
 @page INT = 1,               
 @start INT = 0,               
 @limit INT = 50,               
 @sort NVARCHAR(256) = '',   
...
left join [_Datos].dbo.[m_receptores_item] i on (o.rec_iid = i.rec_iid)
			WHERE 1 = 1 AND o.rec_iid NOT IN (SELECT rec_iid FROM [_Datos].dbo.[m_receptores_item] WHERE rec_cformato = '''') ' + @SqlFilter + ')'
...
left join m_receptores_item i on (o.rec_iid = i.rec_iid)
