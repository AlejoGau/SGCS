CREATE OR ALTER PROCEDURE [dbo].[SearchAnalisisIPR30Dias]
	 @page INT = 1,               
	 @start INT = 0,               
	 @limit INT = 50,               
	 @sort NVARCHAR(256) = '',   
...
SELECT CAST(max(rec_iPuerto) AS INT) as puerto,Count(rec_iid) As cantidad, max(ipc_cdescripcion ) as descripcion
