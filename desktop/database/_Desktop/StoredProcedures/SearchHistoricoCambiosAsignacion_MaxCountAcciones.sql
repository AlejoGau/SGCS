...
CREATE OR ALTER PROCEDURE [dbo].[SearchHistoricoCambiosAsignacion_MaxCountAcciones]
...
BEGIN
    ...
    SELECT @SqlSort = dbo.GetSqlSortForJson(@sort, 'ah.amh_cue_iid ASC, ah.amh_rec_iid DESC, ah.amh_fechahora DESC')
    print '--- @SqlSort';
    print @SqlSort
    print '---';
	...
	SET @Sql = '
		SELECT MAX(cant_acciones) as max_cant_acciones from
        (SELECT c.cue_clinea, c.cue_cnombre, c.cue_ncuenta,amh_rec_iid,count(*) as cant_acciones
        ...
        WHERE 1=1' +@Where+ '
        group by c.cue_clinea, c.cue_cnombre, c.cue_ncuenta,amh_rec_iid) m
