--#############################################################################
-- SOFTGUARD DESKTOP
-- updated : 2017-06-15 08:51:38.103 
--#############################################################################
CREATE OR ALTER PROCEDURE [dbo].[HorarioAlternativoPlantillaByFilter_BAK]
 @page INT = 1,               
 @start INT = 0,               
 @limit INT = 50,               
 @sort NVARCHAR(256) = '',   
...
AS  
 ...
 SELECT @SqlSort = dbo.GetSqlSortForJson(@sort, 'o.[Alt_iid] DESC')
 
 --Filters
 DECLARE @SqlFilter AS NVARCHAR(MAX)
 SELECT @SqlFilter = dbo.GetSqlFilterForJson(@filter, 'm_horarios_alternativos_planilla')
...
BEGIN
	...
	select @fields = '[alt_idKey] Id
	 ...
	 SET @Sql = ' 
		...
		FROM [_Datos].[dbo].[m_horarios_alternativos_planilla] o
...
BEGIN
	 ...
	 SET @Sql = '  FROM _datos..m_horarios_alternativos_planilla o
			WHERE 1 = 1 ' + @SqlFilter
END
