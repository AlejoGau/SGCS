...
// Type: SoftGuard.BusinessObjects.Rest.t_tiposRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  public class t_tiposRestService
  {
    private string _CurrentObjectName;

    public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (Simplet_tipos), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Page, string Start, string Limit, string Sort, string Group, string Filter)
    {
      ...
      t_tipos currentObject = this.GetCurrentObject(0);
      ...
      {
        Simplet_tipos simpletTipos = new Simplet_tipos();
        foreach (PropertyInfo property in simpletTipos.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByFilter.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) simpletTipos, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) simpletTipos);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = TotalRows };
    }

    private t_tipos GetCurrentObject(int Id = 0)
    {
      t_tipos tTipos = ObjectFactoryService.Create<t_tipos>();
      if (Id != 0)
        tTipos.Load(Id);
      return tTipos;
    }
    ...
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "t_tipos");
    }
    ...
    public SimpleBaseObject Create(Simplet_tipos instance)
    {
      instance.Id = 0;
      t_tipos currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, Simplet_tipos instance)
    {
      t_tipos currentObject = this.GetCurrentObject(int.Parse(id));
