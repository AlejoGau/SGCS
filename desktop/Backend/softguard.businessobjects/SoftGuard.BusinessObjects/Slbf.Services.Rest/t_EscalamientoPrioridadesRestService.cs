...
// Type: SoftGuard.BusinessObjects.Rest.t_EscalamientoPrioridadesRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  [DataContractFormat]
  public class t_EscalamientoPrioridadesRestService
  {
    private string _CurrentObjectName;

    public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (Simplet_EscalamientoPrioridades), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Page, string Start, string Limit, string Sort, string Group, string Filter)
    {
      ...
      t_EscalamientoPrioridades currentObject = this.GetCurrentObject(0);
      ...
      foreach (DataRow row in (InternalDataCollectionBase) dataByFilter.Rows)
      {
        ...
        foreach (PropertyInfo property in escalamientoPrioridades.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByFilter.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) escalamientoPrioridades, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) escalamientoPrioridades);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = TotalRows };
    }

    private t_EscalamientoPrioridades GetCurrentObject(int Id = 0)
    {
      t_EscalamientoPrioridades escalamientoPrioridades = ObjectFactoryService.Create<t_EscalamientoPrioridades>();
      if (Id != 0)
        escalamientoPrioridades.Load(Id);
      return escalamientoPrioridades;
    }

    [WebGet(UriTemplate = "/GetCurrentObjectName")]
    public string GetCurrentObjectName()
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "t_EscalamientoPrioridades");
    }
    ...
    public SimpleBaseObject Create(Simplet_EscalamientoPrioridades instance)
    {
      instance.Id = 0;
      t_EscalamientoPrioridades currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, Simplet_EscalamientoPrioridades instance)
    {
      t_EscalamientoPrioridades currentObject = this.GetCurrentObject(int.Parse(id));
