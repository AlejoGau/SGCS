...
// Type: SoftGuard.BusinessObjects.Rest.t_serviciospatrullaRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  public class t_serviciospatrullaRestService
  {
    private string _CurrentObjectName;

    public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (Simplet_serviciospatrulla), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Page, string Start, string Limit, string Sort, string Group, string Filter)
    {
      ...
      t_serviciospatrulla currentObject = this.GetCurrentObject(0);
      ...
      foreach (DataRow row in (InternalDataCollectionBase) dataByFilter.Rows)
      {
        ...
        foreach (PropertyInfo property in serviciospatrulla.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByFilter.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) serviciospatrulla, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) serviciospatrulla);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = TotalRows };
    }

    private t_serviciospatrulla GetCurrentObject(int Id = 0)
    {
      t_serviciospatrulla serviciospatrulla = ObjectFactoryService.Create<t_serviciospatrulla>();
      if (Id != 0)
        serviciospatrulla.Load(Id);
      return serviciospatrulla;
    }

    [WebGet(UriTemplate = "/GetCurrentObjectName")]
    public string GetCurrentObjectName()
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "t_serviciospatrulla");
    }
    ...
    public SimpleBaseObject Create(Simplet_serviciospatrulla instance)
    {
      instance.Id = 0;
      t_serviciospatrulla currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, Simplet_serviciospatrulla instance)
    {
      t_serviciospatrulla currentObject = this.GetCurrentObject(int.Parse(id));
