...
// Type: SoftGuard.BusinessObjects.Rest.m_telefonos_jurisdiccionalesRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  [DataContractFormat]
  public class m_telefonos_jurisdiccionalesRestService
  {
    private string _CurrentObjectName;

    public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (Simplem_telefonos_jurisdiccionales), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Page, string Start, string Limit, string Sort, string Group, string Filter)
    {
      ...
      m_telefonos_jurisdiccionales currentObject = this.GetCurrentObject(0);
      ...
      foreach (DataRow row in (InternalDataCollectionBase) dataByFilter.Rows)
      {
        ...
        foreach (PropertyInfo property in jurisdiccionales.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByFilter.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) jurisdiccionales, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) jurisdiccionales);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = TotalRows };
    }

    private m_telefonos_jurisdiccionales GetCurrentObject(int Id = 0)
    {
      m_telefonos_jurisdiccionales jurisdiccionales = ObjectFactoryService.Create<m_telefonos_jurisdiccionales>();
      if (Id != 0)
        jurisdiccionales.Load(Id);
    ...
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "m_telefonos_jurisdiccionales");
    }
    ...
    public SimpleBaseObject Create(Simplem_telefonos_jurisdiccionales instance)
    {
      instance.Id = 0;
      m_telefonos_jurisdiccionales currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, Simplem_telefonos_jurisdiccionales instance)
    {
      m_telefonos_jurisdiccionales currentObject = this.GetCurrentObject(int.Parse(id));
