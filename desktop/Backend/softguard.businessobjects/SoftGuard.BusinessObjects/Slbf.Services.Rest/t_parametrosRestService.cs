...
// Type: SoftGuard.BusinessObjects.Rest.t_parametrosRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  public class t_parametrosRestService
  {
    private string _CurrentObjectName;

    public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (Simplet_parametros), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Page, string Start, string Limit, string Sort, string Group, string Filter)
    {
      ...
      t_parametros currentObject = this.GetCurrentObject(0);
      ...
      foreach (DataRow row in (InternalDataCollectionBase) dataByFilter.Rows)
      {
        ...
        foreach (PropertyInfo property in simpletParametros.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByFilter.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) simpletParametros, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) simpletParametros);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = TotalRows };
    }

    private t_parametros GetCurrentObject(int Id = 0)
    {
      t_parametros tParametros = ObjectFactoryService.Create<t_parametros>();
      if (Id != 0)
        tParametros.Load(Id);
      return tParametros;
    }
    ...
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "t_parametros");
    }
    ...
    public SimpleBaseObject Create(Simplet_parametros instance)
    {
      instance.Id = 0;
      t_parametros currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, Simplet_parametros instance)
    {
      t_parametros currentObject = this.GetCurrentObject(int.Parse(id));
