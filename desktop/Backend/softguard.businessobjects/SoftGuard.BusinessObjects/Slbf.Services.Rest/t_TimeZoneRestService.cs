...
// Type: SoftGuard.BusinessObjects.Rest.t_TimeZoneRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  [DataContractFormat]
  [ServiceContract]
  public class t_TimeZoneRestService
  {
    private string _CurrentObjectName;

    public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (Simplet_TimeZone), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Page, string Start, string Limit, string Sort, string Group, string Filter)
    {
      ...
      t_TimeZone currentObject = this.GetCurrentObject(0);
      ...
      foreach (DataRow row in (InternalDataCollectionBase) dataByFilter.Rows)
      {
        ...
        foreach (PropertyInfo property in simpletTimeZone.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByFilter.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) simpletTimeZone, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) simpletTimeZone);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = TotalRows };
    }

    private t_TimeZone GetCurrentObject(int Id = 0)
    {
      t_TimeZone tTimeZone = ObjectFactoryService.Create<t_TimeZone>();
      if (Id != 0)
        tTimeZone.Load(Id);
      return tTimeZone;
    }
    ...
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "t_TimeZone");
    }
    ...
    public SimpleBaseObject Create(Simplet_TimeZone instance)
    {
      instance.Id = 0;
      t_TimeZone currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, Simplet_TimeZone instance)
    {
      t_TimeZone currentObject = this.GetCurrentObject(int.Parse(id));
