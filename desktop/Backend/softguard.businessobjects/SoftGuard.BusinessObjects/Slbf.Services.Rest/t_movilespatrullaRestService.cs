...
// Type: SoftGuard.BusinessObjects.Rest.t_movilespatrullaRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  public class t_movilespatrullaRestService
  {
    private string _CurrentObjectName;

    public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (Simplet_movilespatrulla), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Page, string Start, string Limit, string Sort, string Group, string Filter)
    {
      ...
      t_movilespatrulla currentObject = this.GetCurrentObject(0);
      ...
      foreach (DataRow row in (InternalDataCollectionBase) dataByFilter.Rows)
      {
        ...
        foreach (PropertyInfo property in simpletMovilespatrulla.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByFilter.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) simpletMovilespatrulla, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) simpletMovilespatrulla);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = TotalRows };
    }

    private t_movilespatrulla GetCurrentObject(int Id = 0)
    {
      t_movilespatrulla tMovilespatrulla = ObjectFactoryService.Create<t_movilespatrulla>();
      if (Id != 0)
        tMovilespatrulla.Load(Id);
      return tMovilespatrulla;
    }

    [WebGet(UriTemplate = "/GetCurrentObjectName")]
    public string GetCurrentObjectName()
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "t_movilespatrulla");
    }
    ...
    public SimpleBaseObject Create(Simplet_movilespatrulla instance)
    {
      instance.Id = 0;
      t_movilespatrulla currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, Simplet_movilespatrulla instance)
    {
      t_movilespatrulla currentObject = this.GetCurrentObject(int.Parse(id));
