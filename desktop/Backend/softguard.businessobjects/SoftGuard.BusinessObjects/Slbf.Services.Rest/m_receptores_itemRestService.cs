...
// Type: SoftGuard.BusinessObjects.Rest.m_receptores_itemRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  public class m_receptores_itemRestService
  {
    private string _CurrentObjectName;

    public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (Simplem_receptores_item), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Page, string Start, string Limit, string Sort, string Group, string Filter)
    {
      ...
      m_receptores_item currentObject = this.GetCurrentObject(0);
      ...
      {
        Simplem_receptores_item simplemReceptoresItem = new Simplem_receptores_item();
        foreach (PropertyInfo property in simplemReceptoresItem.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByFilter.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) simplemReceptoresItem, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) simplemReceptoresItem);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = TotalRows };
    }

    private m_receptores_item GetCurrentObject(int Id = 0)
    {
      m_receptores_item mReceptoresItem = ObjectFactoryService.Create<m_receptores_item>();
      if (Id != 0)
        mReceptoresItem.Load(Id);
      return mReceptoresItem;
    }

    [WebGet(UriTemplate = "/GetCurrentObjectName")]
    public string GetCurrentObjectName()
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "m_receptores_item");
    }
    ...
    public SimpleBaseObject Create(Simplem_receptores_item instance)
    {
      instance.Id = 0;
      m_receptores_item currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, Simplem_receptores_item instance)
    {
      m_receptores_item currentObject = this.GetCurrentObject(int.Parse(id));
