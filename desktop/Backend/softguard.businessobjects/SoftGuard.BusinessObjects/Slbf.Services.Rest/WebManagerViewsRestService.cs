...
// Type: SoftGuard.BusinessObjects.Rest.WebManagerViewsRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  [DataContractFormat]
  public class WebManagerViewsRestService
  {
    private string _CurrentObjectName;

    public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (SimpleWebManagerViews), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Limit, string Page, string text)
    {
      ...
      WebManagerViews currentObject = this.GetCurrentObject(0);
      ...
      foreach (DataRow row in (InternalDataCollectionBase) dataByText.Rows)
      {
        ...
        foreach (PropertyInfo property in simpleWebManagerViews.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByText.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) simpleWebManagerViews, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) simpleWebManagerViews);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = RowTotal };
    }

    private WebManagerViews GetCurrentObject(int Id = 0)
    {
      WebManagerViews webManagerViews = ObjectFactoryService.Create<WebManagerViews>();
      if (Id != 0)
        webManagerViews.Load(Id);
      return webManagerViews;
    }

    [WebGet(UriTemplate = "/GetCurrentObjectName")]
    public string GetCurrentObjectName()
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "WebManagerViews");
    }
    ...
    public SimpleBaseObject Create(SimpleWebManagerViews instance)
    {
      WebManagerViews currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, SimpleWebManagerViews instance)
    {
      WebManagerViews currentObject = this.GetCurrentObject(instance.Id);
