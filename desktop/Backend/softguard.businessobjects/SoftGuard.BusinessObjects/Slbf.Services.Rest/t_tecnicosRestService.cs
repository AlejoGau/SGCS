...
// Type: SoftGuard.BusinessObjects.Rest.t_tecnicosRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  [DataContractFormat]
  public class t_tecnicosRestService
  {
    private string _CurrentObjectName;

    public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (Simplet_tecnicos), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Page, string Start, string Limit, string Sort, string Group, string Filter)
    {
      ...
      t_tecnicos currentObject = this.GetCurrentObject(0);
      ...
      foreach (DataRow row in (InternalDataCollectionBase) dataByFilter.Rows)
      {
        ...
        foreach (PropertyInfo property in simpletTecnicos.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByFilter.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) simpletTecnicos, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) simpletTecnicos);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = TotalRows };
    }

    private t_tecnicos GetCurrentObject(int Id = 0)
    {
      t_tecnicos tTecnicos = ObjectFactoryService.Create<t_tecnicos>();
      if (Id != 0)
        tTecnicos.Load(Id);
      return tTecnicos;
    }
    ...
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "t_tecnicos");
    }
    ...
    public SimpleBaseObject Create(Simplet_tecnicos instance)
    {
      instance.Id = 0;
      t_tecnicos currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, Simplet_tecnicos instance)
    {
      t_tecnicos currentObject = this.GetCurrentObject(int.Parse(id));
