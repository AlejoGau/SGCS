...
// Type: SoftGuard.BusinessObjects.Rest.t_resolucionesRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  public class t_resolucionesRestService
  {
    private string _CurrentObjectName;

    public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (Simplet_resoluciones), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Page, string Start, string Limit, string Sort, string Group, string Filter)
    {
      ...
      t_resoluciones currentObject = this.GetCurrentObject(0);
      ...
      foreach (DataRow row in (InternalDataCollectionBase) dataByFilter.Rows)
      {
        ...
        foreach (PropertyInfo property in simpletResoluciones.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByFilter.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) simpletResoluciones, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) simpletResoluciones);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = TotalRows };
    }

    private t_resoluciones GetCurrentObject(int Id = 0)
    {
      t_resoluciones tResoluciones = ObjectFactoryService.Create<t_resoluciones>();
      if (Id != 0)
        tResoluciones.Load(Id);
      return tResoluciones;
    }

    [WebGet(UriTemplate = "/GetCurrentObjectName")]
    public string GetCurrentObjectName()
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "t_resoluciones");
    }
    ...
    public SimpleBaseObject Create(Simplet_resoluciones instance)
    {
      instance.Id = 0;
      t_resoluciones currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, Simplet_resoluciones instance)
    {
      t_resoluciones currentObject = this.GetCurrentObject(int.Parse(id));
