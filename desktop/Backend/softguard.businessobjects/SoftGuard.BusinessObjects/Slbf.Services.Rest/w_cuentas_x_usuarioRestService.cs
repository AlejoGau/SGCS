...
// Type: SoftGuard.BusinessObjects.Rest.w_cuentas_x_usuarioRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  [DataContractFormat]
  [ServiceContract]
  public class w_cuentas_x_usuarioRestService
  {
    private string _CurrentObjectName;

    public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (Simplew_cuentas_x_usuario), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Page, string Start, string Limit, string Sort, string Group, string Filter)
    {
      ...
      w_cuentas_x_usuario currentObject = this.GetCurrentObject(0);
      ...
      foreach (DataRow row in (InternalDataCollectionBase) dataByFilter.Rows)
      {
        ...
        foreach (PropertyInfo property in simplewCuentasXUsuario.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByFilter.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) simplewCuentasXUsuario, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) simplewCuentasXUsuario);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = TotalRows };
    }

    private w_cuentas_x_usuario GetCurrentObject(int Id = 0)
    {
      w_cuentas_x_usuario wCuentasXUsuario = ObjectFactoryService.Create<w_cuentas_x_usuario>();
      if (Id != 0)
        wCuentasXUsuario.Load(Id);
      return wCuentasXUsuario;
    }

    [WebGet(UriTemplate = "/GetCurrentObjectName")]
    public string GetCurrentObjectName()
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "w_cuentas_x_usuario");
    }
    ...
    public SimpleBaseObject Create(Simplew_cuentas_x_usuario instance)
    {
      instance.Id = 0;
      w_cuentas_x_usuario currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, Simplew_cuentas_x_usuario instance)
    {
      w_cuentas_x_usuario currentObject = this.GetCurrentObject(int.Parse(id));
