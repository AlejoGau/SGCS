...
// Type: SoftGuard.BusinessObjects.Rest.t_tags_agRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  public class t_tags_agRestService
  {
    private string _CurrentObjectName;

    public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (Simplet_tags_ag), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Page, string Start, string Limit, string Sort, string Group, string Filter)
    {
      ...
      t_tags_ag currentObject = this.GetCurrentObject(0);
      ...
      foreach (DataRow row in (InternalDataCollectionBase) dataByFilter.Rows)
      {
        ...
        foreach (PropertyInfo property in simpletTagsAg.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByFilter.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) simpletTagsAg, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) simpletTagsAg);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = TotalRows };
    }

    private t_tags_ag GetCurrentObject(int Id = 0)
    {
      t_tags_ag tTagsAg = ObjectFactoryService.Create<t_tags_ag>();
      if (Id != 0)
        tTagsAg.Load(Id);
      return tTagsAg;
    }
    ...
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "t_tags_ag");
    }
    ...
    public SimpleBaseObject Create(Simplet_tags_ag instance)
    {
      instance.Id = 0;
      t_tags_ag currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, Simplet_tags_ag instance)
    {
      t_tags_ag currentObject = this.GetCurrentObject(int.Parse(id));
