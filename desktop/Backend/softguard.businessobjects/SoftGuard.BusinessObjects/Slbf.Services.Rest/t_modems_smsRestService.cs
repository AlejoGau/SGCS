...
// Type: SoftGuard.BusinessObjects.Rest.t_modems_smsRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  [DataContractFormat]
  public class t_modems_smsRestService
  {
    private string _CurrentObjectName;

    public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (Simplet_modems_sms), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Page, string Start, string Limit, string Sort, string Group, string Filter)
    {
      ...
      t_modems_sms currentObject = this.GetCurrentObject(0);
      ...
      {
        Simplet_modems_sms simpletModemsSms = new Simplet_modems_sms();
        foreach (PropertyInfo property in simpletModemsSms.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByFilter.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) simpletModemsSms, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) simpletModemsSms);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = TotalRows };
    }

    private t_modems_sms GetCurrentObject(int Id = 0)
    {
      t_modems_sms tModemsSms = ObjectFactoryService.Create<t_modems_sms>();
      if (Id != 0)
        tModemsSms.Load(Id);
      return tModemsSms;
    }
    ...
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "t_modems_sms");
    }
    ...
    public SimpleBaseObject Create(Simplet_modems_sms instance)
    {
      instance.Id = 0;
      t_modems_sms currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, Simplet_modems_sms instance)
    {
      t_modems_sms currentObject = this.GetCurrentObject(int.Parse(id));
