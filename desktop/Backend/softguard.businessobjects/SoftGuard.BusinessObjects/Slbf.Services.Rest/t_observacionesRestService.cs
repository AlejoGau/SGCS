...
// Type: SoftGuard.BusinessObjects.Rest.t_observacionesRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  public class t_observacionesRestService
  {
    private string _CurrentObjectName;

    public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (Simplet_observaciones), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Page, string Start, string Limit, string Sort, string Group, string Filter)
    {
      ...
      t_observaciones currentObject = this.GetCurrentObject(0);
      ...
      foreach (DataRow row in (InternalDataCollectionBase) dataByFilter.Rows)
      {
        ...
        foreach (PropertyInfo property in simpletObservaciones.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByFilter.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) simpletObservaciones, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) simpletObservaciones);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = TotalRows };
    }

    private t_observaciones GetCurrentObject(int Id = 0)
    {
      t_observaciones tObservaciones = ObjectFactoryService.Create<t_observaciones>();
      if (Id != 0)
        tObservaciones.Load(Id);
    ...
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "t_observaciones");
    }
    ...
    public SimpleBaseObject Create(Simplet_observaciones instance)
    {
      instance.Id = 0;
      t_observaciones currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, Simplet_observaciones instance)
    {
      t_observaciones currentObject = this.GetCurrentObject(int.Parse(id));
