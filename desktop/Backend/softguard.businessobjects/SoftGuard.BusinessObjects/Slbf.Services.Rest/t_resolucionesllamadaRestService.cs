...
// Type: SoftGuard.BusinessObjects.Rest.t_resolucionesllamadaRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  public class t_resolucionesllamadaRestService
  {
    private string _CurrentObjectName;

    public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (Simplet_resolucionesllamada), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Page, string Start, string Limit, string Sort, string Group, string Filter)
    {
      ...
      t_resolucionesllamada currentObject = this.GetCurrentObject(0);
      ...
      foreach (DataRow row in (InternalDataCollectionBase) dataByFilter.Rows)
      {
        ...
        foreach (PropertyInfo property in resolucionesllamada.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByFilter.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) resolucionesllamada, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) resolucionesllamada);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = TotalRows };
    }

    private t_resolucionesllamada GetCurrentObject(int Id = 0)
    {
      t_resolucionesllamada resolucionesllamada = ObjectFactoryService.Create<t_resolucionesllamada>();
      if (Id != 0)
        resolucionesllamada.Load(Id);
      return resolucionesllamada;
    }

    [WebGet(UriTemplate = "/GetCurrentObjectName")]
    public string GetCurrentObjectName()
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "t_resolucionesllamada");
    }
    ...
    public SimpleBaseObject Create(Simplet_resolucionesllamada instance)
    {
      instance.Id = 0;
      t_resolucionesllamada currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, Simplet_resolucionesllamada instance)
    {
      t_resolucionesllamada currentObject = this.GetCurrentObject(int.Parse(id));
