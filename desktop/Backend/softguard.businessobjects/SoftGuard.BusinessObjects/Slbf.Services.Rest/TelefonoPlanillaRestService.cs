...
// Type: SoftGuard.BusinessObjects.Rest.TelefonoPlanillaRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  public class TelefonoPlanillaRestService
  {
    private string _CurrentObjectName;

    public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (SimpleTelefonoPlanilla), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Page, string Start, string Limit, string Sort, string Group, string Filter)
    {
      ...
      TelefonoPlanilla currentObject = this.GetCurrentObject(0);
      ...
      {
        SimpleTelefonoPlanilla telefonoPlanilla = new SimpleTelefonoPlanilla();
        foreach (PropertyInfo property in telefonoPlanilla.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByFilter.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) telefonoPlanilla, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) telefonoPlanilla);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = TotalRows };
    }

    private TelefonoPlanilla GetCurrentObject(int Id = 0)
    {
      TelefonoPlanilla telefonoPlanilla = ObjectFactoryService.Create<TelefonoPlanilla>();
      if (Id != 0)
        telefonoPlanilla.Load(Id);
      return telefonoPlanilla;
    }

    [WebGet(UriTemplate = "/GetCurrentObjectName")]
    public string GetCurrentObjectName()
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "TelefonoPlanilla");
    }
    ...
    public SimpleBaseObject Create(SimpleTelefonoPlanilla instance)
    {
      instance.Id = 0;
      TelefonoPlanilla currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, SimpleTelefonoPlanilla instance)
    {
      TelefonoPlanilla currentObject = this.GetCurrentObject(int.Parse(id));
