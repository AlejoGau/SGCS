...
namespace SoftGuard.BusinessObjects.Rest
{
    ...
    public partial class ResourceModuleRestService
    {
        public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
        {
            return new List<Type>(){
                typeof(SoftGuard.BusinessObjects.SimpleResourceModule),
        ...
        public PagedOutput GetCollection(string Page, string Start,string Limit, string Sort, string Group, string Filter )
        {
            ...
            {
                DataColumn c;
                var s = new SoftGuard.BusinessObjects.SimpleResourceModule();
        ...
        SoftGuard.BusinessObjects.ResourceModule GetCurrentObject(int Id = 0)
        {
            var Object = Slbf.ObjectFactoryService.Create<SoftGuard.BusinessObjects.ResourceModule>();
        ...
        {
            return _CurrentObjectName ?? (_CurrentObjectName = "ResourceModule");
        }
        ...
        public SimpleBaseObject Create(SoftGuard.BusinessObjects.SimpleResourceModule instance)
        {
			instance.Id = 0;
        ...
        [WebInvoke(UriTemplate = "{id}", Method = "PUT"
            , RequestFormat = WebMessageFormat.Json
            , ResponseFormat = WebMessageFormat.Json)]
        public SimpleBaseObject Update(string id, Stream requestStream)
        {
			var iid = int.Parse(id);
			var p = GetCurrentObject(iid); // this also loads an object
            var json = ReadRequestBody(requestStream);
            var instance = DeserializeJson<SoftGuard.BusinessObjects.SimpleResourceModule>(json);
            var propertyNames = GetTopLevelJsonPropertyNames(json);
            AssignSimpleObjectProperties(p, instance, propertyNames);
			p.Save();
			return p.GetSimpleObject();
        }
        ...
        private static IEnumerable<string> GetTopLevelJsonPropertyNames(string json)
        {
            if (string.IsNullOrWhiteSpace(json))
                return new string[] { };
            ...
            try
            {
                ...
                return (data == null) ? (IEnumerable<string>)new string[] { } : data.Keys;
            }
            catch
            {
                return new string[] { };
        ...
        private void AssignSimpleObjectProperties(object objectTo, object objectFrom, IEnumerable<string> propertyNames)
        {
            if (objectTo == null || objectFrom == null || propertyNames == null)
                return;
            ...
            foreach (var propertyName in propertyNames)
            {
                ...
                if (string.Equals(propertyName, "__type", StringComparison.OrdinalIgnoreCase) ||
                    string.Equals(propertyName, "Id", StringComparison.OrdinalIgnoreCase))
                    continue;

                PropertyInfo propFrom = objectFrom.GetType().GetProperty(propertyName, flags);
                if (propFrom == null)
                    continue;

                object value = propFrom.GetValue(objectFrom, null);
                PropertyInfo propTo = objectTo.GetType().GetProperty(propFrom.Name, flags);
