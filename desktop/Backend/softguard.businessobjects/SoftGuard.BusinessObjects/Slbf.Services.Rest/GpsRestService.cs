...
// Type: SoftGuard.BusinessObjects.Rest.GpsRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  public class GpsRestService
  {
    ...
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (SimpleGps), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Limit, string Page, string text)
    {
      ...
      Gps currentObject = this.GetCurrentObject(0);
      ...
      foreach (DataRow row in (InternalDataCollectionBase) dataByText.Rows)
      {
        ...
        foreach (PropertyInfo property in simpleGps.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByText.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) simpleGps, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) simpleGps);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = RowTotal };
    }

    private Gps GetCurrentObject(int Id = 0)
    {
      Gps gps = ObjectFactoryService.Create<Gps>();
      if (Id != 0)
        gps.Load(Id);
      return gps;
    }
    ...
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "Gps");
    }
    ...
    public SimpleBaseObject Create(SimpleGps instance)
    {
      Gps currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, SimpleGps instance)
    {
      Gps currentObject = this.GetCurrentObject(instance.Id);
    ...
    {
      Gps currentObject = this.GetCurrentObject(int.Parse(id));
    ...
    {
      string str = HttpUtility.UrlDecode(HttpContext.Current.Request.Form.ToString());
      this.GetCurrentObject(int.Parse(id)).MetadataObject().SetDynamicJson((object) str);
    }
  }
}
