...
// Type: SoftGuard.BusinessObjects.Rest.t_gruposRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  public class t_gruposRestService
  {
    private string _CurrentObjectName;

    public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (Simplet_grupos), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Page, string Start, string Limit, string Sort, string Group, string Filter)
    {
      ...
      t_grupos currentObject = this.GetCurrentObject(0);
      ...
      foreach (DataRow row in (InternalDataCollectionBase) dataByFilter.Rows)
      {
        ...
        foreach (PropertyInfo property in simpletGrupos.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByFilter.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) simpletGrupos, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) simpletGrupos);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = TotalRows };
    }

    private t_grupos GetCurrentObject(int Id = 0)
    {
      t_grupos tGrupos = ObjectFactoryService.Create<t_grupos>();
      if (Id != 0)
        tGrupos.Load(Id);
      return tGrupos;
    }
    ...
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "t_grupos");
    }
    ...
    public SimpleBaseObject Create(Simplet_grupos instance)
    {
      instance.Id = 0;
      t_grupos currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, Simplet_grupos instance)
    {
      t_grupos currentObject = this.GetCurrentObject(int.Parse(id));
