...
// Type: SoftGuard.BusinessObjects.Rest.GrupoCuentasRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  public class GrupoCuentasRestService
  {
    private string _CurrentObjectName;

    public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (SimpleGrupoCuentas), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Page, string Start, string Limit, string Sort, string Group, string Filter)
    {
      ...
      GrupoCuentas currentObject = this.GetCurrentObject(0);
      ...
      foreach (DataRow row in (InternalDataCollectionBase) dataByFilter.Rows)
      {
        ...
        foreach (PropertyInfo property in simpleGrupoCuentas.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByFilter.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) simpleGrupoCuentas, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) simpleGrupoCuentas);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = TotalRows };
    }

    private GrupoCuentas GetCurrentObject(int Id = 0)
    {
      GrupoCuentas grupoCuentas = ObjectFactoryService.Create<GrupoCuentas>();
      if (Id != 0)
        grupoCuentas.Load(Id);
      return grupoCuentas;
    }
    ...
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "GrupoCuentas");
    }
    ...
    public SimpleBaseObject Create(SimpleGrupoCuentas instance)
    {
      instance.Id = 0;
      GrupoCuentas currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, SimpleGrupoCuentas instance)
    {
      GrupoCuentas currentObject = this.GetCurrentObject(int.Parse(id));
