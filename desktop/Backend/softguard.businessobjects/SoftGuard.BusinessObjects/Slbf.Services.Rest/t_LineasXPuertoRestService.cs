...
// Type: SoftGuard.BusinessObjects.Rest.t_LineasXPuertoRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  [DataContractFormat]
  [ServiceContract]
  public class t_LineasXPuertoRestService
  {
    private string _CurrentObjectName;

    public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (Simplet_LineasXPuerto), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Page, string Start, string Limit, string Sort, string Group, string Filter)
    {
      ...
      t_LineasXPuerto currentObject = this.GetCurrentObject(0);
      ...
      foreach (DataRow row in (InternalDataCollectionBase) dataByFilter.Rows)
      {
        ...
        foreach (PropertyInfo property in simpletLineasXpuerto.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByFilter.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) simpletLineasXpuerto, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) simpletLineasXpuerto);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = TotalRows };
    }

    private t_LineasXPuerto GetCurrentObject(int Id = 0)
    {
      t_LineasXPuerto tLineasXpuerto = ObjectFactoryService.Create<t_LineasXPuerto>();
      if (Id != 0)
        tLineasXpuerto.Load(Id);
      return tLineasXpuerto;
    }

    [WebGet(UriTemplate = "/GetCurrentObjectName")]
    public string GetCurrentObjectName()
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "t_LineasXPuerto");
    }
    ...
    public SimpleBaseObject Create(Simplet_LineasXPuerto instance)
    {
      instance.Id = 0;
      t_LineasXPuerto currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, Simplet_LineasXPuerto instance)
    {
      t_LineasXPuerto currentObject = this.GetCurrentObject(int.Parse(id));
