...
// Type: SoftGuard.BusinessObjects.Rest.WebManagerGraphicsRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  [DataContractFormat]
  public class WebManagerGraphicsRestService
  {
    private string _CurrentObjectName;

    public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (SimpleWebManagerGraphics), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Limit, string Page, string text)
    {
      ...
      WebManagerGraphics currentObject = this.GetCurrentObject(0);
      ...
      foreach (DataRow row in (InternalDataCollectionBase) dataByText.Rows)
      {
        ...
        foreach (PropertyInfo property in webManagerGraphics.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByText.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) webManagerGraphics, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) webManagerGraphics);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = RowTotal };
    }

    private WebManagerGraphics GetCurrentObject(int Id = 0)
    {
      WebManagerGraphics webManagerGraphics = ObjectFactoryService.Create<WebManagerGraphics>();
      if (Id != 0)
        webManagerGraphics.Load(Id);
      return webManagerGraphics;
    }

    [WebGet(UriTemplate = "/GetCurrentObjectName")]
    public string GetCurrentObjectName()
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "WebManagerGraphics");
    }
    ...
    public SimpleBaseObject Create(SimpleWebManagerGraphics instance)
    {
      WebManagerGraphics currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, SimpleWebManagerGraphics instance)
    {
      WebManagerGraphics currentObject = this.GetCurrentObject(instance.Id);
