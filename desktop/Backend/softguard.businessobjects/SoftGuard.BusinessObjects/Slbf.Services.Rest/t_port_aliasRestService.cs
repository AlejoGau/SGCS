...
// Type: SoftGuard.BusinessObjects.Rest.t_port_aliasRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  [DataContractFormat]
  public class t_port_aliasRestService
  {
    private string _CurrentObjectName;

    public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (Simplet_port_alias), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Page, string Start, string Limit, string Sort, string Group, string Filter)
    {
      ...
      t_port_alias currentObject = this.GetCurrentObject(0);
      ...
      foreach (DataRow row in (InternalDataCollectionBase) dataByFilter.Rows)
      {
        ...
        foreach (PropertyInfo property in simpletPortAlias.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByFilter.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) simpletPortAlias, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) simpletPortAlias);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = TotalRows };
    }

    private t_port_alias GetCurrentObject(int Id = 0)
    {
      t_port_alias tPortAlias = ObjectFactoryService.Create<t_port_alias>();
      if (Id != 0)
        tPortAlias.Load(Id);
      return tPortAlias;
    }
    ...
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "t_port_alias");
    }
    ...
    public SimpleBaseObject Create(Simplet_port_alias instance)
    {
      instance.Id = 0;
      t_port_alias currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, Simplet_port_alias instance)
    {
      t_port_alias currentObject = this.GetCurrentObject(int.Parse(id));
