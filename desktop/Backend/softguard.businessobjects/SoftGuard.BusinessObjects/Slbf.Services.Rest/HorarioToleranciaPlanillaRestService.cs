...
// Type: SoftGuard.BusinessObjects.Rest.HorarioToleranciaPlanillaRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  [DataContractFormat]
  [ServiceContract]
  public class HorarioToleranciaPlanillaRestService
  {
    private string _CurrentObjectName;

    public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (SimpleHorarioToleranciaPlanilla), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Page, string Start, string Limit, string Sort, string Group, string Filter)
    {
      ...
      HorarioToleranciaPlanilla currentObject = this.GetCurrentObject(0);
      ...
      foreach (DataRow row in (InternalDataCollectionBase) dataByFilter.Rows)
      {
        ...
        foreach (PropertyInfo property in toleranciaPlanilla.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByFilter.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) toleranciaPlanilla, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) toleranciaPlanilla);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = TotalRows };
    }

    private HorarioToleranciaPlanilla GetCurrentObject(int Id = 0)
    {
      HorarioToleranciaPlanilla toleranciaPlanilla = ObjectFactoryService.Create<HorarioToleranciaPlanilla>();
      if (Id != 0)
        toleranciaPlanilla.Load(Id);
      return toleranciaPlanilla;
    }

    [WebGet(UriTemplate = "/GetCurrentObjectName")]
    public string GetCurrentObjectName()
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "HorarioToleranciaPlanilla");
    }
    ...
    public SimpleBaseObject Create(SimpleHorarioToleranciaPlanilla instance)
    {
      instance.Id = 0;
      HorarioToleranciaPlanilla currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, SimpleHorarioToleranciaPlanilla instance)
    {
      HorarioToleranciaPlanilla currentObject = this.GetCurrentObject(int.Parse(id));
