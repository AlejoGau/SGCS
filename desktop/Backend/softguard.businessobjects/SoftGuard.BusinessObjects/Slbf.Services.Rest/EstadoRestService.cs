...
// Type: SoftGuard.BusinessObjects.Rest.EstadoRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  public class EstadoRestService
  {
    ...
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (SimpleEstado), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Limit, string Page, string text)
    {
      ...
      Estado currentObject = this.GetCurrentObject(0);
      ...
      {
        row["id"].ToString();
        SimpleEstado simpleEstado = new SimpleEstado();
        foreach (PropertyInfo property in simpleEstado.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByText.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) simpleEstado, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) simpleEstado);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = RowTotal };
    }

    private Estado GetCurrentObject(int Id = 0)
    {
      Estado estado = ObjectFactoryService.Create<Estado>();
      if (Id != 0)
        estado.Load(Id);
      return estado;
    }
    ...
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "Estado");
    }
    ...
    public SimpleBaseObject Create(SimpleEstado instance)
    {
      Estado currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, SimpleEstado instance)
    {
      Estado currentObject = this.GetCurrentObject(instance.Id);
