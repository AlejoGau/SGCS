...
// Type: SoftGuard.BusinessObjects.Rest.HorarioAlternativoPlantillaRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  [DataContractFormat]
  [ServiceContract]
  public class HorarioAlternativoPlantillaRestService
  {
    private string _CurrentObjectName;

    public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (SimpleHorarioAlternativoPlantilla), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Page, string Start, string Limit, string Sort, string Group, string Filter)
    {
      ...
      HorarioAlternativoPlantilla currentObject = this.GetCurrentObject(0);
      ...
      foreach (DataRow row in (InternalDataCollectionBase) dataByFilter.Rows)
      {
        ...
        foreach (PropertyInfo property in alternativoPlantilla.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByFilter.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) alternativoPlantilla, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) alternativoPlantilla);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = TotalRows };
    }

    private HorarioAlternativoPlantilla GetCurrentObject(int Id = 0)
    {
      HorarioAlternativoPlantilla alternativoPlantilla = ObjectFactoryService.Create<HorarioAlternativoPlantilla>();
      if (Id != 0)
        alternativoPlantilla.Load(Id);
      return alternativoPlantilla;
    }

    [WebGet(UriTemplate = "/GetCurrentObjectName")]
    public string GetCurrentObjectName()
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "HorarioAlternativoPlantilla");
    }
    ...
    public SimpleBaseObject Create(SimpleHorarioAlternativoPlantilla instance)
    {
      instance.Id = 0;
      HorarioAlternativoPlantilla currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, SimpleHorarioAlternativoPlantilla instance)
    {
      HorarioAlternativoPlantilla currentObject = this.GetCurrentObject(int.Parse(id));
