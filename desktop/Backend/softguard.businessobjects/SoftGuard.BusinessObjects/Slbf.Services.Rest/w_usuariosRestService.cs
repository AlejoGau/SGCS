...
// Type: SoftGuard.BusinessObjects.Rest.w_usuariosRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  public class w_usuariosRestService
  {
    private string _CurrentObjectName;

    public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (Simplew_usuarios), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Page, string Start, string Limit, string Sort, string Group, string Filter)
    {
      ...
      w_usuarios currentObject = this.GetCurrentObject(0);
      ...
      {
        row["id_login"].ToString();
        Simplew_usuarios simplewUsuarios = new Simplew_usuarios();
        foreach (PropertyInfo property in simplewUsuarios.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByFilter.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) simplewUsuarios, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) simplewUsuarios);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = TotalRows };
    }

    private w_usuarios GetCurrentObject(int Id = 0)
    {
      w_usuarios wUsuarios = ObjectFactoryService.Create<w_usuarios>();
      if (Id != 0)
        wUsuarios.Load(Id);
      return wUsuarios;
    }
    ...
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "w_usuarios");
    }
    ...
    public SimpleBaseObject Create(Simplew_usuarios instance)
    {
      instance.Id = 0;
      w_usuarios currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, Simplew_usuarios instance)
    {
      w_usuarios currentObject = this.GetCurrentObject(int.Parse(id));
