...
// Type: SoftGuard.BusinessObjects.Rest.m_planillasRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  [DataContractFormat]
  public class m_planillasRestService
  {
    private string _CurrentObjectName;

    public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (Simplem_planillas), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Page, string Start, string Limit, string Sort, string Group, string Filter)
    {
      ...
      m_planillas currentObject = this.GetCurrentObject(0);
      ...
      foreach (DataRow row in (InternalDataCollectionBase) dataByFilter.Rows)
      {
        ...
        foreach (PropertyInfo property in simplemPlanillas.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByFilter.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) simplemPlanillas, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) simplemPlanillas);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = TotalRows };
    }

    private m_planillas GetCurrentObject(int Id = 0)
    {
      m_planillas mPlanillas = ObjectFactoryService.Create<m_planillas>();
      if (Id != 0)
        mPlanillas.Load(Id);
      return mPlanillas;
    }
    ...
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "m_planillas");
    }
    ...
    public SimpleBaseObject Create(Simplem_planillas instance)
    {
      instance.Id = 0;
      m_planillas currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, Simplem_planillas instance)
    {
      m_planillas currentObject = this.GetCurrentObject(int.Parse(id));
