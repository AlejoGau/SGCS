...
// Type: SoftGuard.BusinessObjects.Rest.TelefonoRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  public class TelefonoRestService
  {
    private string _CurrentObjectName;

    public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (SimpleTelefono), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Page, string Start, string Limit, string Sort, string Group, string Filter)
    {
      ...
      Telefono currentObject = this.GetCurrentObject(0);
      ...
      {
        SimpleTelefono simpleTelefono = new SimpleTelefono();
        foreach (PropertyInfo property in simpleTelefono.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByFilter.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) simpleTelefono, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) simpleTelefono);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = TotalRows };
    }

    private Telefono GetCurrentObject(int Id = 0)
    {
      Telefono telefono = ObjectFactoryService.Create<Telefono>();
      if (Id != 0)
        telefono.Load(Id);
      return telefono;
    }
    ...
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "Telefono");
    }
    ...
    public SimpleBaseObject Create(SimpleTelefono instance)
    {
      instance.Id = 0;
      Telefono currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, SimpleTelefono instance)
    {
      Telefono currentObject = this.GetCurrentObject(int.Parse(id));
