...
// Type: SoftGuard.BusinessObjects.Rest.ZonaRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  public class ZonaRestService
  {
    ...
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (SimpleZona), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Page, string Start, string Limit, string Sort, string Group, string Filter)
    {
      ...
      Zona currentObject = this.GetCurrentObject(0);
      ...
      foreach (DataRow row in (InternalDataCollectionBase) dataByFilter.Rows)
      {
        ...
        foreach (PropertyInfo property in simpleZona.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByFilter.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) simpleZona, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) simpleZona);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = TotalRows };
    }

    private Zona GetCurrentObject(int Id = 0)
    {
      Zona zona = ObjectFactoryService.Create<Zona>();
      if (Id != 0)
        zona.Load(Id);
      return zona;
    }
    ...
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "Zona");
    }
    ...
    public SimpleBaseObject Create(SimpleZona instance)
    {
      instance.Id = 0;
      Zona currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, SimpleZona instance)
    {
      Zona currentObject = this.GetCurrentObject(int.Parse(id));
