...
// Type: SoftGuard.BusinessObjects.Rest.t_categorizacionRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  [DataContractFormat]
  public class t_categorizacionRestService
  {
    private string _CurrentObjectName;

    public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (Simplet_categorizacion), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Page, string Start, string Limit, string Sort, string Group, string Filter)
    {
      ...
      t_categorizacion currentObject = this.GetCurrentObject(0);
      ...
      {
        Simplet_categorizacion simpletCategorizacion = new Simplet_categorizacion();
        foreach (PropertyInfo property in simpletCategorizacion.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByFilter.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) simpletCategorizacion, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) simpletCategorizacion);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = TotalRows };
    }

    private t_categorizacion GetCurrentObject(int Id = 0)
    {
      t_categorizacion tCategorizacion = ObjectFactoryService.Create<t_categorizacion>();
      if (Id != 0)
        tCategorizacion.Load(Id);
      return tCategorizacion;
    }

    [WebGet(UriTemplate = "/GetCurrentObjectName")]
    public string GetCurrentObjectName()
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "t_categorizacion");
    }
    ...
    public SimpleBaseObject Create(Simplet_categorizacion instance)
    {
      instance.Id = 0;
      t_categorizacion currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, Simplet_categorizacion instance)
    {
      t_categorizacion currentObject = this.GetCurrentObject(int.Parse(id));
