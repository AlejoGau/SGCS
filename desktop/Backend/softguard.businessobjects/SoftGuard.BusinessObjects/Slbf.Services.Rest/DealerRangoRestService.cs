...
// Type: SoftGuard.BusinessObjects.Rest.DealerRangoRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  public class DealerRangoRestService
  {
    private string _CurrentObjectName;

    public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (SimpleDealerRango), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Page, string Start, string Limit, string Sort, string Group, string Filter)
    {
      ...
      DealerRango currentObject = this.GetCurrentObject(0);
      ...
      foreach (DataRow row in (InternalDataCollectionBase) dataByFilter.Rows)
      {
        ...
        foreach (PropertyInfo property in simpleDealerRango.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByFilter.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) simpleDealerRango, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) simpleDealerRango);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = TotalRows };
    }

    private DealerRango GetCurrentObject(int Id = 0)
    {
      DealerRango dealerRango = ObjectFactoryService.Create<DealerRango>();
      if (Id != 0)
        dealerRango.Load(Id);
      return dealerRango;
    }
    ...
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "DealerRango");
    }
    ...
    public SimpleBaseObject Create(SimpleDealerRango instance)
    {
      instance.Id = 0;
      DealerRango currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, SimpleDealerRango instance)
    {
      DealerRango currentObject = this.GetCurrentObject(int.Parse(id));
