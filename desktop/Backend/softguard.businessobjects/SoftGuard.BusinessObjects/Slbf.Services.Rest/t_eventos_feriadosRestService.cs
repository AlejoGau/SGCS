...
// Type: SoftGuard.BusinessObjects.Rest.t_eventos_feriadosRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  [DataContractFormat]
  public class t_eventos_feriadosRestService
  {
    private string _CurrentObjectName;

    public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (Simplet_eventos_feriados), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Page, string Start, string Limit, string Sort, string Group, string Filter)
    {
      ...
      t_eventos_feriados currentObject = this.GetCurrentObject(0);
      ...
      foreach (DataRow row in (InternalDataCollectionBase) dataByFilter.Rows)
      {
        ...
        foreach (PropertyInfo property in simpletEventosFeriados.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByFilter.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) simpletEventosFeriados, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) simpletEventosFeriados);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = TotalRows };
    }

    private t_eventos_feriados GetCurrentObject(int Id = 0)
    {
      t_eventos_feriados tEventosFeriados = ObjectFactoryService.Create<t_eventos_feriados>();
      if (Id != 0)
        tEventosFeriados.Load(Id);
      return tEventosFeriados;
    }

    [WebGet(UriTemplate = "/GetCurrentObjectName")]
    public string GetCurrentObjectName()
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "t_eventos_feriados");
    }
    ...
    public SimpleBaseObject Create(Simplet_eventos_feriados instance)
    {
      instance.Id = 0;
      t_eventos_feriados currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, Simplet_eventos_feriados instance)
    {
      t_eventos_feriados currentObject = this.GetCurrentObject(int.Parse(id));
