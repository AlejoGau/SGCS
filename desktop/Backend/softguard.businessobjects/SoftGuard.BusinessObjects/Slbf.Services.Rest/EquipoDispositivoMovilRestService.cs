...
// Type: SoftGuard.BusinessObjects.Rest.EquipoDispositivoMovilRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  public class EquipoDispositivoMovilRestService
  {
    private string _CurrentObjectName;

    public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (SimpleEquipoDispositivoMovil), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Page, string Start, string Limit, string Sort, string Group, string Filter)
    {
      ...
      EquipoDispositivoMovil currentObject = this.GetCurrentObject(0);
      ...
      foreach (DataRow row in (InternalDataCollectionBase) dataByFilter.Rows)
      {
        ...
        foreach (PropertyInfo property in dispositivoMovil.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByFilter.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) dispositivoMovil, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) dispositivoMovil);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = TotalRows };
    }

    private EquipoDispositivoMovil GetCurrentObject(int Id = 0)
    {
      EquipoDispositivoMovil dispositivoMovil = ObjectFactoryService.Create<EquipoDispositivoMovil>();
      if (Id != 0)
        dispositivoMovil.Load(Id);
    ...
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "EquipoDispositivoMovil");
    }
    ...
    public SimpleBaseObject Create(SimpleEquipoDispositivoMovil instance)
    {
      instance.Id = 0;
      EquipoDispositivoMovil currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, SimpleEquipoDispositivoMovil instance)
    {
      EquipoDispositivoMovil currentObject = this.GetCurrentObject(int.Parse(id));
