...
// Type: Slbf.Services.Rest.HorarioRestService
...
namespace Slbf.Services.Rest
{
  ...
  public class HorarioRestService
  {
    ...
    {
      foreach (MemberInfo knownType in HorarioRestService.GetKnownTypes((ICustomAttributeProvider) null))
      {
    ...
    public SimpleBaseObject Create(SimpleHorario instance)
    {
      ISpecialization currentObject = this.GetCurrentObject();
    ...
    public SimpleBaseObject Update(string id, SimpleHorario instance)
    {
      try
      {
        ISpecialization currentObject = this.GetCurrentObject();
