...
// Type: SoftGuard.BusinessObjects.Rest.t_medicosRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  public class t_medicosRestService
  {
    private string _CurrentObjectName;

    public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (Simplet_medicos), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Page, string Start, string Limit, string Sort, string Group, string Filter)
    {
      ...
      t_medicos currentObject = this.GetCurrentObject(0);
      ...
      foreach (DataRow row in (InternalDataCollectionBase) dataByFilter.Rows)
      {
        ...
        foreach (PropertyInfo property in simpletMedicos.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByFilter.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) simpletMedicos, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) simpletMedicos);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = TotalRows };
    }

    private t_medicos GetCurrentObject(int Id = 0)
    {
      t_medicos tMedicos = ObjectFactoryService.Create<t_medicos>();
      if (Id != 0)
        tMedicos.Load(Id);
      return tMedicos;
    }
    ...
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "t_medicos");
    }
    ...
    public SimpleBaseObject Create(Simplet_medicos instance)
    {
      instance.Id = 0;
      t_medicos currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, Simplet_medicos instance)
    {
      t_medicos currentObject = this.GetCurrentObject(int.Parse(id));
