...
// Type: SoftGuard.BusinessObjects.Rest.TaskStatusRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  public class TaskStatusRestService
  {
    ...
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (SimpleTaskStatus), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Page, string Start, string Limit, string Sort, string Group, string Filter)
    {
      ...
      TaskStatus currentObject = this.GetCurrentObject(0);
      ...
      foreach (DataRow row in (InternalDataCollectionBase) dataByFilter.Rows)
      {
        ...
        foreach (PropertyInfo property in simpleTaskStatus.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByFilter.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) simpleTaskStatus, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) simpleTaskStatus);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = TotalRows };
    }

    private TaskStatus GetCurrentObject(int Id = 0)
    {
      TaskStatus taskStatus = ObjectFactoryService.Create<TaskStatus>();
      if (Id != 0)
        taskStatus.Load(Id);
      return taskStatus;
    }
    ...
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "TaskStatus");
    }
    ...
    public SimpleBaseObject Create(SimpleTaskStatus instance)
    {
      instance.Id = 0;
      TaskStatus currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, SimpleTaskStatus instance)
    {
      TaskStatus currentObject = this.GetCurrentObject(int.Parse(id));
