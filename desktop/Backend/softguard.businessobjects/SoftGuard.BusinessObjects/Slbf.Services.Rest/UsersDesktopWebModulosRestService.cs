...
// Type: SoftGuard.BusinessObjects.Rest.UsersDesktopWebModulosRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  [DataContractFormat]
  [ServiceContract]
  public class UsersDesktopWebModulosRestService
  {
    private string _CurrentObjectName;

    public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (SimpleUsersDesktopWebModulos), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Page, string Start, string Limit, string Sort, string Group, string Filter)
    {
      ...
      UsersDesktopWebModulos currentObject = this.GetCurrentObject(0);
      ...
      foreach (DataRow row in (InternalDataCollectionBase) dataByFilter.Rows)
      {
        ...
        foreach (PropertyInfo property in desktopWebModulos.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByFilter.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) desktopWebModulos, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) desktopWebModulos);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = TotalRows };
    }

    private UsersDesktopWebModulos GetCurrentObject(int Id = 0)
    {
      UsersDesktopWebModulos desktopWebModulos = ObjectFactoryService.Create<UsersDesktopWebModulos>();
      if (Id != 0)
        desktopWebModulos.Load(Id);
      return desktopWebModulos;
    }

    [WebGet(UriTemplate = "/GetCurrentObjectName")]
    public string GetCurrentObjectName()
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "UsersDesktopWebModulos");
    }
    ...
    public SimpleBaseObject Create(SimpleUsersDesktopWebModulos instance)
    {
      instance.Id = 0;
      UsersDesktopWebModulos currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, SimpleUsersDesktopWebModulos instance)
    {
      UsersDesktopWebModulos currentObject = this.GetCurrentObject(int.Parse(id));
