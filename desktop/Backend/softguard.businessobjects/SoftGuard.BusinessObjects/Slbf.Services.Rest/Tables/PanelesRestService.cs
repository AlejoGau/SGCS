...
namespace SoftGuard.EnterpriseServices.Rest
{
    ...
    public class PanelesRestService
    {
        ...
        [WebGet(UriTemplate = "")]        
        public Paneles[] GetCollection()
        {
            PanelesManager Manager = new PanelesManager(ConfigurationManager.ConnectionStrings["Slbf"].ConnectionString);
            // me fijo que tenga token valido.
            var token = Slbf.Security.UserService.GetId();
            IEnumerable<Paneles> Result = Manager.GetAll();
            
            return new List<Paneles>(Result).ToArray();
        }
