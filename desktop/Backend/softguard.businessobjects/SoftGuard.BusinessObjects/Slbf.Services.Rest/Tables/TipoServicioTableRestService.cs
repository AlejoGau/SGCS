...
namespace SoftGuard.EnterpriseServices.Rest
{
    ...
    public class TipoServicioRestService
    {
        ...
        public TipoServicio[] GetCollection()
        {
            TipoServicioManager Manager = new TipoServicioManager(ConfigurationManager.ConnectionStrings["Slbf"].ConnectionString);
            // me fijo que tenga token valido.
            var token = Slbf.Security.UserService.GetId();
            IEnumerable<TipoServicio> Result = Manager.GetAll();
            
            return new List<TipoServicio>(Result).ToArray();
        }
    }
}
