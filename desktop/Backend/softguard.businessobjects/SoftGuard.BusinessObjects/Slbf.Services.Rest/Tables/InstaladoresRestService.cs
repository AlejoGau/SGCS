...
namespace SoftGuard.EnterpriseServices.Rest
{
    ...
    public class InstaladoresRestService
    {
        ...
        public Instaladores[] GetCollection()
        {
            InstaladoresManager Manager = new InstaladoresManager(ConfigurationManager.ConnectionStrings["Slbf"].ConnectionString);

            // me fijo que tenga token valido.
            var token = Slbf.Security.UserService.GetId();

            IEnumerable<Instaladores> Result = Manager.GetAll();
            
            return new List<Instaladores>(Result).ToArray();
        }
    }
}
