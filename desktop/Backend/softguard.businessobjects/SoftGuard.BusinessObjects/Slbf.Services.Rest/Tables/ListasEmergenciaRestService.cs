...
namespace SoftGuard.EnterpriseServices.Rest
{
    ...
    public class ListasEmergenciaRestService
    {
        public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
        {
            return new List<Type>()
            {
                typeof(ListasEmergencia)
            };            
        }


        [WebGet(UriTemplate = "")]        
        public ListasEmergencia[] GetCollection()
        {
            ListasEmergenciaManager Manager = new ListasEmergenciaManager(ConfigurationManager.ConnectionStrings["Slbf"].ConnectionString);
            // me fijo que tenga token valido.
            var token = Slbf.Security.UserService.GetId();
            IEnumerable<ListasEmergencia> Result = Manager.GetAll();
            
            return new List<ListasEmergencia>(Result).ToArray();
        }
