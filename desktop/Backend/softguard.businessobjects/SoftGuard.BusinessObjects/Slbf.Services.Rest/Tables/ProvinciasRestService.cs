...
namespace SoftGuard.EnterpriseServices.Rest
{
    ...
    public class ProvinciasRestService
    {
        ...
        [WebGet(UriTemplate = "")]        
        public Provincias[] GetCollection()
        {
            ProvinciasManager Manager = new ProvinciasManager(ConfigurationManager.ConnectionStrings["Slbf"].ConnectionString);
            // me fijo que tenga token valido.
            var token = Slbf.Security.UserService.GetId();
            IEnumerable<Provincias> Result = Manager.GetAll();
            
            return new List<Provincias>(Result).ToArray();
        }
