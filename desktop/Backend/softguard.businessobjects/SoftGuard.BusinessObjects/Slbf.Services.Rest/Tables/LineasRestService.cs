...
namespace SoftGuard.EnterpriseServices.Rest
{
    ...
    public class LineasRestService
    {
        ...
        public Lineas[] GetCollection()
        {
            LineasManager Manager = new LineasManager(ConfigurationManager.ConnectionStrings["Slbf"].ConnectionString);
            // me fijo que tenga token valido.

            var token = Slbf.Security.UserService.GetId();
            IEnumerable<Lineas> Result = Manager.GetAll();
            
            return new List<Lineas>(Result).ToArray();
        }
    }
}
