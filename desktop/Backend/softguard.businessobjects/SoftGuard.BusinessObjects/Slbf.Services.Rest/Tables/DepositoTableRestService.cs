...
namespace SoftGuard.EnterpriseServices.Rest
{
    ...
    public class DepositoRestService
    {
        ...
        public Deposito[] GetCollection()
        {
            DepositoManager Manager = new DepositoManager(ConfigurationManager.ConnectionStrings["Slbf"].ConnectionString);
            // me fijo que tenga token valido.
            var token = Slbf.Security.UserService.GetId();
            IEnumerable<Deposito> Result = Manager.GetAll();
            
            return new List<Deposito>(Result).ToArray();
        }
    }
}
