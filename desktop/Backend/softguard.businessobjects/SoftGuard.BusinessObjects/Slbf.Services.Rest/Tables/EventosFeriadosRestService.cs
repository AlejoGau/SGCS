...
namespace SoftGuard.EnterpriseServices.Rest
{
    ...
    public class EventosFeriadosRestService
    {
        public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
        {
            return new List<Type>()
            {
                typeof(EventosFeriados)
            };            
        }


        [WebGet(UriTemplate = "")]        
        public EventosFeriados[] GetCollection()
        {
            EventosFeriadosManager Manager = new EventosFeriadosManager(ConfigurationManager.ConnectionStrings["Slbf"].ConnectionString);
            // me fijo que tenga token valido.
            var token = Slbf.Security.UserService.GetId();

            IEnumerable<EventosFeriados> Result = Manager.GetAll();
            
            return new List<EventosFeriados>(Result).ToArray();
        }
