...
namespace SoftGuard.EnterpriseServices.Rest
{
    ...
    public class MedicosRestService
    {
        ...
        public Medicos[] GetCollection()
        {
            MedicosManager Manager = new MedicosManager(ConfigurationManager.ConnectionStrings["Slbf"].ConnectionString);
            // me fijo que tenga token valido.
            var token = Slbf.Security.UserService.GetId();
            IEnumerable<Medicos> Result = Manager.GetAll();
            
            return new List<Medicos>(Result).ToArray();
        }
    }
}
