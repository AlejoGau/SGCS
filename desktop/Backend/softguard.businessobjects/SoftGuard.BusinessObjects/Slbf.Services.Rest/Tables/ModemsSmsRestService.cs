...
namespace SoftGuard.EnterpriseServices.Rest
{
    ...
    public class ModemsSmsRestService
    {
        ...
        public ModemsSms[] GetCollection()
        {
            ModemsSmsManager Manager = new ModemsSmsManager(ConfigurationManager.ConnectionStrings["Slbf"].ConnectionString);
            // me fijo que tenga token valido.
            var token = Slbf.Security.UserService.GetId();
            IEnumerable<ModemsSms> Result = Manager.GetAll();
            
            return new List<ModemsSms>(Result).ToArray();
        }
    }
}
