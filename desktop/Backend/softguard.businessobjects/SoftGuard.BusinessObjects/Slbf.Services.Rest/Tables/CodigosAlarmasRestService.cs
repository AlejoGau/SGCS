...
namespace SoftGuard.EnterpriseServices.Rest
{
    ...
    public class CodigosAlarmasRestService
    {
        public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
        {
            return new List<Type>()
            {
                typeof(CodigosAlarmas)
            };            
        }


        [WebGet(UriTemplate = "")]        
        public CodigosAlarmas[] GetCollection()
        {
            CodigosAlarmasManager Manager = new CodigosAlarmasManager(ConfigurationManager.ConnectionStrings["Slbf"].ConnectionString);

            // me fijo que tenga token valido.
            var token = Slbf.Security.UserService.GetId();


            IEnumerable<CodigosAlarmas> Result = Manager.GetAll();
            
            return new List<CodigosAlarmas>(Result).ToArray();
        }
