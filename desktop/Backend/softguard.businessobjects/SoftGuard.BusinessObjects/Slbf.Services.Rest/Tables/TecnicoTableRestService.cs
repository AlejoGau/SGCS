...
namespace SoftGuard.EnterpriseServices.Rest
{
    ...
    public class TecnicoRestService
    {
        ...
        public Tecnico[] GetCollection()
        {
            TecnicoManager Manager = new TecnicoManager(ConfigurationManager.ConnectionStrings["Slbf"].ConnectionString);
            // me fijo que tenga token valido.
            var token = Slbf.Security.UserService.GetId();
            IEnumerable<Tecnico> Result = Manager.GetAll();
            
            return new List<Tecnico>(Result).ToArray();
        }
    }
}
