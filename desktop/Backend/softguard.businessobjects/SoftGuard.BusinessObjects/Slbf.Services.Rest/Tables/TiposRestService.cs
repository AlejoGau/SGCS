...
namespace SoftGuard.EnterpriseServices.Rest
{
    ...
    public class TiposRestService
    {
        ...
        public Tipos[] GetCollection()
        {
            TiposManager Manager = new TiposManager(ConfigurationManager.ConnectionStrings["Slbf"].ConnectionString);
            // me fijo que tenga token valido.
            var token = Slbf.Security.UserService.GetId();
            IEnumerable<Tipos> Result = Manager.GetAll();
            
            return new List<Tipos>(Result).ToArray();
        }
