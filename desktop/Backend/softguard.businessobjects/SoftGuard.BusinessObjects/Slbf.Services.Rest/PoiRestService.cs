...
// Type: SoftGuard.BusinessObjects.Rest.PoiRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  public class PoiRestService
  {
    ...
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (SimplePoi), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Page, string Start, string Limit, string Sort, string Group, string Filter)
    {
      ...
      Poi currentObject = this.GetCurrentObject(0);
      ...
      foreach (DataRow row in (InternalDataCollectionBase) dataByFilter.Rows)
      {
        ...
        foreach (PropertyInfo property in simplePoi.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByFilter.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) simplePoi, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) simplePoi);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = TotalRows };
    }

    private Poi GetCurrentObject(int Id = 0)
    {
      Poi poi = ObjectFactoryService.Create<Poi>();
      if (Id != 0)
        poi.Load(Id);
      return poi;
    }
    ...
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "Poi");
    }
    ...
    public SimpleBaseObject Create(SimplePoi instance)
    {
      instance.Id = 0;
      Poi currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, SimplePoi instance)
    {
      Poi currentObject = this.GetCurrentObject(int.Parse(id));
