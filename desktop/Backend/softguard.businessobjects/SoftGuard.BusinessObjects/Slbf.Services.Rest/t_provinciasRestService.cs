...
// Type: SoftGuard.BusinessObjects.Rest.t_provinciasRestService
...
namespace SoftGuard.BusinessObjects.Rest
{
  ...
  [DataContractFormat]
  public class t_provinciasRestService
  {
    private string _CurrentObjectName;

    public static IEnumerable<Type> GetKnownTypes(ICustomAttributeProvider provider)
    {
      return (IEnumerable<Type>) new List<Type>() { typeof (Simplet_provincias), typeof (SimpleMembershipUser), typeof (SimpleUserAccount), typeof (SimpleMenu), typeof (SimplePermission), typeof (SimpleRole), typeof (SimpleTaxonomy), typeof (SimpleMenuItem), typeof (SimpleGroups), typeof (SimpleGroups), typeof (SimpleMetaData) };
    }
    ...
    public PagedOutput GetCollection(string Page, string Start, string Limit, string Sort, string Group, string Filter)
    {
      ...
      t_provincias currentObject = this.GetCurrentObject(0);
      ...
      foreach (DataRow row in (InternalDataCollectionBase) dataByFilter.Rows)
      {
        ...
        foreach (PropertyInfo property in simpletProvincias.GetType().GetProperties())
        {
          DataColumn column;
          if ((column = dataByFilter.Columns[property.Name]) != null && row[column] != DBNull.Value)
            property.SetValue((object) simpletProvincias, row[column], (object[]) null);
        }
        simpleBaseObjectList.Add((SimpleBaseObject) simpletProvincias);
      }
      return new PagedOutput() { rows = (IEnumerable<object>) simpleBaseObjectList, total = TotalRows };
    }

    private t_provincias GetCurrentObject(int Id = 0)
    {
      t_provincias tProvincias = ObjectFactoryService.Create<t_provincias>();
      if (Id != 0)
        tProvincias.Load(Id);
      return tProvincias;
    }
    ...
    {
      return this._CurrentObjectName ?? (this._CurrentObjectName = "t_provincias");
    }
    ...
    public SimpleBaseObject Create(Simplet_provincias instance)
    {
      instance.Id = 0;
      t_provincias currentObject = this.GetCurrentObject(0);
    ...
    public SimpleBaseObject Update(string id, Simplet_provincias instance)
    {
      t_provincias currentObject = this.GetCurrentObject(int.Parse(id));
