...
// Type: SoftGuard.BusinessObjects.Spet_TimeZone
...
{
  public abstract class Spet_TimeZone : ISpecialization, ICanCopyProperties
  {
    private bool _AutoCommit = false;
    protected Dalt_TimeZone _DalObject;
    ...
    public string ttz_cTitle
    {
      get
      {
        return this._DalObject.ttz_cTitle;
      }
      set
      {
        this._DalObject.ttz_cTitle = value;
      }
    }
    ...
    public Decimal ttz_nOffSet
    {
      get
      {
        return this._DalObject.ttz_nOffSet;
      }
      set
      {
        this._DalObject.ttz_nOffSet = value;
      }
    }
    ...
    public Spet_TimeZone(SqlHelper SqlConfig, int UserId)
    {
      this.InitClass(SqlConfig, UserId);
      this._Taxonomies.Load(this.Security.UserId);
    }

    public Spet_TimeZone(SqlHelper SqlConfig, int UserId, int Id)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Id);
    }

    public Spet_TimeZone(SqlHelper SqlConfig, int UserId, Simplet_TimeZone Simple)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Simple);
    }
    ...
    public virtual void Load(Simplet_TimeZone Simple)
    {
    ...
    public virtual void Delete()
    {
      if (this._Relations.Count != 0)
        throw new RuntimeException("The t_TimeZone has dependencies.");
      if (this._AutoCommit)
        this.BeginTran();
      try
      {
    ...
    public Dalt_TimeZone GetDalObject()
    {
    ...
    public IEnumerable<Simplet_TimeZone> GetByParent(string ObjectType, int ObjectId)
    {
      return this._DalObject.GetByParent(ObjectType, ObjectId);
    }

    public IEnumerable<Simplet_TimeZone> GetByChild(string ObjectType, int ObjectId)
    {
    ...
    public BaseObject NewChild(ISpecialization Child, Slbf.ValueCollection Values)
    {
      ...
      if (this._DalObject.Id == 0)
        throw new InvalidObjectException(this.GetObject(), "The t_TimeZone is null.");
    ...
    {
      this._DalObject = new Dalt_TimeZone(SqlConfig, UserId);
    ...
    {
      t_TimeZone tTimeZone = (t_TimeZone) Object;
      tTimeZone.Name = this.Name;
      tTimeZone.ttz_cTitle = this.ttz_cTitle;
      tTimeZone.ttz_nOffSet = this.ttz_nOffSet;
    }
