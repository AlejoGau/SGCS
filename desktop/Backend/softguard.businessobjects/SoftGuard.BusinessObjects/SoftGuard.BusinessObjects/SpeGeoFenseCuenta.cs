...
// Type: SoftGuard.BusinessObjects.SpeGeoFenseCuenta
...
{
  public abstract class SpeGeoFenseCuenta : ISpecialization, ICanCopyProperties
  {
    private bool _AutoCommit = false;
    protected DalGeoFenseCuenta _DalObject;
    ...
    public int GeoFenseId
    {
      get
      {
        return this._DalObject.GeoFenseId;
      }
      set
      {
        this._DalObject.GeoFenseId = value;
      }
    }
    ...
    public SpeGeoFenseCuenta(SqlHelper SqlConfig, int UserId)
    {
      this.InitClass(SqlConfig, UserId);
      this._Taxonomies.Load(this.Security.UserId);
    }

    public SpeGeoFenseCuenta(SqlHelper SqlConfig, int UserId, int Id)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Id);
    }

    public SpeGeoFenseCuenta(SqlHelper SqlConfig, int UserId, SimpleGeoFenseCuenta Simple)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Simple);
    }
    ...
    public virtual void Load(SimpleGeoFenseCuenta Simple)
    {
    ...
    public virtual void Delete()
    {
      if (this._Relations.Count != 0)
        throw new RuntimeException("The GeoFenseCuenta has dependencies.");
      if (this._AutoCommit)
        this.BeginTran();
      try
      {
    ...
    public DalGeoFenseCuenta GetDalObject()
    {
    ...
    public BaseObject NewChild(ISpecialization Child, Slbf.ValueCollection Values)
    {
      ...
      if (this._DalObject.Id == 0)
        throw new InvalidObjectException(this.GetObject(), "The GeoFenseCuenta is null.");
    ...
    {
      this._DalObject = new DalGeoFenseCuenta(SqlConfig, UserId);
    ...
    {
      GeoFenseCuenta geoFenseCuenta = (GeoFenseCuenta) Object;
      geoFenseCuenta.Name = this.Name;
      geoFenseCuenta.GeoFenseId = this.GeoFenseId;
      geoFenseCuenta.CuentaId = this.CuentaId;
    }
