...
// Type: SoftGuard.BusinessObjects.Spet_teclados
...
{
  public abstract class Spet_teclados : ISpecialization, ICanCopyProperties
  {
    private bool _AutoCommit = false;
    protected Dalt_teclados _DalObject;
    ...
    public string tec_cdescripcion
    {
      get
      {
        return this._DalObject.tec_cdescripcion;
      }
      set
      {
        this._DalObject.tec_cdescripcion = value;
      }
    }
    ...
    public string tec_cobservacion
    {
      get
      {
        return this._DalObject.tec_cobservacion;
      }
      set
      {
        this._DalObject.tec_cobservacion = value;
      }
    }
    ...
    public Spet_teclados(SqlHelper SqlConfig, int UserId)
    {
      this.InitClass(SqlConfig, UserId);
      this._Taxonomies.Load(this.Security.UserId);
    }

    public Spet_teclados(SqlHelper SqlConfig, int UserId, int Id)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Id);
    }

    public Spet_teclados(SqlHelper SqlConfig, int UserId, Simplet_teclados Simple)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Simple);
    }
    ...
    public virtual void Load(Simplet_teclados Simple)
    {
    ...
    public virtual void Delete()
    {
      if (this._Relations.Count != 0)
        throw new RuntimeException("The t_teclados has dependencies.");
      if (this._AutoCommit)
        this.BeginTran();
      try
      {
    ...
    public Dalt_teclados GetDalObject()
    {
    ...
    public IEnumerable<Simplet_teclados> GetByParent(string ObjectType, int ObjectId)
    {
      return this._DalObject.GetByParent(ObjectType, ObjectId);
    }

    public IEnumerable<Simplet_teclados> GetByChild(string ObjectType, int ObjectId)
    {
    ...
    public BaseObject NewChild(ISpecialization Child, Slbf.ValueCollection Values)
    {
      ...
      if (this._DalObject.Id == 0)
        throw new InvalidObjectException(this.GetObject(), "The t_teclados is null.");
    ...
    {
      this._DalObject = new Dalt_teclados(SqlConfig, UserId);
    ...
    {
      t_teclados tTeclados = (t_teclados) Object;
      tTeclados.Name = this.Name;
      tTeclados.tec_cdescripcion = this.tec_cdescripcion;
      tTeclados.tec_cobservacion = this.tec_cobservacion;
    }
