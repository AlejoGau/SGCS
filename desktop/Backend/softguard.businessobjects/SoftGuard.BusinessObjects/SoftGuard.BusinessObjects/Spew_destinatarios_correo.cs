...
// Type: SoftGuard.BusinessObjects.Spew_destinatarios_correo
...
{
  public abstract class Spew_destinatarios_correo : ISpecialization, ICanCopyProperties
  {
    private bool _AutoCommit = false;
    protected Dalw_destinatarios_correo _DalObject;
    ...
    public string destino
    {
      get
      {
        return this._DalObject.destino;
      }
      set
      {
        this._DalObject.destino = value;
      }
    }
    ...
    public string email_destino
    {
      get
      {
        return this._DalObject.email_destino;
      }
      set
      {
        this._DalObject.email_destino = value;
      }
    }
    ...
    public Spew_destinatarios_correo(SqlHelper SqlConfig, int UserId)
    {
      this.InitClass(SqlConfig, UserId);
      this._Taxonomies.Load(this.Security.UserId);
    }

    public Spew_destinatarios_correo(SqlHelper SqlConfig, int UserId, int Id)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Id);
    }

    public Spew_destinatarios_correo(SqlHelper SqlConfig, int UserId, Simplew_destinatarios_correo Simple)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Simple);
    }
    ...
    public virtual void Load(Simplew_destinatarios_correo Simple)
    {
    ...
    public virtual void Delete()
    {
      if (this._Relations.Count != 0)
        throw new RuntimeException("The w_destinatarios_correo has dependencies.");
      if (this._AutoCommit)
        this.BeginTran();
      try
      {
    ...
    public Dalw_destinatarios_correo GetDalObject()
    {
    ...
    public IEnumerable<Simplew_destinatarios_correo> GetByParent(string ObjectType, int ObjectId)
    {
      return this._DalObject.GetByParent(ObjectType, ObjectId);
    }

    public IEnumerable<Simplew_destinatarios_correo> GetByChild(string ObjectType, int ObjectId)
    {
    ...
    public BaseObject NewChild(ISpecialization Child, Slbf.ValueCollection Values)
    {
      ...
      if (this._DalObject.Id == 0)
        throw new InvalidObjectException(this.GetObject(), "The w_destinatarios_correo is null.");
    ...
    {
      this._DalObject = new Dalw_destinatarios_correo(SqlConfig, UserId);
    ...
    {
      w_destinatarios_correo destinatariosCorreo = (w_destinatarios_correo) Object;
