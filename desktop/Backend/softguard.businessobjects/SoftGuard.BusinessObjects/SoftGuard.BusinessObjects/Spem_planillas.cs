...
// Type: SoftGuard.BusinessObjects.Spem_planillas
...
{
  public abstract class Spem_planillas : ISpecialization, ICanCopyProperties
  {
    private bool _AutoCommit = false;
    protected Dalm_planillas _DalObject;
    ...
    public string pla_cDescripcion
    {
      get
      {
        return this._DalObject.pla_cDescripcion;
      }
      set
      {
        this._DalObject.pla_cDescripcion = value;
      }
    }
    ...
    public string pla_cNombreTabla
    {
      get
      {
        return this._DalObject.pla_cNombreTabla;
      }
      set
      {
        this._DalObject.pla_cNombreTabla = value;
      }
    }
    ...
    public Spem_planillas(SqlHelper SqlConfig, int UserId)
    {
      this.InitClass(SqlConfig, UserId);
      this._Taxonomies.Load(this.Security.UserId);
    }

    public Spem_planillas(SqlHelper SqlConfig, int UserId, int Id)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Id);
    }

    public Spem_planillas(SqlHelper SqlConfig, int UserId, Simplem_planillas Simple)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Simple);
    }
    ...
    public virtual void Load(Simplem_planillas Simple)
    {
    ...
    public virtual void Delete()
    {
      if (this._Relations.Count != 0)
        throw new RuntimeException("The m_planillas has dependencies.");
      if (this._AutoCommit)
        this.BeginTran();
      try
      {
    ...
    public Dalm_planillas GetDalObject()
    {
    ...
    public IEnumerable<Simplem_planillas> GetByParent(string ObjectType, int ObjectId)
    {
      return this._DalObject.GetByParent(ObjectType, ObjectId);
    }

    public IEnumerable<Simplem_planillas> GetByChild(string ObjectType, int ObjectId)
    {
    ...
    public BaseObject NewChild(ISpecialization Child, Slbf.ValueCollection Values)
    {
      ...
      if (this._DalObject.Id == 0)
        throw new InvalidObjectException(this.GetObject(), "The m_planillas is null.");
    ...
    {
      this._DalObject = new Dalm_planillas(SqlConfig, UserId);
    ...
    {
      m_planillas mPlanillas = (m_planillas) Object;
      mPlanillas.Name = this.Name;
      mPlanillas.pla_cDescripcion = this.pla_cDescripcion;
      mPlanillas.pla_cNombreTabla = this.pla_cNombreTabla;
    }
