...
// Type: SoftGuard.BusinessObjects.Spew_cuentas_x_usuario
...
{
  public abstract class Spew_cuentas_x_usuario : ISpecialization, ICanCopyProperties
  {
    private bool _AutoCommit = false;
    protected Dalw_cuentas_x_usuario _DalObject;
    ...
    public int id_usuario
    {
      get
      {
        return this._DalObject.id_usuario;
      }
      set
      {
        this._DalObject.id_usuario = value;
      }
    }
    ...
    public int cue_iid
    {
      get
      {
        return this._DalObject.cue_iid;
      }
      set
      {
        this._DalObject.cue_iid = value;
      }
    }
    ...
    public Spew_cuentas_x_usuario(SqlHelper SqlConfig, int UserId)
    {
      this.InitClass(SqlConfig, UserId);
      this._Taxonomies.Load(this.Security.UserId);
    }

    public Spew_cuentas_x_usuario(SqlHelper SqlConfig, int UserId, int Id)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Id);
    }

    public Spew_cuentas_x_usuario(SqlHelper SqlConfig, int UserId, Simplew_cuentas_x_usuario Simple)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Simple);
    }
    ...
    public virtual void Load(Simplew_cuentas_x_usuario Simple)
    {
    ...
    public virtual void Delete()
    {
      if (this._Relations.Count != 0)
        throw new RuntimeException("The w_cuentas_x_usuario has dependencies.");
      if (this._AutoCommit)
        this.BeginTran();
      try
      {
    ...
    public Dalw_cuentas_x_usuario GetDalObject()
    {
    ...
    public IEnumerable<Simplew_cuentas_x_usuario> GetByParent(string ObjectType, int ObjectId)
    {
      return this._DalObject.GetByParent(ObjectType, ObjectId);
    }

    public IEnumerable<Simplew_cuentas_x_usuario> GetByChild(string ObjectType, int ObjectId)
    {
    ...
    public BaseObject NewChild(ISpecialization Child, Slbf.ValueCollection Values)
    {
      ...
      if (this._DalObject.Id == 0)
        throw new InvalidObjectException(this.GetObject(), "The w_cuentas_x_usuario is null.");
    ...
    {
      this._DalObject = new Dalw_cuentas_x_usuario(SqlConfig, UserId);
    ...
    {
      w_cuentas_x_usuario wCuentasXUsuario = (w_cuentas_x_usuario) Object;
      wCuentasXUsuario.Name = this.Name;
      wCuentasXUsuario.id_usuario = this.id_usuario;
      wCuentasXUsuario.cue_iid = this.cue_iid;
    }
