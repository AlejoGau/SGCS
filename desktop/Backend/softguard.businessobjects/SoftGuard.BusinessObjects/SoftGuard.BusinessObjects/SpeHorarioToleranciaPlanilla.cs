...
// Type: SoftGuard.BusinessObjects.SpeHorarioToleranciaPlanilla
...
{
  public abstract class SpeHorarioToleranciaPlanilla : ISpecialization, ICanCopyProperties
  {
    private bool _AutoCommit = false;
    protected DalHorarioToleranciaPlanilla _DalObject;
    ...
    public int tol_iid
    {
      get
      {
        return this._DalObject.tol_iid;
      }
      set
      {
        this._DalObject.tol_iid = value;
      }
    }
    ...
    public SpeHorarioToleranciaPlanilla(SqlHelper SqlConfig, int UserId)
    {
      this.InitClass(SqlConfig, UserId);
      this._Taxonomies.Load(this.Security.UserId);
    }

    public SpeHorarioToleranciaPlanilla(SqlHelper SqlConfig, int UserId, int Id)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Id);
    }

    public SpeHorarioToleranciaPlanilla(SqlHelper SqlConfig, int UserId, SimpleHorarioToleranciaPlanilla Simple)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Simple);
    }
    ...
    public virtual void Load(SimpleHorarioToleranciaPlanilla Simple)
    {
    ...
    public virtual void Delete()
    {
      if (this._Relations.Count != 0)
        throw new RuntimeException("The HorarioToleranciaPlanilla has dependencies.");
      if (this._AutoCommit)
        this.BeginTran();
      try
      {
    ...
    public DalHorarioToleranciaPlanilla GetDalObject()
    {
    ...
    public BaseObject NewChild(ISpecialization Child, Slbf.ValueCollection Values)
    {
      ...
      if (this._DalObject.Id == 0)
        throw new InvalidObjectException(this.GetObject(), "The HorarioToleranciaPlanilla is null.");
    ...
    {
      this._DalObject = new DalHorarioToleranciaPlanilla(SqlConfig, UserId);
    ...
    {
      HorarioToleranciaPlanilla toleranciaPlanilla = (HorarioToleranciaPlanilla) Object;
