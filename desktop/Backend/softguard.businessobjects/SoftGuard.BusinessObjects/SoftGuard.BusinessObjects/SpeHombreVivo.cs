...
// Type: SoftGuard.BusinessObjects.SpeHombreVivo
...
{
  public abstract class SpeHombreVivo : ISpecialization, ICanCopyProperties
  {
    private bool _AutoCommit = false;
    protected DalHombreVivo _DalObject;
    ...
    public DateTime? fecha
    {
      get
      {
        return this._DalObject.fecha;
      }
      set
      {
        this._DalObject.fecha = value;
      }
    }
    ...
    public int proximo
    {
      get
      {
        return this._DalObject.proximo;
      }
      set
      {
        this._DalObject.proximo = value;
      }
    }
    ...
    public int idscheduler
    {
      get
      {
        return this._DalObject.idscheduler;
      }
      set
      {
        this._DalObject.idscheduler = value;
      }
    }
    ...
    public string imei
    {
      get
      {
        return this._DalObject.imei;
      }
      set
      {
        this._DalObject.imei = value;
      }
    }
    ...
    public int idcuenta
    {
      get
      {
        return this._DalObject.idcuenta;
      }
      set
      {
        this._DalObject.idcuenta = value;
      }
    }
    ...
    public SpeHombreVivo(SqlHelper SqlConfig, int UserId)
    {
      this.InitClass(SqlConfig, UserId);
      this._Taxonomies.Load(this.Security.UserId);
    }

    public SpeHombreVivo(SqlHelper SqlConfig, int UserId, int Id)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Id);
    }

    public SpeHombreVivo(SqlHelper SqlConfig, int UserId, SimpleHombreVivo Simple)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Simple);
    }
    ...
    public virtual void Load(SimpleHombreVivo Simple)
    {
    ...
    public virtual void Delete()
    {
      if (this._Relations.Count != 0)
        throw new RuntimeException("The HombreVivo has dependencies.");
      if (this._AutoCommit)
        this.BeginTran();
      try
      {
    ...
    public DalHombreVivo GetDalObject()
    {
    ...
    public BaseObject NewChild(ISpecialization Child, Slbf.ValueCollection Values)
    {
      ...
      if (this._DalObject.Id == 0)
        throw new InvalidObjectException(this.GetObject(), "The HombreVivo is null.");
    ...
    {
      this._DalObject = new DalHombreVivo(SqlConfig, UserId);
    ...
    {
      HombreVivo hombreVivo = (HombreVivo) Object;
      hombreVivo.Name = this.Name;
      hombreVivo.fecha = this.fecha;
      hombreVivo.proximo = this.proximo;
      hombreVivo.idscheduler = this.idscheduler;
      hombreVivo.imei = this.imei;
      hombreVivo.idcuenta = this.idcuenta;
    }
