...
// Type: SoftGuard.BusinessObjects.GeoFenseCuenta
...
{
  public class GeoFenseCuenta : SpeGeoFenseCuenta
  {
    public GeoFenseCuenta(SqlHelper SqlConfig, int UserId)
      : base(SqlConfig, UserId)
    {
    }

    public GeoFenseCuenta(SqlHelper SqlConfig, string Token)
      : base(SqlConfig, UserService.GetId(Token))
    {
      this._DalObject.Token = Token ?? UserService.GetContextToken((object) null);
    }

    public GeoFenseCuenta(SqlHelper SqlConfig, int UserId, int Id)
      : base(SqlConfig, UserId, Id)
    {
    }

    public GeoFenseCuenta(SqlHelper SqlConfig, int UserId, SimpleGeoFenseCuenta Simple)
