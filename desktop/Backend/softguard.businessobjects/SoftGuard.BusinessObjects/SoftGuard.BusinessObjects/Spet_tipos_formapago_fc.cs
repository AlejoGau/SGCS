...
// Type: SoftGuard.BusinessObjects.Spet_tipos_formapago_fc
...
{
  public abstract class Spet_tipos_formapago_fc : ISpecialization, ICanCopyProperties
  {
    private bool _AutoCommit = false;
    protected Dalt_tipos_formapago_fc _DalObject;
    ...
    public string tfp_ccodigo
    {
      get
      {
        return this._DalObject.tfp_ccodigo;
      }
      set
      {
        this._DalObject.tfp_ccodigo = value;
      }
    }
    ...
    public string tfp_cdescripcion
    {
      get
      {
        return this._DalObject.tfp_cdescripcion;
      }
      set
      {
        this._DalObject.tfp_cdescripcion = value;
      }
    }
    ...
    public Spet_tipos_formapago_fc(SqlHelper SqlConfig, int UserId)
    {
      this.InitClass(SqlConfig, UserId);
      this._Taxonomies.Load(this.Security.UserId);
    }

    public Spet_tipos_formapago_fc(SqlHelper SqlConfig, int UserId, int Id)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Id);
    }

    public Spet_tipos_formapago_fc(SqlHelper SqlConfig, int UserId, Simplet_tipos_formapago_fc Simple)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Simple);
    }
    ...
    public virtual void Load(Simplet_tipos_formapago_fc Simple)
    {
    ...
    public virtual void Delete()
    {
      if (this._Relations.Count != 0)
        throw new RuntimeException("The t_tipos_formapago_fc has dependencies.");
      if (this._AutoCommit)
        this.BeginTran();
      try
      {
    ...
    public Dalt_tipos_formapago_fc GetDalObject()
    {
    ...
    public IEnumerable<Simplet_tipos_formapago_fc> GetByParent(string ObjectType, int ObjectId)
    {
      return this._DalObject.GetByParent(ObjectType, ObjectId);
    }

    public IEnumerable<Simplet_tipos_formapago_fc> GetByChild(string ObjectType, int ObjectId)
    {
    ...
    public BaseObject NewChild(ISpecialization Child, Slbf.ValueCollection Values)
    {
      ...
      if (this._DalObject.Id == 0)
        throw new InvalidObjectException(this.GetObject(), "The t_tipos_formapago_fc is null.");
    ...
    {
      this._DalObject = new Dalt_tipos_formapago_fc(SqlConfig, UserId);
    ...
    {
      t_tipos_formapago_fc tiposFormapagoFc = (t_tipos_formapago_fc) Object;
