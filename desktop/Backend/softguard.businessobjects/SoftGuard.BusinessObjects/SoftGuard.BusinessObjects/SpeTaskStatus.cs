...
// Type: SoftGuard.BusinessObjects.SpeTaskStatus
...
{
  public abstract class SpeTaskStatus : ISpecialization, ICanCopyProperties
  {
    private bool _AutoCommit = false;
    protected DalTaskStatus _DalObject;
    ...
    public DateTime? LastExecutionDate
    {
      get
      {
        return this._DalObject.LastExecutionDate;
      }
      set
      {
        this._DalObject.LastExecutionDate = value;
      }
    }
    ...
    public int Repetition
    {
      get
      {
        return this._DalObject.Repetition;
      }
      set
      {
        this._DalObject.Repetition = value;
      }
    }
    ...
    public SpeTaskStatus(SqlHelper SqlConfig, int UserId)
    {
      this.InitClass(SqlConfig, UserId);
      this._Taxonomies.Load(this.Security.UserId);
    }

    public SpeTaskStatus(SqlHelper SqlConfig, int UserId, int Id)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Id);
    }

    public SpeTaskStatus(SqlHelper SqlConfig, int UserId, SimpleTaskStatus Simple)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Simple);
    }
    ...
    public virtual void Load(SimpleTaskStatus Simple)
    {
    ...
    public virtual void Delete()
    {
      if (this._Relations.Count != 0)
        throw new RuntimeException("The TaskStatus has dependencies.");
      if (this._AutoCommit)
        this.BeginTran();
      try
      {
    ...
    public DalTaskStatus GetDalObject()
    {
    ...
    public IEnumerable<SimpleTaskStatus> GetByParent(string ObjectType, int ObjectId)
    {
      return this._DalObject.GetByParent(ObjectType, ObjectId);
    }

    public IEnumerable<SimpleTaskStatus> GetByChild(string ObjectType, int ObjectId)
    {
    ...
    public BaseObject NewChild(ISpecialization Child, Slbf.ValueCollection Values)
    {
      ...
      if (this._DalObject.Id == 0)
        throw new InvalidObjectException(this.GetObject(), "The TaskStatus is null.");
    ...
    {
      this._DalObject = new DalTaskStatus(SqlConfig, UserId);
    ...
    public void CopyPropertiesTo(ISpecialization Object)
    {
      ...
      taskStatus.LastExecutionDate = this.LastExecutionDate;
      taskStatus.Repetition = this.Repetition;
    }
