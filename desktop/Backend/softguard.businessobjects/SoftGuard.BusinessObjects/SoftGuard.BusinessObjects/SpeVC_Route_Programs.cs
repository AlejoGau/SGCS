...
// Type: SoftGuard.BusinessObjects.SpeVC_Route_Programs
...
{
  public abstract class SpeVC_Route_Programs : ISpecialization, ICanCopyProperties
  {
    private bool _AutoCommit = false;
    protected DalVC_Route_Programs _DalObject;
    ...
    public int routeId
    {
      get
      {
        return this._DalObject.routeId;
      }
      set
      {
        this._DalObject.routeId = value;
      }
    }
    ...
    public SpeVC_Route_Programs(SqlHelper SqlConfig, int UserId)
    {
      this.InitClass(SqlConfig, UserId);
      this._Taxonomies.Load(this.Security.UserId);
    }

    public SpeVC_Route_Programs(SqlHelper SqlConfig, int UserId, int Id)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Id);
    }

    public SpeVC_Route_Programs(SqlHelper SqlConfig, int UserId, SimpleVC_Route_Programs Simple)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Simple);
    }
    ...
    public virtual void Load(SimpleVC_Route_Programs Simple)
    {
    ...
    public virtual void Delete()
    {
      if (this._Relations.Count != 0)
        throw new RuntimeException("The VC_Route_Programs has dependencies.");
      if (this._AutoCommit)
        this.BeginTran();
      try
      {
    ...
    public DalVC_Route_Programs GetDalObject()
    {
    ...
    public BaseObject NewChild(ISpecialization Child, Slbf.ValueCollection Values)
    {
      ...
      if (this._DalObject.Id == 0)
        throw new InvalidObjectException(this.GetObject(), "The VC_Route_Programs is null.");
    ...
    {
      this._DalObject = new DalVC_Route_Programs(SqlConfig, UserId);
    ...
    {
      VC_Route_Programs vcRoutePrograms = (VC_Route_Programs) Object;
      vcRoutePrograms.Name = this.Name;
      vcRoutePrograms.routeId = this.routeId;
      vcRoutePrograms.programtype = this.programtype;
      vcRoutePrograms.starthour = this.starthour;
      vcRoutePrograms.startminutes = this.startminutes;
      vcRoutePrograms.dayofweek = this.dayofweek;
      vcRoutePrograms.dayofmonth = this.dayofmonth;
    }
