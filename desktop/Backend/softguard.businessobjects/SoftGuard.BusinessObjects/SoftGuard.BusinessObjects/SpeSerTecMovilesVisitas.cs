...
// Type: SoftGuard.BusinessObjects.SpeSerTecMovilesVisitas
...
{
  public abstract class SpeSerTecMovilesVisitas : ISpecialization, ICanCopyProperties
  {
    private bool _AutoCommit = false;
    protected DalSerTecMovilesVisitas _DalObject;
    ...
    public int smv_iMovil
    {
      get
      {
        return this._DalObject.smv_iMovil;
      }
      set
      {
        this._DalObject.smv_iMovil = value;
      }
    }
    ...
    public int smv_iVisita
    {
      get
      {
        return this._DalObject.smv_iVisita;
      }
      set
      {
        this._DalObject.smv_iVisita = value;
      }
    }
    ...
    public SpeSerTecMovilesVisitas(SqlHelper SqlConfig, int UserId)
    {
      this.InitClass(SqlConfig, UserId);
      this._Taxonomies.Load(this.Security.UserId);
    }

    public SpeSerTecMovilesVisitas(SqlHelper SqlConfig, int UserId, int Id)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Id);
    }

    public SpeSerTecMovilesVisitas(SqlHelper SqlConfig, int UserId, SimpleSerTecMovilesVisitas Simple)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Simple);
    }
    ...
    public virtual void Load(SimpleSerTecMovilesVisitas Simple)
    {
    ...
    public virtual void Delete()
    {
      if (this._Relations.Count != 0)
        throw new RuntimeException("The SerTecMovilesVisitas has dependencies.");
      if (this._AutoCommit)
        this.BeginTran();
      try
      {
    ...
    public DalSerTecMovilesVisitas GetDalObject()
    {
    ...
    public IEnumerable<SimpleSerTecMovilesVisitas> GetByParent(string ObjectType, int ObjectId)
    {
      return this._DalObject.GetByParent(ObjectType, ObjectId);
    }

    public IEnumerable<SimpleSerTecMovilesVisitas> GetByChild(string ObjectType, int ObjectId)
    {
    ...
    public BaseObject NewChild(ISpecialization Child, Slbf.ValueCollection Values)
    {
      ...
      if (this._DalObject.Id == 0)
        throw new InvalidObjectException(this.GetObject(), "The SerTecMovilesVisitas is null.");
    ...
    {
      this._DalObject = new DalSerTecMovilesVisitas(SqlConfig, UserId);
    ...
    {
      SerTecMovilesVisitas tecMovilesVisitas = (SerTecMovilesVisitas) Object;
