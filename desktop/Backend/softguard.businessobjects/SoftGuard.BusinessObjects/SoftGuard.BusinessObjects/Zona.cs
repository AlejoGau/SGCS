...
// Type: SoftGuard.BusinessObjects.Zona
...
{
  public class Zona : SpeZona
  {
    public Zona(SqlHelper SqlConfig, int UserId)
      : base(SqlConfig, UserId)
    {
    }

    public Zona(SqlHelper SqlConfig, string Token)
      : base(SqlConfig, UserService.GetId(Token))
    {
      this._DalObject.Token = Token ?? UserService.GetContextToken((object) null);
    }

    public Zona(SqlHelper SqlConfig, int UserId, int Id)
      : base(SqlConfig, UserId, Id)
    {
    }

    public Zona(SqlHelper SqlConfig, int UserId, SimpleZona Simple)
