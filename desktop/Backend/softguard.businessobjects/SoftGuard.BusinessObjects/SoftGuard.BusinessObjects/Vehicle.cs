...
// Type: SoftGuard.BussinesObjects.Vehicle
...
{
  public class Vehicle : SpeVehicle
  {
    public Vehicle(SqlHelper SqlConfig, int UserId)
      : base(SqlConfig, UserId)
    {
    }

    public Vehicle(SqlHelper SqlConfig, string Token)
      : base(SqlConfig, UserService.GetId(Token))
    {
      this._DalObject.Token = Token ?? UserService.GetContextToken((object) null);
    }

    public Vehicle(SqlHelper SqlConfig, int UserId, int Id)
      : base(SqlConfig, UserId, Id)
    {
    }

    public Vehicle(SqlHelper SqlConfig, int UserId, SimpleVehicle Simple)
