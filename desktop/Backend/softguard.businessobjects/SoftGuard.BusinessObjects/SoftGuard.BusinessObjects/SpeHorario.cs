...
// Type: SoftGuard.BusinessObjects.SpeHorario
...
{
  public abstract class SpeHorario : ISpecialization, ICanCopyProperties
  {
    private bool _AutoCommit = false;
    protected DalHorario _DalObject;
    ...
    public int hor_iidcuenta
    {
      get
      {
        return this._DalObject.hor_iidcuenta;
      }
      set
      {
        this._DalObject.hor_iidcuenta = value;
      }
    }
    ...
    public SpeHorario(SqlHelper SqlConfig, int UserId)
    {
      this.InitClass(SqlConfig, UserId);
      this._Taxonomies.Load(this.Security.UserId);
    }

    public SpeHorario(SqlHelper SqlConfig, int UserId, int Id)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Id);
    }

    public SpeHorario(SqlHelper SqlConfig, int UserId, SimpleHorario Simple)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Simple);
    }
    ...
    public virtual void Load(SimpleHorario Simple)
    {
    ...
    public virtual void Delete()
    {
      if (this._Relations.Count != 0)
        throw new RuntimeException("The Horario has dependencies.");
      if (this._AutoCommit)
        this.BeginTran();
      try
      {
    ...
    public DalHorario GetDalObject()
    {
    ...
    public BaseObject NewChild(ISpecialization Child, Slbf.ValueCollection Values)
    {
      ...
      if (this._DalObject.Id == 0)
        throw new InvalidObjectException(this.GetObject(), "The Horario is null.");
    ...
    {
      this._DalObject = new DalHorario(SqlConfig, UserId);
    ...
    public void CopyPropertiesTo(ISpecialization Object)
    {
      ...
      horario.Name = this.Name;
      horario.hor_iidcuenta = this.hor_iidcuenta;
      horario.hor_ndiaapertura = this.hor_ndiaapertura;
      horario.hor_choraapertura = this.hor_choraapertura;
      horario.hor_ndiacierre = this.hor_ndiacierre;
      horario.hor_choracierre = this.hor_choracierre;
    }
