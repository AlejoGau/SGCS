...
// Type: SoftGuard.BusinessObjects.SpeSerTecFormaViajeVisitas
...
{
  public abstract class SpeSerTecFormaViajeVisitas : ISpecialization, ICanCopyProperties
  {
    private bool _AutoCommit = false;
    protected DalSerTecFormaViajeVisitas _DalObject;
    ...
    public string sfv_cNombre
    {
      get
      {
        return this._DalObject.sfv_cNombre;
      }
      set
      {
        this._DalObject.sfv_cNombre = value;
      }
    }
    ...
    public SpeSerTecFormaViajeVisitas(SqlHelper SqlConfig, int UserId)
    {
      this.InitClass(SqlConfig, UserId);
      this._Taxonomies.Load(this.Security.UserId);
    }

    public SpeSerTecFormaViajeVisitas(SqlHelper SqlConfig, int UserId, int Id)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Id);
    }

    public SpeSerTecFormaViajeVisitas(SqlHelper SqlConfig, int UserId, SimpleSerTecFormaViajeVisitas Simple)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Simple);
    }
    ...
    public virtual void Load(SimpleSerTecFormaViajeVisitas Simple)
    {
    ...
    public virtual void Delete()
    {
      if (this._Relations.Count != 0)
        throw new RuntimeException("The SerTecFormaViajeVisitas has dependencies.");
      if (this._AutoCommit)
        this.BeginTran();
      try
      {
    ...
    public DalSerTecFormaViajeVisitas GetDalObject()
    {
    ...
    public BaseObject NewChild(ISpecialization Child, Slbf.ValueCollection Values)
    {
      ...
      if (this._DalObject.Id == 0)
        throw new InvalidObjectException(this.GetObject(), "The SerTecFormaViajeVisitas is null.");
    ...
    {
      this._DalObject = new DalSerTecFormaViajeVisitas(SqlConfig, UserId);
    ...
    {
      SerTecFormaViajeVisitas formaViajeVisitas = (SerTecFormaViajeVisitas) Object;
      formaViajeVisitas.Name = this.Name;
      formaViajeVisitas.sfv_cNombre = this.sfv_cNombre;
    }
