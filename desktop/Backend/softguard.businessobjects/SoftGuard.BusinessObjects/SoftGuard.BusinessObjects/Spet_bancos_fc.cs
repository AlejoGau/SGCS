...
// Type: SoftGuard.BusinessObjects.Spet_bancos_fc
...
{
  public abstract class Spet_bancos_fc : ISpecialization, ICanCopyProperties
  {
    private bool _AutoCommit = false;
    protected Dalt_bancos_fc _DalObject;
    ...
    public string bco_ccodigo
    {
      get
      {
        return this._DalObject.bco_ccodigo;
      }
      set
      {
        this._DalObject.bco_ccodigo = value;
      }
    }
    ...
    public string bco_cnombre
    {
      get
      {
        return this._DalObject.bco_cnombre;
      }
      set
      {
        this._DalObject.bco_cnombre = value;
      }
    }
    ...
    public Spet_bancos_fc(SqlHelper SqlConfig, int UserId)
    {
      this.InitClass(SqlConfig, UserId);
      this._Taxonomies.Load(this.Security.UserId);
    }

    public Spet_bancos_fc(SqlHelper SqlConfig, int UserId, int Id)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Id);
    }

    public Spet_bancos_fc(SqlHelper SqlConfig, int UserId, Simplet_bancos_fc Simple)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Simple);
    }
    ...
    public virtual void Load(Simplet_bancos_fc Simple)
    {
    ...
    public virtual void Delete()
    {
      if (this._Relations.Count != 0)
        throw new RuntimeException("The t_bancos_fc has dependencies.");
      if (this._AutoCommit)
        this.BeginTran();
      try
      {
    ...
    public Dalt_bancos_fc GetDalObject()
    {
    ...
    public IEnumerable<Simplet_bancos_fc> GetByParent(string ObjectType, int ObjectId)
    {
      return this._DalObject.GetByParent(ObjectType, ObjectId);
    }

    public IEnumerable<Simplet_bancos_fc> GetByChild(string ObjectType, int ObjectId)
    {
    ...
    public BaseObject NewChild(ISpecialization Child, Slbf.ValueCollection Values)
    {
      ...
      if (this._DalObject.Id == 0)
        throw new InvalidObjectException(this.GetObject(), "The t_bancos_fc is null.");
    ...
    {
      this._DalObject = new Dalt_bancos_fc(SqlConfig, UserId);
    ...
    {
      t_bancos_fc tBancosFc = (t_bancos_fc) Object;
      tBancosFc.Name = this.Name;
      tBancosFc.bco_ccodigo = this.bco_ccodigo;
      tBancosFc.bco_cnombre = this.bco_cnombre;
    }
