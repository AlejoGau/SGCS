...
// Type: SoftGuard.BusinessObjects.w_destinatarios_correo
...
{
  public class w_destinatarios_correo : Spew_destinatarios_correo
  {
    public w_destinatarios_correo(SqlHelper SqlConfig, int UserId)
      : base(SqlConfig, UserId)
    {
    }

    public w_destinatarios_correo(SqlHelper SqlConfig, string Token)
      : base(SqlConfig, UserService.GetId(Token))
    {
      this._DalObject.Token = Token ?? UserService.GetContextToken((object) null);
    }

    public w_destinatarios_correo(SqlHelper SqlConfig, int UserId, int Id)
      : base(SqlConfig, UserId, Id)
    {
    }

    public w_destinatarios_correo(SqlHelper SqlConfig, int UserId, Simplew_destinatarios_correo Simple)
