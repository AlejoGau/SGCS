...
// Type: SoftGuard.BusinessObjects.t_mailConnector
...
{
  public class t_mailConnector : Spet_mailConnector
  {
    public t_mailConnector(SqlHelper SqlConfig, int UserId)
      : base(SqlConfig, UserId)
    {
    }

    public t_mailConnector(SqlHelper SqlConfig, string Token)
      : base(SqlConfig, UserService.GetId(Token))
    {
      this._DalObject.Token = Token ?? UserService.GetContextToken((object) null);
    }

    public t_mailConnector(SqlHelper SqlConfig, int UserId, int Id)
      : base(SqlConfig, UserId, Id)
    {
    }

    public t_mailConnector(SqlHelper SqlConfig, int UserId, Simplet_mailConnector Simple)
