...
// Type: SoftGuard.BusinessObjects.SpeDeposito
...
{
  public abstract class SpeDeposito : ISpecialization, ICanCopyProperties
  {
    private bool _AutoCommit = false;
    protected DalDeposito _DalObject;
    ...
    public string dep_ccodigo
    {
      get
      {
        return this._DalObject.dep_ccodigo;
      }
      set
      {
        this._DalObject.dep_ccodigo = value;
      }
    }
    ...
    public string dep_cdescripcion
    {
      get
      {
        return this._DalObject.dep_cdescripcion;
      }
      set
      {
        this._DalObject.dep_cdescripcion = value;
      }
    }
    ...
    public SpeDeposito(SqlHelper SqlConfig, int UserId)
    {
      this.InitClass(SqlConfig, UserId);
      this._Taxonomies.Load(this.Security.UserId);
    }

    public SpeDeposito(SqlHelper SqlConfig, int UserId, int Id)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Id);
    }

    public SpeDeposito(SqlHelper SqlConfig, int UserId, SimpleDeposito Simple)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Simple);
    }
    ...
    public virtual void Load(SimpleDeposito Simple)
    {
    ...
    public virtual void Delete()
    {
      if (this._Relations.Count != 0)
        throw new RuntimeException("The Deposito has dependencies.");
      if (this._AutoCommit)
        this.BeginTran();
      try
      {
    ...
    public DalDeposito GetDalObject()
    {
    ...
    public IEnumerable<SimpleDeposito> GetByParent(string ObjectType, int ObjectId)
    {
      return this._DalObject.GetByParent(ObjectType, ObjectId);
    }

    public IEnumerable<SimpleDeposito> GetByChild(string ObjectType, int ObjectId)
    {
    ...
    public BaseObject NewChild(ISpecialization Child, Slbf.ValueCollection Values)
    {
      ...
      if (this._DalObject.Id == 0)
        throw new InvalidObjectException(this.GetObject(), "The Deposito is null.");
    ...
    {
      this._DalObject = new DalDeposito(SqlConfig, UserId);
    ...
    {
      Deposito deposito = (Deposito) Object;
      deposito.Name = this.Name;
      deposito.dep_ccodigo = this.dep_ccodigo;
      deposito.dep_cdescripcion = this.dep_cdescripcion;
    }
