...
// Type: SoftGuard.BusinessObjects.SpeGrupoCuentas
...
{
  public abstract class SpeGrupoCuentas : ISpecialization, ICanCopyProperties
  {
    private bool _AutoCommit = false;
    protected DalGrupoCuentas _DalObject;
    ...
    public string tgc_cdescripcion
    {
      get
      {
        return this._DalObject.tgc_cdescripcion;
      }
      set
      {
        this._DalObject.tgc_cdescripcion = value;
      }
    }
    ...
    public SpeGrupoCuentas(SqlHelper SqlConfig, int UserId)
    {
      this.InitClass(SqlConfig, UserId);
      this._Taxonomies.Load(this.Security.UserId);
    }

    public SpeGrupoCuentas(SqlHelper SqlConfig, int UserId, int Id)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Id);
    }

    public SpeGrupoCuentas(SqlHelper SqlConfig, int UserId, SimpleGrupoCuentas Simple)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Simple);
    }
    ...
    public virtual void Load(SimpleGrupoCuentas Simple)
    {
    ...
    public virtual void Delete()
    {
      if (this._Relations.Count != 0)
        throw new RuntimeException("The GrupoCuentas has dependencies.");
      if (this._AutoCommit)
        this.BeginTran();
      try
      {
    ...
    public DalGrupoCuentas GetDalObject()
    {
    ...
    public BaseObject NewChild(ISpecialization Child, Slbf.ValueCollection Values)
    {
      ...
      if (this._DalObject.Id == 0)
        throw new InvalidObjectException(this.GetObject(), "The GrupoCuentas is null.");
    ...
    {
      this._DalObject = new DalGrupoCuentas(SqlConfig, UserId);
    ...
    {
      GrupoCuentas grupoCuentas = (GrupoCuentas) Object;
      grupoCuentas.Name = this.Name;
      grupoCuentas.tgc_cdescripcion = this.tgc_cdescripcion;
    }
