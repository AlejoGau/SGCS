...
// Type: SoftGuard.BusinessObjects.Spep_recepcion_notas
...
{
  public abstract class Spep_recepcion_notas : ISpecialization, ICanCopyProperties
  {
    private bool _AutoCommit = false;
    protected Dalp_recepcion_notas _DalObject;
    ...
    public int rec_iidrecepcion
    {
      get
      {
        return this._DalObject.rec_iidrecepcion;
      }
      set
      {
        this._DalObject.rec_iidrecepcion = value;
      }
    }
    ...
    public int rec_itipo
    {
      get
      {
        return this._DalObject.rec_itipo;
      }
      set
      {
        this._DalObject.rec_itipo = value;
      }
    }
    ...
    public string rec_mnota
    {
      get
      {
        return this._DalObject.rec_mnota;
      }
      set
      {
        this._DalObject.rec_mnota = value;
      }
    }
    ...
    public Spep_recepcion_notas(SqlHelper SqlConfig, int UserId)
    {
      this.InitClass(SqlConfig, UserId);
      this._Taxonomies.Load(this.Security.UserId);
    }

    public Spep_recepcion_notas(SqlHelper SqlConfig, int UserId, int Id)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Id);
    }

    public Spep_recepcion_notas(SqlHelper SqlConfig, int UserId, Simplep_recepcion_notas Simple)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Simple);
    }
    ...
    public virtual void Load(Simplep_recepcion_notas Simple)
    {
    ...
    public virtual void Delete()
    {
      if (this._Relations.Count != 0)
        throw new RuntimeException("The p_recepcion_notas has dependencies.");
      if (this._AutoCommit)
        this.BeginTran();
      try
      {
    ...
    public Dalp_recepcion_notas GetDalObject()
    {
    ...
    public BaseObject NewChild(ISpecialization Child, Slbf.ValueCollection Values)
    {
      ...
      if (this._DalObject.Id == 0)
        throw new InvalidObjectException(this.GetObject(), "The p_recepcion_notas is null.");
    ...
    {
      this._DalObject = new Dalp_recepcion_notas(SqlConfig, UserId);
    ...
    {
      p_recepcion_notas pRecepcionNotas = (p_recepcion_notas) Object;
      pRecepcionNotas.Name = this.Name;
      pRecepcionNotas.rec_iidrecepcion = this.rec_iidrecepcion;
      pRecepcionNotas.rec_itipo = this.rec_itipo;
      pRecepcionNotas.rec_mnota = this.rec_mnota;
    }
