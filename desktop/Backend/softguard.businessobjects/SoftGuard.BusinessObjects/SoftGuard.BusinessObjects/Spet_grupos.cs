...
// Type: SoftGuard.BusinessObjects.Spet_grupos
...
{
  public abstract class Spet_grupos : ISpecialization, ICanCopyProperties
  {
    private bool _AutoCommit = false;
    protected Dalt_grupos _DalObject;
    ...
    public string gru_ccodigo
    {
      get
      {
        return this._DalObject.gru_ccodigo;
      }
      set
      {
        this._DalObject.gru_ccodigo = value;
      }
    }
    ...
    public string gru_cdescripcion
    {
      get
      {
        return this._DalObject.gru_cdescripcion;
      }
      set
      {
        this._DalObject.gru_cdescripcion = value;
      }
    }
    ...
    public Spet_grupos(SqlHelper SqlConfig, int UserId)
    {
      this.InitClass(SqlConfig, UserId);
      this._Taxonomies.Load(this.Security.UserId);
    }

    public Spet_grupos(SqlHelper SqlConfig, int UserId, int Id)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Id);
    }

    public Spet_grupos(SqlHelper SqlConfig, int UserId, Simplet_grupos Simple)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Simple);
    }
    ...
    public virtual void Load(Simplet_grupos Simple)
    {
    ...
    public virtual void Delete()
    {
      if (this._Relations.Count != 0)
        throw new RuntimeException("The t_grupos has dependencies.");
      if (this._AutoCommit)
        this.BeginTran();
      try
      {
    ...
    public Dalt_grupos GetDalObject()
    {
    ...
    public IEnumerable<Simplet_grupos> GetByParent(string ObjectType, int ObjectId)
    {
      return this._DalObject.GetByParent(ObjectType, ObjectId);
    }

    public IEnumerable<Simplet_grupos> GetByChild(string ObjectType, int ObjectId)
    {
    ...
    public BaseObject NewChild(ISpecialization Child, Slbf.ValueCollection Values)
    {
      ...
      if (this._DalObject.Id == 0)
        throw new InvalidObjectException(this.GetObject(), "The t_grupos is null.");
    ...
    {
      this._DalObject = new Dalt_grupos(SqlConfig, UserId);
    ...
    {
      t_grupos tGrupos = (t_grupos) Object;
      tGrupos.Name = this.Name;
      tGrupos.gru_ccodigo = this.gru_ccodigo;
      tGrupos.gru_cdescripcion = this.gru_cdescripcion;
    }
