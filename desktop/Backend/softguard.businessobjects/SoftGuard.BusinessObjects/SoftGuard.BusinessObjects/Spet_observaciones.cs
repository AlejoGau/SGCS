...
// Type: SoftGuard.BusinessObjects.Spet_observaciones
...
{
  public abstract class Spet_observaciones : ISpecialization, ICanCopyProperties
  {
    private bool _AutoCommit = false;
    protected Dalt_observaciones _DalObject;
    ...
    public string obs_ccodigo
    {
      get
      {
        return this._DalObject.obs_ccodigo;
      }
      set
      {
        this._DalObject.obs_ccodigo = value;
      }
    }
    ...
    public string obs_cdescripcion
    {
      get
      {
        return this._DalObject.obs_cdescripcion;
      }
      set
      {
        this._DalObject.obs_cdescripcion = value;
      }
    }
    ...
    public string obs_mobservacion
    {
      get
      {
        return this._DalObject.obs_mobservacion;
      }
      set
      {
        this._DalObject.obs_mobservacion = value;
      }
    }
    ...
    public Spet_observaciones(SqlHelper SqlConfig, int UserId)
    {
      this.InitClass(SqlConfig, UserId);
      this._Taxonomies.Load(this.Security.UserId);
    }

    public Spet_observaciones(SqlHelper SqlConfig, int UserId, int Id)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Id);
    }

    public Spet_observaciones(SqlHelper SqlConfig, int UserId, Simplet_observaciones Simple)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Simple);
    }
    ...
    public virtual void Load(Simplet_observaciones Simple)
    {
    ...
    public virtual void Delete()
    {
      if (this._Relations.Count != 0)
        throw new RuntimeException("The t_observaciones has dependencies.");
      if (this._AutoCommit)
        this.BeginTran();
      try
      {
    ...
    public Dalt_observaciones GetDalObject()
    {
    ...
    public BaseObject NewChild(ISpecialization Child, Slbf.ValueCollection Values)
    {
      ...
      if (this._DalObject.Id == 0)
        throw new InvalidObjectException(this.GetObject(), "The t_observaciones is null.");
    ...
    {
      this._DalObject = new Dalt_observaciones(SqlConfig, UserId);
    ...
    {
      t_observaciones tObservaciones = (t_observaciones) Object;
      tObservaciones.Name = this.Name;
      tObservaciones.obs_ccodigo = this.obs_ccodigo;
      tObservaciones.obs_cdescripcion = this.obs_cdescripcion;
      tObservaciones.obs_mobservacion = this.obs_mobservacion;
    }
