...
// Type: SoftGuard.BusinessObjects.Spet_serviciospatrulla
...
{
  public abstract class Spet_serviciospatrulla : ISpecialization, ICanCopyProperties
  {
    private bool _AutoCommit = false;
    protected Dalt_serviciospatrulla _DalObject;
    ...
    public string tsp_ccodigo
    {
      get
      {
        return this._DalObject.tsp_ccodigo;
      }
      set
      {
        this._DalObject.tsp_ccodigo = value;
      }
    }
    ...
    public string tsp_cdescripcion
    {
      get
      {
        return this._DalObject.tsp_cdescripcion;
      }
      set
      {
        this._DalObject.tsp_cdescripcion = value;
      }
    }
    ...
    public string tsp_cpathicon
    {
      get
      {
        return this._DalObject.tsp_cpathicon;
      }
      set
      {
        this._DalObject.tsp_cpathicon = value;
      }
    }
    ...
    public Spet_serviciospatrulla(SqlHelper SqlConfig, int UserId)
    {
      this.InitClass(SqlConfig, UserId);
      this._Taxonomies.Load(this.Security.UserId);
    }

    public Spet_serviciospatrulla(SqlHelper SqlConfig, int UserId, int Id)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Id);
    }

    public Spet_serviciospatrulla(SqlHelper SqlConfig, int UserId, Simplet_serviciospatrulla Simple)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Simple);
    }
    ...
    public virtual void Load(Simplet_serviciospatrulla Simple)
    {
    ...
    public virtual void Delete()
    {
      if (this._Relations.Count != 0)
        throw new RuntimeException("The t_serviciospatrulla has dependencies.");
      if (this._AutoCommit)
        this.BeginTran();
      try
      {
    ...
    public Dalt_serviciospatrulla GetDalObject()
    {
    ...
    public BaseObject NewChild(ISpecialization Child, Slbf.ValueCollection Values)
    {
      ...
      if (this._DalObject.Id == 0)
        throw new InvalidObjectException(this.GetObject(), "The t_serviciospatrulla is null.");
    ...
    {
      this._DalObject = new Dalt_serviciospatrulla(SqlConfig, UserId);
    ...
    {
      t_serviciospatrulla serviciospatrulla = (t_serviciospatrulla) Object;
