...
// Type: SoftGuard.BusinessObjects.SpeSchedulerTemplate
...
{
  public abstract class SpeSchedulerTemplate : ISpecialization, ICanCopyProperties
  {
    private bool _AutoCommit = false;
    protected DalSchedulerTemplate _DalObject;
    ...
    public SpeSchedulerTemplate(SqlHelper SqlConfig, int UserId)
    {
      this.InitClass(SqlConfig, UserId);
      this._Taxonomies.Load(this.Security.UserId);
    }

    public SpeSchedulerTemplate(SqlHelper SqlConfig, int UserId, int Id)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Id);
    }

    public SpeSchedulerTemplate(SqlHelper SqlConfig, int UserId, SimpleSchedulerTemplate Simple)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Simple);
    }
    ...
    public virtual void Load(SimpleSchedulerTemplate Simple)
    {
    ...
    public virtual void Delete()
    {
      if (this._Relations.Count != 0)
        throw new RuntimeException("The SchedulerTemplate has dependencies.");
      if (this._AutoCommit)
        this.BeginTran();
      try
      {
    ...
    public DalSchedulerTemplate GetDalObject()
    {
    ...
    public IEnumerable<SimpleSchedulerTemplate> GetByParent(string ObjectType, int ObjectId)
    {
      return this._DalObject.GetByParent(ObjectType, ObjectId);
    }

    public IEnumerable<SimpleSchedulerTemplate> GetByChild(string ObjectType, int ObjectId)
    {
    ...
    public BaseObject NewChild(ISpecialization Child, Slbf.ValueCollection Values)
    {
      ...
      if (this._DalObject.Id == 0)
        throw new InvalidObjectException(this.GetObject(), "The SchedulerTemplate is null.");
    ...
    {
      this._DalObject = new DalSchedulerTemplate(SqlConfig, UserId);
    ...
    {
      SchedulerTemplate schedulerTemplate = (SchedulerTemplate) Object;
