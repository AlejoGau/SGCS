...
// Type: SoftGuard.BusinessObjects.t_redirector
...
{
  public class t_redirector : Spet_redirector
  {
    public t_redirector(SqlHelper SqlConfig, int UserId)
      : base(SqlConfig, UserId)
    {
    }

    public t_redirector(SqlHelper SqlConfig, string Token)
      : base(SqlConfig, UserService.GetId(Token))
    {
      this._DalObject.Token = Token ?? UserService.GetContextToken((object) null);
    }

    public t_redirector(SqlHelper SqlConfig, int UserId, int Id)
      : base(SqlConfig, UserId, Id)
    {
    }

    public t_redirector(SqlHelper SqlConfig, int UserId, Simplet_redirector Simple)
