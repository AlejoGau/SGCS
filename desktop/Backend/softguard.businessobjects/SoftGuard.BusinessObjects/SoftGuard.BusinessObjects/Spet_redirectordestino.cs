...
// Type: SoftGuard.BusinessObjects.Spet_redirectordestino
...
{
  public abstract class Spet_redirectordestino : ISpecialization, ICanCopyProperties
  {
    private bool _AutoCommit = false;
    protected Dalt_redirectordestino _DalObject;
    ...
    public string rrd_cnombre
    {
      get
      {
        return this._DalObject.rrd_cnombre;
      }
      set
      {
        this._DalObject.rrd_cnombre = value;
      }
    }
    ...
    public string rrd_curl
    {
      get
      {
        return this._DalObject.rrd_curl;
      }
      set
      {
        this._DalObject.rrd_curl = value;
      }
    }
    ...
    public string rrd_cconfig
    {
      get
      {
        return this._DalObject.rrd_cconfig;
      }
      set
      {
        this._DalObject.rrd_cconfig = value;
      }
    }
    ...
    public string rrd_cmetadata
    {
      get
      {
        return this._DalObject.rrd_cmetadata;
      }
      set
      {
        this._DalObject.rrd_cmetadata = value;
      }
    }
    ...
    public Spet_redirectordestino(SqlHelper SqlConfig, int UserId)
    {
      this.InitClass(SqlConfig, UserId);
      this._Taxonomies.Load(this.Security.UserId);
    }

    public Spet_redirectordestino(SqlHelper SqlConfig, int UserId, int Id)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Id);
    }

    public Spet_redirectordestino(SqlHelper SqlConfig, int UserId, Simplet_redirectordestino Simple)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Simple);
    }
    ...
    public virtual void Load(Simplet_redirectordestino Simple)
    {
    ...
    public virtual void Delete()
    {
      if (this._Relations.Count != 0)
        throw new RuntimeException("The t_redirectordestino has dependencies.");
      if (this._AutoCommit)
        this.BeginTran();
      try
      {
    ...
    public Dalt_redirectordestino GetDalObject()
    {
    ...
    public BaseObject NewChild(ISpecialization Child, Slbf.ValueCollection Values)
    {
      ...
      if (this._DalObject.Id == 0)
        throw new InvalidObjectException(this.GetObject(), "The t_redirectordestino is null.");
    ...
    {
      this._DalObject = new Dalt_redirectordestino(SqlConfig, UserId);
    ...
    {
      t_redirectordestino redirectordestino = (t_redirectordestino) Object;
