...
// Type: SoftGuard.BusinessObjects.t_resolucionesllamada
...
{
  public class t_resolucionesllamada : Spet_resolucionesllamada
  {
    public t_resolucionesllamada(SqlHelper SqlConfig, int UserId)
      : base(SqlConfig, UserId)
    {
    }

    public t_resolucionesllamada(SqlHelper SqlConfig, string Token)
      : base(SqlConfig, UserService.GetId(Token))
    {
      this._DalObject.Token = Token ?? UserService.GetContextToken((object) null);
    }

    public t_resolucionesllamada(SqlHelper SqlConfig, int UserId, int Id)
      : base(SqlConfig, UserId, Id)
    {
    }

    public t_resolucionesllamada(SqlHelper SqlConfig, int UserId, Simplet_resolucionesllamada Simple)
