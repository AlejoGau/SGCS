...
// Type: SoftGuard.BusinessObjects.Gps
...
{
  public class Gps : SpeGps
  {
    public Gps(SqlHelper SqlConfig, int UserId)
      : base(SqlConfig, UserId)
    {
    }

    public Gps(SqlHelper SqlConfig, string Token)
      : base(SqlConfig, UserService.GetId(Token))
    {
      this._DalObject.Token = Token ?? UserService.GetContextToken((object) null);
    }

    public Gps(SqlHelper SqlConfig, int UserId, int Id)
      : base(SqlConfig, UserId, Id)
    {
    }

    public Gps(SqlHelper SqlConfig, int UserId, SimpleGps Simple)
    ...
    {
      SimpleMetaData first = MetadataManager.GetFirst(this.Id, this.GetObjectType().Id, Name);
