...
// Type: SoftGuard.BusinessObjects.Spet_videoidXtrainfo
...
{
  public abstract class Spet_videoidXtrainfo : ISpecialization, ICanCopyProperties
  {
    private bool _AutoCommit = false;
    protected Dalt_videoidXtrainfo _DalObject;
    ...
    public int tvi_iid
    {
      get
      {
        return this._DalObject.tvi_iid;
      }
      set
      {
        this._DalObject.tvi_iid = value;
      }
    }
    ...
    public int tvi_iLauncher
    {
      get
      {
        return this._DalObject.tvi_iLauncher;
      }
      set
      {
        this._DalObject.tvi_iLauncher = value;
      }
    }
    ...
    public string tvi_cConfig
    {
      get
      {
        return this._DalObject.tvi_cConfig;
      }
      set
      {
        this._DalObject.tvi_cConfig = value;
      }
    }
    ...
    public Spet_videoidXtrainfo(SqlHelper SqlConfig, int UserId)
    {
      this.InitClass(SqlConfig, UserId);
      this._Taxonomies.Load(this.Security.UserId);
    }

    public Spet_videoidXtrainfo(SqlHelper SqlConfig, int UserId, int Id)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Id);
    }

    public Spet_videoidXtrainfo(SqlHelper SqlConfig, int UserId, Simplet_videoidXtrainfo Simple)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Simple);
    }
    ...
    public virtual void Load(Simplet_videoidXtrainfo Simple)
    {
    ...
    public virtual void Delete()
    {
      if (this._Relations.Count != 0)
        throw new RuntimeException("The t_videoidXtrainfo has dependencies.");
      if (this._AutoCommit)
        this.BeginTran();
      try
      {
    ...
    public Dalt_videoidXtrainfo GetDalObject()
    {
    ...
    public BaseObject NewChild(ISpecialization Child, Slbf.ValueCollection Values)
    {
      ...
      if (this._DalObject.Id == 0)
        throw new InvalidObjectException(this.GetObject(), "The t_videoidXtrainfo is null.");
    ...
    {
      this._DalObject = new Dalt_videoidXtrainfo(SqlConfig, UserId);
    ...
    {
      t_videoidXtrainfo tVideoidXtrainfo = (t_videoidXtrainfo) Object;
      tVideoidXtrainfo.Name = this.Name;
      tVideoidXtrainfo.tvi_iid = this.tvi_iid;
      tVideoidXtrainfo.tvi_iLauncher = this.tvi_iLauncher;
      tVideoidXtrainfo.tvi_cConfig = this.tvi_cConfig;
    }
