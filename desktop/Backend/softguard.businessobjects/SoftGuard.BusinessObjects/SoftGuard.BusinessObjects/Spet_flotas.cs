...
// Type: SoftGuard.BusinessObjects.Spet_flotas
...
{
  public abstract class Spet_flotas : ISpecialization, ICanCopyProperties
  {
    private bool _AutoCommit = false;
    protected Dalt_flotas _DalObject;
    ...
    public string flo_ccodigo
    {
      get
      {
        return this._DalObject.flo_ccodigo;
      }
      set
      {
        this._DalObject.flo_ccodigo = value;
      }
    }
    ...
    public string flo_cdescripcion
    {
      get
      {
        return this._DalObject.flo_cdescripcion;
      }
      set
      {
        this._DalObject.flo_cdescripcion = value;
      }
    }
    ...
    public Spet_flotas(SqlHelper SqlConfig, int UserId)
    {
      this.InitClass(SqlConfig, UserId);
      this._Taxonomies.Load(this.Security.UserId);
    }

    public Spet_flotas(SqlHelper SqlConfig, int UserId, int Id)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Id);
    }

    public Spet_flotas(SqlHelper SqlConfig, int UserId, Simplet_flotas Simple)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Simple);
    }
    ...
    public virtual void Load(Simplet_flotas Simple)
    {
    ...
    public virtual void Delete()
    {
      if (this._Relations.Count != 0)
        throw new RuntimeException("The t_flotas has dependencies.");
      if (this._AutoCommit)
        this.BeginTran();
      try
      {
    ...
    public Dalt_flotas GetDalObject()
    {
    ...
    public IEnumerable<Simplet_flotas> GetByParent(string ObjectType, int ObjectId)
    {
      return this._DalObject.GetByParent(ObjectType, ObjectId);
    }

    public IEnumerable<Simplet_flotas> GetByChild(string ObjectType, int ObjectId)
    {
    ...
    public BaseObject NewChild(ISpecialization Child, Slbf.ValueCollection Values)
    {
      ...
      if (this._DalObject.Id == 0)
        throw new InvalidObjectException(this.GetObject(), "The t_flotas is null.");
    ...
    {
      this._DalObject = new Dalt_flotas(SqlConfig, UserId);
    ...
    {
      t_flotas tFlotas = (t_flotas) Object;
      tFlotas.Name = this.Name;
      tFlotas.flo_ccodigo = this.flo_ccodigo;
      tFlotas.flo_cdescripcion = this.flo_cdescripcion;
    }
