...
// Type: SoftGuard.BusinessObjects.Spet_LineasXPuerto
...
{
  public abstract class Spet_LineasXPuerto : ISpecialization, ICanCopyProperties
  {
    private bool _AutoCommit = false;
    protected Dalt_LineasXPuerto _DalObject;
    ...
    public int lxp_iAlias
    {
      get
      {
        return this._DalObject.lxp_iAlias;
      }
      set
      {
        this._DalObject.lxp_iAlias = value;
      }
    }
    ...
    public Decimal lxp_nLinea
    {
      get
      {
        return this._DalObject.lxp_nLinea;
      }
      set
      {
        this._DalObject.lxp_nLinea = value;
      }
    }
    ...
    public Decimal lxp_nEstado
    {
      get
      {
        return this._DalObject.lxp_nEstado;
      }
      set
      {
        this._DalObject.lxp_nEstado = value;
      }
    }
    ...
    public Spet_LineasXPuerto(SqlHelper SqlConfig, int UserId)
    {
      this.InitClass(SqlConfig, UserId);
      this._Taxonomies.Load(this.Security.UserId);
    }

    public Spet_LineasXPuerto(SqlHelper SqlConfig, int UserId, int Id)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Id);
    }

    public Spet_LineasXPuerto(SqlHelper SqlConfig, int UserId, Simplet_LineasXPuerto Simple)
    {
      this.InitClass(SqlConfig, UserId);
      this.Load(Simple);
    }
    ...
    public virtual void Load(Simplet_LineasXPuerto Simple)
    {
    ...
    public virtual void Delete()
    {
      if (this._Relations.Count != 0)
        throw new RuntimeException("The t_LineasXPuerto has dependencies.");
      if (this._AutoCommit)
        this.BeginTran();
      try
      {
    ...
    public Dalt_LineasXPuerto GetDalObject()
    {
    ...
    public BaseObject NewChild(ISpecialization Child, Slbf.ValueCollection Values)
    {
      ...
      if (this._DalObject.Id == 0)
        throw new InvalidObjectException(this.GetObject(), "The t_LineasXPuerto is null.");
    ...
    {
      this._DalObject = new Dalt_LineasXPuerto(SqlConfig, UserId);
    ...
    {
      t_LineasXPuerto tLineasXpuerto = (t_LineasXPuerto) Object;
      tLineasXpuerto.Name = this.Name;
      tLineasXpuerto.lxp_iAlias = this.lxp_iAlias;
      tLineasXpuerto.lxp_nLinea = this.lxp_nLinea;
      tLineasXpuerto.lxp_nEstado = this.lxp_nEstado;
    }
