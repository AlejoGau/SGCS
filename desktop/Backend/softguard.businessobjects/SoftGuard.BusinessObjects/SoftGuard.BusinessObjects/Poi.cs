...
// Type: SoftGuard.BusinessObjects.Poi
...
{
  public class Poi : SpePoi
  {
    public Poi(SqlHelper SqlConfig, int UserId)
      : base(SqlConfig, UserId)
    {
    }

    public Poi(SqlHelper SqlConfig, string Token)
      : base(SqlConfig, UserService.GetId(Token))
    {
      this._DalObject.Token = Token ?? UserService.GetContextToken((object) null);
    }

    public Poi(SqlHelper SqlConfig, int UserId, int Id)
      : base(SqlConfig, UserId, Id)
    {
    }

    public Poi(SqlHelper SqlConfig, int UserId, SimplePoi Simple)
